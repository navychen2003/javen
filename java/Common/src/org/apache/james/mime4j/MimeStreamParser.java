 ...
 * you should wrap it in a {@link org.apache.james.mime4j.james.mime4j.EOLConvertingInputStream} instance.
 *
 */
@SuppressWarnings({"all"})
public class MimeStreamParser {
	private static MimeLogger log = MimeLogger.getLogger(MimeStreamParser.class);
    ...
    public void parse(InputStream is) throws IOException {
        if (DEBUG_LOG_MESSAGE && log.isDebugEnabled()) {
            is = DecodeUtil.createLoggingInputStream(is);
        }
    ...
    private void parseEntity(InputStream is) throws IOException {
        ...
        if (bd.isMultipart()) {
            ...
            while (tempIs.hasMoreParts()) {
                ...
                if (tempIs.parentEOF()) {
                    if (log.isWarnEnabled()) {
                        log.warn("MimeStreamParser: Line " + rootStream.getLineNumber() 
            ...
            if (bd.isBase64Encoded()) {
                log.warn("MimeStreamParser: base64 encoded message/rfc822 detected");
                is = new EOLConvertingInputStream(
                		DecodeUtil.createBase64InputStream(is));
            } else if (bd.isQuotedPrintableEncoded()) {
                log.warn("MimeStreamParser: quoted-printable encoded message/rfc822 detected");
                is = new EOLConvertingInputStream(
                        DecodeUtil.createQuotedPrintableInputStream(is));
            }
            bodyDescriptors.addFirst(bd);
            parseMessage(is);
            bodyDescriptors.removeFirst();
        ...
        while (pos < sb.length()) {
            ...
            if (pos >= sb.length() - 2 || fieldChars.get(sb.charAt(pos + 2))) {
                ...
                if (!valid && log.isWarnEnabled()) {
                    log.warn("MimeStreamParser: Line " + startLineNumber 
