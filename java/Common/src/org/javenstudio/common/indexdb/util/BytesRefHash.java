...
public final class BytesRefHash {
   ...
   */
  public BytesRefHash() { 
    this(new BytePool(new BytePool.DirectAllocator()));
  }

  /**
   * Creates a new {@link BytesRefHash}
   */
  public BytesRefHash(BytePool pool) {
   ...
   * Creates a new {@link BytesRefHash}
   */
  public BytesRefHash(BytePool pool, int capacity,
    ...
    bytesUsed.addAndGet(hashSize * JvmUtil.NUM_BYTES_INT);
  }
  ...
  private boolean shrink(int targetSize) {
    ...
    if (newSize != hashSize) {
      bytesUsed.addAndGet(JvmUtil.NUM_BYTES_INT
          * -(hashSize - newSize));
      hashSize = newSize;
      ords = new int[hashSize];
      Arrays.fill(ords, -1);
  ...
  public int add(BytesRef bytes, int code) {
    ...
    if (e == -1) {
      // new entry
      final int len2 = 2 + bytes.mLength;
      if (len2 + pool.getByteUpto() > BytePool.BYTE_BLOCK_SIZE) {
        if (len2 > BytePool.BYTE_BLOCK_SIZE) {
          throw new MaxBytesLengthExceededException("bytes can be at most "
              + (BytePool.BYTE_BLOCK_SIZE - 2) + " in length; got " + bytes.mLength);
        }
        pool.nextBuffer();
      }
      final byte[] buffer = pool.getBuffer();
      final int bufferUpto = pool.getByteUpto();
      ...
      bytesStart[e] = bufferUpto + pool.getByteOffset();
      ...
      if (length < 128) {
        ...
        pool.increaseByteUpto(length + 1);
        assert length >= 0: "Length must be positive: " + length;
        System.arraycopy(bytes.mBytes, bytes.mOffset, buffer, bufferUpto + 1,
            length);
      } else {
        // 2 byte to store length
        buffer[bufferUpto] = (byte) (0x80 | (length & 0x7f));
        buffer[bufferUpto + 1] = (byte) ((length >> 7) & 0xff);
        pool.increaseByteUpto(length + 2);
        System.arraycopy(bytes.mBytes, bytes.mOffset, buffer, bufferUpto + 2,
            length);
      }
  ...
  private void rehash(final int newSize, boolean hashOnData) {
    ...
    bytesUsed.addAndGet(JvmUtil.NUM_BYTES_INT * (newSize));
    ...
    for (int i = 0; i < hashSize; i++) {
      ...
      if (e0 != -1) {
        ...
        if (hashOnData) {
          ...
          final int start = off & BytePool.BYTE_BLOCK_MASK;
          final byte[] bytes = pool.getBufferAt(off >> BytePool.BYTE_BLOCK_SHIFT);
          code = 0;
          final int len;
          int pos;
    ...
    hashMask = newMask;
    bytesUsed.addAndGet(JvmUtil.NUM_BYTES_INT * (-ords.length));
    ords = newHash;
    hashSize = newSize;
    hashHalfSize = newSize / 2;
  }
  ...
  public void reinit() {
    ...
    if (ords == null) {
      ords = new int[hashSize];
      bytesUsed.addAndGet(JvmUtil.NUM_BYTES_INT * hashSize);
    }
  }
  ...
  public static interface BytesStartArray {
    /**
   ...
   *  instance.
   */
  public static class TrackingDirectBytesStartArray implements BytesStartArray {
    ...
    public int[] clear() {
      if (bytesStart != null) {
        bytesUsed.addAndGet(-bytesStart.length * JvmUtil.NUM_BYTES_INT);
      }
    ...
    public int[] grow() {
      ...
      bytesUsed.addAndGet((bytesStart.length - oldSize) * JvmUtil.NUM_BYTES_INT);
      return bytesStart;
    }
   ...
   *  instance.
   */
  public static class DirectBytesStartArray implements BytesStartArray {
