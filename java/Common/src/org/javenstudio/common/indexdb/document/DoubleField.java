 ...
 * {@link SortField.Type#DOUBLE}. <code>DoubleField</code> 
...
public final class DoubleField extends AbstractField {
	...
	static {
		...
		TYPE_NOT_STORED.setNumericType(NumericType.DOUBLE);
	...
	static {
		...
		TYPE_STORED.setNumericType(NumericType.DOUBLE);
	...
	public DoubleField(String name, double value, Field.Store stored) {
		super(name, stored == Field.Store.YES ? TYPE_STORED : TYPE_NOT_STORED, Double.valueOf(value));
	}
	...
	public DoubleField(String name, double value, FieldType type) {
		...
		if (type.getNumericType() != NumericType.DOUBLE) 
			throw new IllegalArgumentException("type.numericType() must be DOUBLE but got " + type.getNumericType());
	}
