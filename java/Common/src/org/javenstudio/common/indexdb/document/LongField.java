 ...
 * {@link SortField.Type#LONG}. <code>LongField</code> 
...
public final class LongField extends AbstractField {
	...
	static {
		...
		TYPE_NOT_STORED.setNumericType(NumericType.LONG);
	...
	static {
		...
		TYPE_STORED.setNumericType(NumericType.LONG);
	...
	public LongField(String name, long value, Field.Store stored) {
		super(name, stored == Field.Store.YES ? TYPE_STORED : TYPE_NOT_STORED, Long.valueOf(value));
	}
	...
	public LongField(String name, long value, FieldType type) {
		...
		if (type.getNumericType() != NumericType.LONG) 
			throw new IllegalArgumentException("type.numericType() must be LONG but got " + type.getNumericType());
	}
