 ...
 * {@link SortField.Type#INT}. <code>IntField</code> 
...
public final class IntField extends AbstractField {
	...
	static {
		...
		TYPE_NOT_STORED.setNumericType(NumericType.INT);
	...
	static {
		...
		TYPE_STORED.setNumericType(NumericType.INT);
	...
	public IntField(String name, int value, Field.Store stored) {
		super(name, stored == Field.Store.YES ? TYPE_STORED : TYPE_NOT_STORED, Integer.valueOf(value));
	}
	...
	public IntField(String name, int value, FieldType type) {
		...
		if (type.getNumericType() != NumericType.INT) 
			throw new IllegalArgumentException("type.numericType() must be INT but got " + type.getNumericType());
	}
