...
public class Sort implements ISort {
	...
	public Sort(ISortField field) {
		setSortFields(field);
	}

	/** Sorts in succession by the criteria in each SortField. */
	public Sort(ISortField... fields) {
		setSortFields(fields);
	}

	/** Sets the sort to the given criteria. */
	public void setSortFields(ISortField field) {
		mFields = new ISortField[] { field };
		checkFields();
	}

	/** Sets the sort to the given criteria in succession. */
	public void setSortFields(ISortField... fields) {
		mFields = fields;
		checkFields();
	}
	...
	private void checkFields() { 
		if (mFields == null || mFields.length == 0)
			throw new IllegalArgumentException("SortFields cannot be empty");
	...
	public ISort rewrite(ISearcher searcher) throws IOException {
		...
		ISortField[] rewrittenSortFields = new ISortField[mFields.length];
		for (int i = 0; i < mFields.length; i++) {
			rewrittenSortFields[i] = mFields[i].rewrite(searcher);
			if (mFields[i] != rewrittenSortFields[i]) 
				changed = true;
		}
	...
	public String toString() {
		...
		for (int i = 0; i < mFields.length; i++) {
			buffer.append(mFields[i].toString());
	...
	public boolean equals(Object o) {
		...
		return Arrays.equals(this.mFields, other.mFields);
	}

	/** Returns a hash code value for this object. */
	@Override
	public int hashCode() {
		return 0x45aaf665 + Arrays.hashCode(mFields);
	}
