 ...
 */
public abstract class TopDocsCollector<T extends IScoreDoc> extends Collector {
	 ...
	 */
	protected PriorityQueue<T> mQueue;

	/** The total number of documents that the collector encountered. */
	protected int mTotalHits = 0;
	...
	protected ITopDocs newTopDocs(IScoreDoc[] results, int start) {
		return results == null ? EMPTY_TOPDOCS : new TopDocs(mTotalHits, results);
	}
	...
	protected int topDocsSize() {
		...
		return mTotalHits < mQueue.size() ? mTotalHits : mQueue.size();
	}
	...
	public ITopDocs getTopDocs() {
		...
		// results than pq.size(). Therefore return all results until either
		// pq.size() or totalHits.
		return getTopDocs(0, topDocsSize());
	}
	 ...
	 */
	public ITopDocs getTopDocs(int start) {
		// In case pq was populated with sentinel values, there might be less
		// results than pq.size(). Therefore return all results until either
		// pq.size() or totalHits.
		return getTopDocs(start, topDocsSize());
	}
	 ...
	 */
	public ITopDocs getTopDocs(int start, int howMany) {
		// In case pq was populated with sentinel values, there might be less
		// results than pq.size(). Therefore return all results until either
		// pq.size() or totalHits.
		...
		for (int i = mQueue.size() - start - howMany; i > 0; i--) { 
			mQueue.pop(); 
		}
