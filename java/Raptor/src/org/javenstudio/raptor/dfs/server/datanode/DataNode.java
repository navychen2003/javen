...
import org.javenstudio.raptor.dfs.DFSUtil;
import org.javenstudio.raptor.dfs.protocol.Block;
import org.javenstudio.raptor.dfs.protocol.BlockListAsLongs;
import org.javenstudio.raptor.dfs.protocol.ClientDatanodeProtocol;
import org.javenstudio.raptor.dfs.protocol.DataTransferProtocol;
...
import org.javenstudio.raptor.dfs.protocol.UnregisteredDatanodeException;
...
import org.javenstudio.raptor.dfs.server.common.GenerationStamp;
...
import org.javenstudio.raptor.dfs.server.datanode.metrics.DataNodeMetrics;
import org.javenstudio.raptor.dfs.server.namenode.FSNamesystem;
import org.javenstudio.raptor.dfs.server.namenode.NameNode;
import org.javenstudio.raptor.dfs.server.protocol.BlockCommand;
import org.javenstudio.raptor.dfs.server.protocol.BlockMetaDataInfo;
...
import org.javenstudio.raptor.dfs.server.protocol.DisallowedDatanodeException;
...
import org.javenstudio.raptor.util.Daemon;
import org.javenstudio.raptor.util.DiskChecker;
import org.javenstudio.raptor.util.ReflectionUtils;
import org.javenstudio.raptor.util.StringUtils;
import org.javenstudio.raptor.util.DiskChecker.DiskErrorException;
import org.javenstudio.raptor.util.DiskChecker.DiskOutOfSpaceException;
...
public class DataNode extends Configured 
  ...
  public static final Logger LOG = Logger.getLogger(DataNode.class);
  
  public static final int DEFAULT_PORT = 10010; 
  public static final int DEFAULT_IPCPORT = 10020; 
  public static final int DEFAULT_HTTPPORT = 10075; 
  ...
  public static final String DN_CLIENTTRACE_FORMAT =
        ...
        ", blockid: %s"; // block id
  static final Logger ClientTraceLog = LOG;
    //Logger.getLogger(DataNode.class.getName() + ".clienttrace");
  ...
  void startDataNode(Configuration conf, 
    ...
    this.machineName = DFSUtil.getLocalHostName(); 
    this.machineAddress = DFSUtil.getLocalNetworkAddress(conf); 
    if (machineName == null && conf.get("slave.host.name") != null) {
    ...
    this.socketTimeout =  conf.getInt("dfs.socket.timeout", DfsConstants.READ_TIMEOUT);
    this.socketWriteTimeout = conf.getInt("dfs.datanode.socket.write.timeout",
                                          DfsConstants.WRITE_TIMEOUT);
    ...
    this.dnRegistration = new DatanodeRegistration(machineAddress + ":" + tmpPort);
    ...
    if (simulatedFSDataset) {
        setNewStorageID(dnRegistration, machineAddress);
        ...
        try {
          ...
          this.data = (FSDatasetInterface) ReflectionUtils.newInstance(
              Class.forName("org.javenstudio.raptor.dfs.server.datanode.SimulatedFSDataset"), conf);
    ...
    this.dnRegistration.setName(machineAddress + ":" + tmpPort);
    LOG.info("Opened xceiver server at " + tmpPort);
 
    this.threadGroup = new ThreadGroup("dataXceiverServer");
    DataXceiverServer dataServer = new DataXceiverServer(ss, conf, this); 
    this.dataXceiverServer = new Daemon(threadGroup, dataServer);
    ...
    if (infoAddr != null) {
      ...
      //this.infoServer = StatusHttpServer.createDatanodeInfoServer(conf, infoHost, tmpInfoPort);
    ...
    int handlerCount = conf.getInt("dfs.datanode.handler.count", 3); 
    ipcServer = RPC.getServer(this, ipcAddr.getHostName(), ipcAddr.getPort(), handlerCount, false, conf);
    ipcServer.start();
    InetSocketAddress ipcAddress = ipcServer.getListenerAddress(); 
    dnRegistration.setIpcPort(ipcAddress.getPort());

    //this.serviceInfo = ServiceInfo.create(this, ipcAddr.getPort(), conf);
    //this.serviceInfo.addServer(this.getClass().getSimpleName(),
    //    machineAddress, ipcAddress.getPort(), handlerCount,
    //    new String[]{InterDatanodeProtocol.class.getName(),
    //                 ClientDatanodeProtocol.class.getName()});
    //this.serviceInfo.addServer(DataXceiver.class.getSimpleName(),
    //    machineAddress, selfAddr.getPort(), dataServer.getMaxXceiverCount(),
    //    new String[]{DataXceiver.class.getName()});
    //this.serviceInfo.addServer(ServerInfo.Protocol.HTTP,
    //    machineAddress, infoServer.getPort());
  ...
  public static void setNewStorageID(DatanodeRegistration dnReg, String ipaddr) {
    /* Return 
  ...
  private void register() throws IOException {
    ...
    while(shouldRun) {
      try {
        // reset name to machineAddress. Mainly for web interface.
        dnRegistration.name = machineAddress + ":" + dnRegistration.getPort();
    ...
    public void run() {
      ...
      try {
        ...
        long writeTimeout = socketWriteTimeout + 
                            DfsConstants.WRITE_TIMEOUT_EXTENSION * (targets.length-1);
  ...
  private void initServiceClient(Configuration conf) throws IOException {
    ...
    //if (!conf.getBoolean("datanode.singled.mode", false)) 
    //  this.client.startup();
  }
   ...
   */
  public static DataNode instantiateDataNode(String argv[],
                                      Configuration conf) throws IOException {
    if (conf == null)
      conf = ConfigurationFactory.create();

    /*DatanodeOptions options = StartupOptions.getDatanodeOptions();
    ...
    if (options.hasOption(options.SINGLED)) {
      conf.setBoolean("datanode.singled.mode", true);
    }
    ...
    if (options.hasOption(options.NAMENODEADDRESS)) {
      ...
      String address = ClusterHelper.parseAddressPort(conf, val, NameNode.DEFAULT_PORT);
      FileSystem.setDefaultUri(conf, address); 
    }
    if (options.hasOption(options.DATANODEADDRESS)) {
      String val = options.getOptionValue(options.DATANODEADDRESS);
      String address = ClusterHelper.parseAddressPort(conf, val, DEFAULT_PORT);
      conf.set("dfs.datanode.address", address);
    }
    if (options.hasOption(options.IPCADDRESS)) {
      String val = options.getOptionValue(options.IPCADDRESS);
      String address = ClusterHelper.parseAddressPort(conf, val, DEFAULT_IPCPORT);
      conf.set("dfs.datanode.ipc.address", address);
    }
    if (options.hasOption(options.HTTPADDRESS)) {
      ...
      conf.set("dfs.datanode.http.address", address);
    }
    if (options.hasOption(options.THREADCOUNT)) {
      ...
      conf.setInt("dfs.datanode.handler.count", threadcount);
    }
    if (options.hasOption(options.DATADIRS)) {
      String[] vals = options.getOptionValues(options.DATADIRS);
      StringBuffer sbuf = new StringBuffer(); 
      for (int i=0; vals != null && i < vals.length; i++) {
        if (sbuf.length() > 0) sbuf.append(','); 
        sbuf.append(vals[i]); 
      }
      conf.set("dfs.data.dir", sbuf.toString()); 
    }*/

    DFSShell.initDefaultUri(conf, false); 
  ...
  public static void doMain(Configuration conf, String argv[]) {
    try {
      //ConfigurationFactory.setQuietMode(StartupOptions.getDatanodeOptions().hasCommandOption(argv));
      StringUtils.startupShutdownMessage(DataNode.class, argv, LOG);
      DataNode datanode = createDataNode(argv, conf);
      if (datanode != null)
        datanode.join();
    } catch (Throwable e) {
