...
import org.javenstudio.raptor.dfs.protocol.FSConstants.UpgradeAction;
...
import org.javenstudio.raptor.fs.shell.Command;
...
public class DFSAdmin extends FsShell {
  ...
  public void report() throws IOException {
    if (fs instanceof DistributedFileSystem) {
      ...
      double presentUsed = presentCapacity > 0 ? ((1.0 * used) / presentCapacity) : 0;
      ...
      System.out.println("DFS Used%: "
                         + StringUtils.limitDecimalTo2(presentUsed * 100)
                         + "%");
   ...
   * @see org.javenstudio.raptor.dfs.protocol.ClientProtocol#saveNamespace()
   */
  ...
  private void printHelp(String cmd) {
    String summary = "java dfsadmin is the command to execute DFS administrative commands.\n" +
      "The full syntax is: \n\n" +
      "java dfsadmin [-report] [-safemode <enter | leave | get | wait>]\n" +
      "\t[-saveNamespace]\n" +
      "\t[-refreshNodes]\n" +
    ...
    String metaSave = "-metasave <filename>: \tSave Namenode's primary data structures\n" +
      "\t\tto <filename> in the directory specified by hawk.log.dir property.\n" +
  ...
  private static void printUsage(String cmd) {
    if ("-report".equals(cmd)) {
      ...
      System.err.println("Usage: java DFSAdmin [command] <options...> <parameters...>");
      System.err.println("the commands and options are");
      System.err.println(" [-report]                          Reports basic filesystem information and statistics");
      System.err.println(" [-safemode enter | leave | get | wait]  Safe mode maintenance command");
      System.err.println(" [-saveNamespace]                   Save current namespace into storage directories and reset edits log");
      System.err.println(" [-refreshNodes]                    Updates the set of hosts allowed to connect to namenode");
      System.err.println(" [-finalizeUpgrade]                 Finalize upgrade of DFS");
      System.err.println(" [-upgradeProgress status | details | force]  request current distributed upgrade status");
      System.err.println(" [-metasave filename]               Save Namenode's primary data structures");
      System.err.println(" [-refreshServiceAcl]               Reload the service-level authorization policy file");
      System.err.println(" ["+SetQuotaCommand.USAGE+"]       Set the quota <quota> for each directory");
      System.err.println(" ["+ClearQuotaCommand.USAGE+"]  Clear the quota for each directory");
      System.err.println(" ["+SetSpaceQuotaCommand.USAGE+"]  Set the disk space quota <quota> for each directory");
      System.err.println(" ["+ClearSpaceQuotaCommand.USAGE+"]   Clear the disk space quota for each directory");      
      System.err.println(" [-help [cmd]]                      Displays help for the given command or all commands");
      System.err.println();
  ...
  public static void main(String[] argv) throws Exception {
	doMain(ConfigurationFactory.create(true), argv);
  }
