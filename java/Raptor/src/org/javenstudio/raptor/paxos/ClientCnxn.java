...
import org.javenstudio.raptor.paxos.Paxos.WatchRegistration;
import org.javenstudio.raptor.paxos.common.PathUtils;
import org.javenstudio.raptor.paxos.proto.AuthPacket;
import org.javenstudio.raptor.paxos.proto.ConnectRequest;
import org.javenstudio.raptor.paxos.proto.ConnectResponse;
...
import org.javenstudio.raptor.paxos.server.ByteBufferInputStream;
...
public class ClientCnxn {
     ...
     * the environment variable "paxos.disableAutoWatchReset" to "true" 
     */
    ...
    static {
        ...
        disableAutoWatchReset =
            Boolean.getBoolean("paxos.disableAutoWatchReset");
        if (LOG.isDebugEnabled()) {
            LOG.debug("paxos.disableAutoWatchReset is "
                    + disableAutoWatchReset);
        }
        packetLen = Integer.getInteger("paxos.maxbuffer", 4096 * 1024);
    }
    ...
    static class Packet {
        ...
        Packet(RequestHeader header, ReplyHeader replyHeader, Writable record,
                Writable response, ByteBuffer bb,
                WatchRegistration watchRegistration) {
            this.header = header;
            ...
            if (bb != null) {
                ...
                try {
                    ...
                    DataOutput boa = new DataOutputStream(baos); 
                    boa.writeInt(-1); // We'll fill this in later
                    header.write(boa);
                    if (record != null) {
                        record.write(boa);
                    }
     ...
     * @param paxos
     *                the paxos object that this connection is related to.
     * @param watcher watcher for this connection
     * @throws IOException
     */
    public ClientCnxn(String hosts, int sessionTimeout, Paxos paxos,
            ClientWatchManager watcher) throws IOException {
        this(hosts, sessionTimeout, paxos, watcher, 0, new byte[16]);
    }
    ...
    final static UncaughtExceptionHandler uncaughtExceptionHandler = 
    ...
    class EventThread extends Thread {
         ...
         */
        private volatile PaxosState sessionState = PaxosState.Disconnected;
            ...
            try {
                while (true) {
                    ...
                    try {
                    	...
                    	if (LOG.isErrorEnabled())
                    		LOG.error("Caught unexpected throwable: " + t, t);
                    }
                }
    ...
    private void conLossPacket(Packet p) {
        ...
        switch(paxos.state) {
        case AUTH_FAILED:
            p.replyHeader.setErr(PaxosException.Code.AUTHFAILED.intValue());
            break;
        case CLOSED:
            p.replyHeader.setErr(PaxosException.Code.SESSIONEXPIRED.intValue());
            break;
        default:
            p.replyHeader.setErr(PaxosException.Code.CONNECTIONLOSS.intValue());
        }
        finishPacket(p);
    }
    ...
    class SendThread extends Thread {
        ...
        void readConnectResult() throws IOException {
            ByteBufferInputStream bbis = new ByteBufferInputStream(
                    incomingBuffer);
            DataInput bbia = new DataInputStream(bbis); 
            ConnectResponse conRsp = ConnectResponse.read(bbia);
            negotiatedSessionTimeout = conRsp.getTimeOut();
            if (negotiatedSessionTimeout <= 0) {
                paxos.state = States.CLOSED;

                eventThread.queueEvent(new WatchedEvent(
                        Watcher.Event.EventType.None,
                        Watcher.Event.PaxosState.Expired, null));
                throw new SessionExpiredException(
                        "Unable to reconnect to Paxos service, session 0x"
            ...
            eventThread.queueEvent(new WatchedEvent(Watcher.Event.EventType.None,
                    Watcher.Event.PaxosState.SyncConnected, null));
        }
        ...
        void readResponse() throws IOException {
            ByteBufferInputStream bbis = new ByteBufferInputStream(
                    incomingBuffer);
            DataInput bbia = new DataInputStream(bbis); 
            ReplyHeader replyHdr = ReplyHeader.read(bbia);
            ...
            if (replyHdr.getXid() == -1) {
                ...
                WatcherEvent event = WatcherEvent.read(bbia);
            ...
            try {
                if (packet.header.getXid() != replyHdr.getXid()) {
                    packet.replyHeader.setErr(
                            PaxosException.Code.CONNECTIONLOSS.intValue());
                ...
                if (packet.response != null && replyHdr.getErr() == 0) {
                    packet.response.readFields(bbia);
                }
        ...
        private void primeConnection(SelectionKey k) throws IOException {
            ...
            synchronized (outgoingQueue) {
                ...
                if (!disableAutoWatchReset &&
                        (!paxos.getDataWatches().isEmpty()
                         || !paxos.getExistWatches().isEmpty()
                         || !paxos.getChildWatches().isEmpty()))
                {
                    SetWatches sw = new SetWatches(lastZxid,
                            paxos.getDataWatches(),
                            paxos.getExistWatches(),
                            paxos.getChildWatches());
                    RequestHeader h = new RequestHeader();
                    h.setType(PaxosDefs.OpCode.setWatches);
                    h.setXid(-8);
        ...
        public void run() {
            ...
            while (paxos.state.isAlive()) {
                try {
                    ...
                    if (paxos.state != States.CONNECTED) {
                    ...
                    if (closing) {
                        ...
                        cleanup();
                        if (paxos.state.isAlive()) {
                            eventThread.queueEvent(new WatchedEvent(
                                    Event.EventType.None,
                                    Event.PaxosState.Disconnected,
                                    null));
                        }
            ...
            PaxosTrace.logTraceMessage(LOG, PaxosTrace.getTextTraceLevel(),
    ...
    public void close() throws IOException {
        ...
        try {
            RequestHeader h = new RequestHeader();
            h.setType(PaxosDefs.OpCode.closeSession);
    ...
    public ReplyHeader submitRequest(RequestHeader h, Writable request,
            Writable response, WatchRegistration watchRegistration)
            throws InterruptedException {
    ...
    Packet queuePacket(RequestHeader h, ReplyHeader r, Writable request,
            Writable response, AsyncCallback cb, String clientPath,
