...
import org.javenstudio.raptor.paxos.server.quorum.Leader.Proposal;
...
public class LearnerHandler extends Thread {
    ...
    /**
     * Paxos server identifier of this learner
     */
    ...
    private DataInput ia;

    private DataOutput oa;
    ...
    private void sendPackets() throws InterruptedException {
        long traceMask = PaxosTrace.SERVER_PACKET_TRACE_MASK;
        while (true) {
            try {
                QuorumPacket p;
                ...
                if (LOG.isTraceEnabled()) {
                    PaxosTrace.logQuorumPacket(LOG, traceMask, 'o', p);
                }
                p.write(oa);
            } catch (IOException e) {
                if (!sock.isClosed()) {
    ...
    static public String packetToString(QuorumPacket p) {
        ...
        case Leader.PROPOSAL:
            type = "PROPOSAL";
            DataInput ia = new DataInputStream(new ByteArrayInputStream(p.getData()));
    ...
    public void run() {
        try {
            ia = new DataInputStream(new BufferedInputStream(sock.getInputStream()));
            bufferedOutput = new BufferedOutputStream(sock.getOutputStream());
            oa = new DataOutputStream(bufferedOutput);

            QuorumPacket qp = QuorumPacket.read(ia);
             ...
             * send a diff to the 
             */ 
            LinkedList<Proposal> proposals = leader.zk.getPaxosDatabase().getCommittedLog();
            synchronized(proposals) {
                if (proposals.size() != 0) {
                    if ((leader.zk.getPaxosDatabase().getmaxCommittedLog() >= peerLastZxid)
                            && (leader.zk.getPaxosDatabase().getminCommittedLog() <= peerLastZxid)) {
                        packetToSend = Leader.DIFF;
                        zxidToSend = leader.zk.getPaxosDatabase().getmaxCommittedLog();
            ...
            if (logTxns && (peerLastZxid > leader.zk.getPaxosDatabase().getmaxCommittedLog())) {
            ...
            newLeaderQP.write(oa);
            bufferedOutput.flush();
            
           
            (new QuorumPacket(packetToSend, zxidToSend, null, null)).write(oa);
            bufferedOutput.flush();
            ...
            if (packetToSend == Leader.SNAP) {
                ...
                // Dump data to peer
                leader.zk.getPaxosDatabase().serializeSnapshot(oa);
                UTF8.writeString(oa, "BenWasHere");
            }
            bufferedOutput.flush();
            ...
            while (true) {
                ...
                if (LOG.isTraceEnabled()) {
                    PaxosTrace.logQuorumPacket(LOG, traceMask, 'i', qp);
                }
