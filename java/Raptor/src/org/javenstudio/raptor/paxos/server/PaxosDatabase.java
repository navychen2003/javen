...
import org.javenstudio.raptor.paxos.server.persistence.FileTxnSnapLog.PlayBackListener;
...
/**
 * This class maintains the in memory database of paxos
 * server states that includes the sessions, datatree and the
 * committed logs. It is booted up  after reading the logs
 * and snapshots from the disk.
 */
public class PaxosDatabase {
    private static final Logger LOG = Logger.getLogger(PaxosDatabase.class);
     ...
     */
    public PaxosDatabase(FileTxnSnapLog snapLog) {
    ...
    public long loadDataBase() throws IOException {
        PlayBackListener listener = new PlayBackListener(){
            public void onTxnLoaded(TxnHeader hdr, Writable txn){
    ...
    public void addCommittedProposal(Request request) {
        synchronized (committedLog) {
            ...
            try {
                request.hdr.write(boa);
                if (request.txn != null) {
                    request.txn.write(boa);
                }
                baos.close();
            } catch (IOException e) {
     ...
     * datatree/zkdatabase
     */
    public ProcessTxnResult processTxn(TxnHeader hdr, Writable txn) {
     ...
     */
    public Stat statNode(String path, ServerCnxn serverCnxn) throws PaxosException.NoNodeException {
     ...
     * @throws IOException
     */
    public void deserializeSnapshot(DataInput ia) throws IOException {
        clear();
     ...
     */
    public void serializeSnapshot(DataOutput oa) throws IOException,
