...
public class SessionTrackerImpl extends Thread implements SessionTracker {
    ...
    synchronized public boolean touchSession(long sessionId, int timeout) {
        if (LOG.isTraceEnabled()) {
            PaxosTrace.logTraceMessage(LOG,
                                     PaxosTrace.CLIENT_PING_TRACE_MASK,
    ...
    synchronized public void checkSession(long sessionId, Object owner) throws PaxosException.SessionExpiredException, PaxosException.SessionMovedException {
        SessionImpl session = sessionsById.get(sessionId);
        if (session == null) {
            throw new PaxosException.SessionExpiredException();
        }
        if (session.owner == null) {
            session.owner = owner;
        } else if (session.owner != owner) {
            throw new PaxosException.SessionMovedException();
        }
    }
