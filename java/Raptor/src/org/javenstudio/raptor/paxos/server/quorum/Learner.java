...
public class Learner {       
    ...
    protected DataInput leaderIs;
    protected DataOutput leaderOs;    
    ...
    void readPacket(QuorumPacket pp) throws IOException {
        ...
        if (LOG.isTraceEnabled()) {
            PaxosTrace.logQuorumPacket(LOG, traceMask, 'i', pp);
        }
    }
    ...
    throws IOException, ConnectException, InterruptedException {
        ...
        leaderIs = new DataInputStream(new BufferedInputStream(sock.getInputStream()));
        bufferedOutput = new BufferedOutputStream(sock.getOutputStream());
        leaderOs = new DataOutputStream(bufferedOutput);
    } 
    ...
    protected void syncWithLeader(long newLeaderZxid) throws IOException, InterruptedException{
        ...
        synchronized (zk) {
            if (qp.getType() == Leader.DIFF) {
                ...
                zk.getPaxosDatabase().clear();
                zk.getPaxosDatabase().deserializeSnapshot(leaderIs);
                String signature = UTF8.readString(leaderIs);
                ...
                boolean truncated=zk.getPaxosDatabase().truncateLog(qp.getZxid());
                if (!truncated) {
            ...
            zk.getPaxosDatabase().setlastProcessedZxid(newLeaderZxid);
        }
    ...
    public void shutdown() {
        // set the paxos server to null
        self.cnxnFactory.setPaxosServer(null);
        // clear all the connections
        self.cnxnFactory.clear();
        // shutdown previous paxos
        if (zk != null) {
            zk.shutdown();
        }
    }
