...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    static public class Factory extends Thread {
        ...
        */
         ...
         * limits of the operating system). startup(server) must be called subsequently.
         * @param port
         * @throws IOException
         */
         ...
         * startup(server) must be called subsequently.
        ...
        public void startup(PaxosServer server) throws IOException,
                InterruptedException {
            start();
            server.startup();
            setPaxosServer(server);
        }

        public void setPaxosServer(PaxosServer server) {
            this.server = server;
            if (server != null) {
                this.outstandingLimit = server.getGlobalOutstandingLimit();
        ...
        protected NIOServerCnxn createConnection(SocketChannel sock,
            ...
            return new NIOServerCnxn(server, sock, sk, this);
        }
    ...
    /** The PaxosServer for this connection. May be null if the server
    ...
    void doIO(SelectionKey k) throws InterruptedException {
        try {
            ...
            LOG.warn("error: " + e); // tell user why
    ...
    private void readRequest() throws IOException {
        ...
        DataInput bia = new DataInputStream(bais); 
        RequestHeader h = RequestHeader.read(bia);
        ...
        if (h.getType() == OpCode.auth) {
            ...
            PaxosServer.byteBuffer2Writable(incomingBuffer, authPacket);
            String scheme = authPacket.getScheme();
            AuthenticationProvider ap = ProviderRegistry.getProvider(scheme);
            if (ap == null
                    || (ap.handleAuthentication(this, authPacket.getAuth())
                            != PaxosException.Code.OK)) {
                if (ap == null) {
                ...
                ReplyHeader rh = new ReplyHeader(h.getXid(), 0,
                        PaxosException.Code.AUTHFAILED.intValue());
                ...
                ReplyHeader rh = new ReplyHeader(h.getXid(), 0,
                        PaxosException.Code.OK.intValue());
                sendResponse(rh, null, null);
            }
            return;
        } else {
    ...
    private void readConnectRequest() throws IOException, InterruptedException {
        DataInput bia = new DataInputStream(new ByteBufferInputStream(incomingBuffer));
        ConnectRequest connReq = ConnectRequest.read(bia);
        if (LOG.isDebugEnabled()) {
        ...
        if (zk == null) {
            throw new IOException("PaxosServer not running");
        }
        if (connReq.getLastZxidSeen() > zk.getPaxosDatabase().getDataTreeLastProcessedZxid()) {
            String msg = "Refusing session request for client "
                + sock.socket().getRemoteSocketAddress()
                + " as it has seen zxid 0x"
                + Long.toHexString(connReq.getLastZxidSeen())
                + " our last zxid is 0x"
                + Long.toHexString(zk.getPaxosDatabase().getDataTreeLastProcessedZxid())
    ...
    /*
     * See <a href="{@docRoot}/../../../docs/paxosAdmin.html#sc_zkCommands">
    ...
    private static final String ZK_NOT_SERVING =
        "This Paxos instance is not currently serving requests";
    ...
    private class TraceMaskCommand extends CommandThread {
        ...
        @Override
        public void commandRun() {
            long traceMask = PaxosTrace.getTextTraceLevel();
            pw.print(traceMask);
        }
    }
    ...
    private class StatCommand extends CommandThread {
        ...
        public void commandRun() {
            if (zk == null) {
                ...
                pw.print("Paxos version: ");
                pw.println("0.1"); //Version.getFullVersion());
                if (len == statCmd) {
                ...
                pw.print("Node count: ");
                pw.println(zk.getPaxosDatabase().getNodeCount());
            }
    ...
    private class WatchCommand extends CommandThread {
        ...
        public void commandRun() {
            if (zk == null) {
                ...
                DataTree dt = zk.getPaxosDatabase().getDataTree();
                if (len == wchsCmd) {
    ...
    throws IOException
    {
        ...
        if (len == ruokCmd) {
            ...
            PaxosTrace.setTextTraceLevel(traceMask);
    ...
    private boolean readLength(SelectionKey k) throws IOException {
        ...
        if (len < 0 || len > ByteBufferInputStream.maxBuffer) {
     ...
     * (non-Javadoc)
     *
     * @see org.javenstudio.raptor.paxos.server.ServerCnxnIface#getSessionTimeout()
     */
    ...
    public NIOServerCnxn(PaxosServer zk, SocketChannel sock,
     ...
     * (non-Javadoc)
     *
     * @see org.javenstudio.raptor.paxos.server.ServerCnxnIface#sendResponse(org.javenstudio.raptor.paxos.proto.ReplyHeader,
     *      org.apache.jute.Record, java.lang.String)
     */
    synchronized public void sendResponse(ReplyHeader h, Writable r, String tag) {
        try {
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            // Make space for length
            DataOutput bos = new DataOutputStream(baos); 
            try {
     ...
     * (non-Javadoc)
     *
     * @see org.javenstudio.raptor.paxos.server.ServerCnxnIface#process(org.javenstudio.raptor.paxos.proto.WatcherEvent)
     */
     ...
     * (non-Javadoc)
     *
     * @see org.javenstudio.raptor.paxos.server.ServerCnxnIface#getSessionId()
     */
