...
import org.javenstudio.raptor.paxos.AsyncCallback.ACLCallback;
import org.javenstudio.raptor.paxos.AsyncCallback.Children2Callback;
import org.javenstudio.raptor.paxos.AsyncCallback.ChildrenCallback;
import org.javenstudio.raptor.paxos.AsyncCallback.DataCallback;
import org.javenstudio.raptor.paxos.AsyncCallback.StatCallback;
import org.javenstudio.raptor.paxos.AsyncCallback.StringCallback;
import org.javenstudio.raptor.paxos.AsyncCallback.VoidCallback;
...
import org.javenstudio.raptor.paxos.proto.RequestHeader;
...
/**
 * This is the main class of Paxos client library. To use a Paxos
 * service, an application must first instantiate an object of Paxos class.
 * All the iterations will be done by calling the methods of Paxos class.
 ...
 * the session valid.
 * <p>
 * The application can call Paxos APIs through a client as long as the
 * session ID of the client remains valid.
 * <p>
 * If for some reason, the client fails to send heart beats to the server for a
 * prolonged period of time (exceeding the sessionTimeout value, for instance),
 * the server will expire the session, and the session ID will become invalid.
 * The client object will no longer be usable. To make Paxos API calls, the
 * application must create a new client object.
 * <p>
 * If the Paxos server the client currently connects to fails or otherwise
 * does not respond, the client will automatically try to connect to another
 * server before its session ID expires. If successful, the application can
 * continue to use the client.
 * <p>
 * Some successful Paxos API calls can leave watches on the "data nodes" in
 * the Paxos server. Other successful Paxos API calls can trigger those
 ...
 * EventNone and state sPaxosStateDisconnected.
 *
 */
public class Paxos {
    ...
    static {
        LOG = Logger.getLogger(Paxos.class);

        Environment.logEnv("Client environment:", LOG);
    }


    private final PaxosWatchManager watchManager = new PaxosWatchManager();
...
/**
     ...
     *
     * We are implementing this as a nested class of Paxos so that
     * the public methods will not be exposed as part of the Paxos client
     * API.
     */
    private static class PaxosWatchManager implements ClientWatchManager {
        ...
        /* (non-Javadoc)
         * @see ClientWatchManager#materialize(Event.PaxosState, Event.EventType, java.lang.String)
         */
        public Set<Watcher> materialize(Watcher.Event.PaxosState state,
                                        ...
                                        String clientPath)
        {
            ...
            case None:
                ...
                if (ClientCnxn.getDisableAutoResetWatch() &&
                        state != Watcher.Event.PaxosState.SyncConnected)
                {
    ...
    class ExistsWatchRegistration extends WatchRegistration {
        ...
        @Override
        protected boolean shouldAddWatch(int rc) {
            return rc == 0 || rc == PaxosException.Code.NONODE.intValue();
        }
    }
    ...
    /**
     * To create a Paxos client object, the application needs to pass a
     * connection string containing a comma separated list of host:port pairs,
     * each corresponding to a Paxos server.
     * <p>
     ...
     * has returned.
     * <p>
     * The instantiated Paxos client object will pick an arbitrary server
     ...
     */
    public Paxos(String connectString, int sessionTimeout, Watcher watcher)
        throws IOException
    {
    ...
    /**
     * The session id for this Paxos client instance. The value returned is
    ...
    /**
     * The session password for this Paxos client instance. The value
    ...
    /**
     * The negotiated session timeout for this Paxos client instance. The
     ...
     * invalid. All the ephemeral nodes in the Paxos server associated with
     ...
     * or not.
     * <p>
     * An ephemeral node will be removed by the Paxos automatically when the
     ...
     * <p>
     * If a node with the same actual path already exists in the Paxos, a
     * PaxosException with error code PaxosException.NodeExists will be
     * thrown. Note that since a different actual path is used for each
     * invocation of creating sequential node with the same path argument, the
     * call will never throw "file exists" PaxosException.
     * <p>
     * If the parent node does not exist in the Paxos, a PaxosException
     * with error code PaxosException.NoNode will be thrown.
     * <p>
     * An ephemeral node cannot have children. If the parent node of the given
     * path is ephemeral, a PaxosException with error code
     * PaxosException.NoChildrenForEphemerals will be thrown.
     * <p>
     * This operation, if successful, will trigger all the watches left on the
     * node of the given path by exists and getData API calls, and the watches
     * left on the parent node by getChildren API calls.
     * <p>
     * If a node is created successfully, the Paxos server will trigger the
     ...
     * @throws PaxosException if the server returns a non-zero error code
     * @throws PaxosException.InvalidACLException if the ACL is invalid, null, or empty
        ...
        throws PaxosException, InterruptedException
    {
        ...
        h.setType(PaxosDefs.OpCode.create);
        ...
        if (acl != null && acl.size() == 0) {
            throw new PaxosException.InvalidACLException();
        }
        request.setAcl(acl);
        ReplyHeader r = cnxn.submitRequest(h, request, response, null);
        if (r.getErr() != 0) {
            throw PaxosException.create(PaxosException.Code.get(r.getErr()),
                    clientPath);
        }
        if (cnxn.chrootPath == null) {
     ...
     * <p>
     * A PaxosException with error code PaxosException.NoNode will be thrown
     * if the nodes does not exist.
     * <p>
     * A PaxosException with error code PaxosException.BadVersion will be
     * thrown if the given version does not match the node's version.
     * <p>
     * A PaxosException with error code PaxosException.NotEmpty will be thrown
     ...
     * @throws PaxosException If the server signals an error with a non-zero
     *   return code.
     * @throws IllegalArgumentException if an invalid path is specified
     */
        ...
        throws InterruptedException, PaxosException
    {
        ...
        RequestHeader h = new RequestHeader();
        h.setType(PaxosDefs.OpCode.delete);
     ...
     * @return the stat of the node of the given path; return null if no such a
     *         node exists.
     * @throws PaxosException If the server signals an error
     * @throws InterruptedException If the server transaction is interrupted.
     * @throws IllegalArgumentException if an invalid path is specified
     */
        ...
        throws PaxosException, InterruptedException
    {
        ...
        RequestHeader h = new RequestHeader();
        h.setType(PaxosDefs.OpCode.exists);
        ...
        if (r.getErr() != 0) {
            if (r.getErr() == PaxosException.Code.NONODE.intValue()) {
                return null;
            }
            throw PaxosException.create(PaxosException.Code.get(r.getErr()),
                    clientPath);
     ...
     * @throws InterruptedException If the server transaction is interrupted.
     */
    public Stat exists(String path, boolean watch) throws PaxosException,
     ...
     * @throws PaxosException If the server signals an error with a non-zero error code
     * @throws InterruptedException If the server transaction is interrupted.
     * @throws IllegalArgumentException if an invalid path is specified
     */
    public byte[] getData(final String path, Watcher watcher, Stat stat)
        throws PaxosException, InterruptedException
     {
        ...
        RequestHeader h = new RequestHeader();
        h.setType(PaxosDefs.OpCode.getData);
        ...
        throws PaxosException, InterruptedException
    {
        ...
        RequestHeader h = new RequestHeader();
        h.setType(PaxosDefs.OpCode.setData);
        ...
        throws PaxosException, InterruptedException
    {
        ...
        RequestHeader h = new RequestHeader();
        h.setType(PaxosDefs.OpCode.getACL);
     ...
     * @throws InterruptedException If the server transaction is interrupted.
     * @throws PaxosException If the server signals an error with a non-zero error code.
     * @throws org.javenstudio.raptor.paxos.PaxosException.InvalidACLException If the acl is invalide.
     * @throws IllegalArgumentException if an invalid path is specified
     */
        ...
        throws PaxosException, InterruptedException
    {
        ...
        RequestHeader h = new RequestHeader();
        h.setType(PaxosDefs.OpCode.setACL);
    ...
    public List<String> getChildren(final String path, Watcher watcher)
        ...
        RequestHeader h = new RequestHeader();
        h.setType(PaxosDefs.OpCode.getChildren);
        ...
        throws PaxosException, InterruptedException
    {
        ...
        RequestHeader h = new RequestHeader();
        h.setType(PaxosDefs.OpCode.getChildren2);
    ...
    public void sync(final String path, VoidCallback cb, Object ctx){
        ...
        RequestHeader h = new RequestHeader();
        h.setType(PaxosDefs.OpCode.sync);
    ...
    /**
     * String representation of this Paxos client. Suitable for things
     * like logging.
     * 
