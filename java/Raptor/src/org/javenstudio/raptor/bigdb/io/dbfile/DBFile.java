...
import org.javenstudio.raptor.bigdb.KeyValue.KeyComparator;
 ...
 */
public class DBFile {
  static final Logger LOG = Logger.getLogger(DBFile.class);
  ...
  public static class Writer implements Closeable {
    ...
    public Writer(FileSystem fs, Path path, int blocksize,
      String compress, final KeyComparator comparator) throws IOException {
    ...
    public Writer(final FSDataOutputStream ostream, final int blocksize,
      final String  compress, final KeyComparator c) throws IOException {
  ...
  public static class Reader implements Closeable {
    ...
    protected static class Scanner implements DBFileScanner {
      ...
      @Override
      public String toString() {
        return "DBFileScanner for reader " + String.valueOf(reader);
      }
    }
  ...
  public static void main(String []args) throws IOException {
    /*try {
      ...
      options.addOption("f", "file", true,
        "File to scan. Pass full-path; e.g. hdfs://a:9000/bigdb/.META./12/34");
      options.addOption("r", "region", true,
        "Region to scan. Pass region name; e.g. '.META.,,1'");
      if (args.length == 0) {
        HelpFormatter formatter = new HelpFormatter();
        formatter.printHelp("DBFile ", options, true);
        System.exit(-1);
      }
      ...
      conf.set("fs.defaultFS",
        conf.get(org.javenstudio.raptor.bigdb.DBConstants.BIGDB_DIR));
      ...
      if (cmd.hasOption("r")) {
        ...
        byte[][] hri = DBRegionInfo.parseRegionName(rn);
        Path rootDir = FSUtils.getRootDir(conf);
        Path tableDir = new Path(rootDir, Bytes.toString(hri[0]));
        String enc = DBRegionInfo.encodeRegionName(rn);
      ...
      for (Path file : files) {
        ...
        DBFile.Reader reader = new DBFile.Reader(fs, file, null, false);
        Map<byte[],byte[]> fileInfo = reader.loadFileInfo();
        // scan over file and read key/value's and check if requested
        DBFileScanner scanner = reader.getScanner(false, false);
        scanner.seekTo();
        KeyValue pkv = null;
        int count = 0;
        do {
