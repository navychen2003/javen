...
import org.javenstudio.raptor.bigdb.client.RetriesExhaustedException;
 ...
 *
 * This is a local bigdb copy of the hadoop RPC so we can do things like
 * address HADOOP-414 for bigdb-only and try other bigdb-specific
...
public class DBRPC {
  ...
  protected static final Logger LOG = Logger.getLogger(DBRPC.class);
  ...
  private static class Invocation implements Writable, Configurable {
    ...
    public void readFields(DataInput in) throws IOException {
      ...
      DBObjectWritable objectWritable = new DBObjectWritable();
      for (int i = 0; i < parameters.length; i++) {
        parameters[i] = DBObjectWritable.readObject(in, objectWritable,
          this.conf);
    ...
    public void write(DataOutput out) throws IOException {
      ...
      for (int i = 0; i < parameterClasses.length; i++) {
        DBObjectWritable.writeObject(out, parameters[i], parameterClasses[i],
                                   conf);
      }
    }
  ...
  static private class ClientCache {
    private Map<SocketFactory, DBClient> clients =
      new HashMap<SocketFactory, DBClient>();
     ...
     */
    protected synchronized DBClient getClient(Configuration conf,
      ...
      // per-job, we choose (a).
      DBClient client = clients.get(factory);
      if (client == null) {
        // Make an bigdb client instead of hadoop Client.
        client = new DBClient(DBObjectWritable.class, conf, factory);
     ...
     */
    protected void stopClient(DBClient client) {
      synchronized (this) {
        client.decCount();
  ...
  private static class Invoker implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args)
      ...
      DBObjectWritable value = (DBObjectWritable)
  ...
  public static class VersionMismatch extends IOException {
     ...
     *          (eg. org.javenstudio.raptor.mapred.InterTrackerProtocol)
     */
  ...
  public static VersionedProtocol waitForProxy(Class protocol,
    ...
    // BigDB does limited number of reconnects which is different from hadoop.
  ...
  public static Object[] call(Method method, Object[][] params,
    ...
    DBClient client = CLIENTS.getClient(conf);
    try {
    ...
    for (int i = 0; i < values.length; i++)
      if (wrappedValues[i] != null)
        values[i] = ((DBObjectWritable)wrappedValues[i]).get();
  ...
  /** An RPC Server. */
  public static class Server extends DBServer {
    ...
    public Writable call(Writable param, long receivedTime) throws IOException {
      try {
        ...
        return new DBObjectWritable(method.getReturnType(), value);
