...
public class Store implements HeapSize {
  ...
  private final DBColumnDescriptor family;
  final FileSystem fs;
  ...
  throws IOException {
    DBRegionInfo info = region.regionInfo;
    this.fs = fs;
    ...
    if (maxFileSize == DBConstants.DEFAULT_MAX_FILE_SIZE) {
      maxFileSize = conf.getLong("bigdb.hregion.max.filesize",
    ...
    this.maxFilesToCompact = conf.getInt("bigdb.hstore.compaction.max", 10);
  ...
  public void bulkLoadDBFile(String srcPathStr) throws IOException {
    ...
    try {
      ...
      reader = new DBFile.Reader(srcPath.getFileSystem(conf),
          srcPath, null, false);
      reader.loadFileInfo();
      ...
      DBRegionInfo hri = region.getRegionInfo();
  ...
  private boolean isMajorCompaction(final List<StoreFile> filesToCompact) throws IOException {
    ...
    if (lowTimestamp > 0l && lowTimestamp < (now - this.majorCompactionTime)) {
      ...
      if (filesToCompact.size() == 1 &&
          ...
          (this.ttl == DBConstants.FOREVER || elapsedTime < this.ttl)) {
        if (LOG.isDebugEnabled()) {
  ...
  DBRegion getDBRegion() {
    return this.region;
  }

  DBRegionInfo getDBRegionInfo() {
