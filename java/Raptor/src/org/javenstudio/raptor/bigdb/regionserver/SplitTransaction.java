...
import org.javenstudio.raptor.bigdb.io.Reference.Range;
...
import org.javenstudio.raptor.bigdb.util.PairOfSameType;
...
class SplitTransaction {
   ...
   * Region to split
   */
  private final DBRegion parent;
  private DBRegionInfo hri_a;
  private DBRegionInfo hri_b;
  ...
  public boolean prepare() {
    ...
    DBRegionInfo hri = this.parent.getRegionInfo();
    // Check splitrow.
    ...
    this.hri_a = new DBRegionInfo(hri.getTableDesc(), startKey, this.splitrow,
      false, rid);
    this.hri_b = new DBRegionInfo(hri.getTableDesc(), this.splitrow, endKey,
      false, rid);
    return true;
  }
   ...
   * @see #rollback(OnlineRegions)
   */
  PairOfSameType<DBRegion> execute(final OnlineRegions or, final boolean updateMeta)
  throws IOException {
    ...
    DBRegion a = createDaughterRegion(this.hri_a);

    // Ditto
    this.journal.add(JournalEntry.STARTED_REGION_B_CREATION);
    DBRegion b = createDaughterRegion(this.hri_b);
   ...
   */
  DBRegion createDaughterRegion(final DBRegionInfo hri)
  throws IOException {
    // Package private so unit tests have access.
    FileSystem fs = this.parent.getFilesystem();
    Path regionDir = getSplitDirForDaughter(this.parent.getFilesystem(),
      this.splitdir, hri);
    DBRegion r = DBRegion.newDBRegion(this.parent.getTableDir(),
  ...
  private static Path getSplitDirForDaughter(final FileSystem fs,
      final Path splitdir, final DBRegionInfo hri)
  throws IOException {
   ...
   * @throws IOException
   */
  private DBTable getTable(final Configuration conf) throws IOException {
  ...
  private Put createOfflineParentPut() throws IOException  {
    DBRegionInfo editedParentRegionInfo =
      new DBRegionInfo(this.parent.getRegionInfo());
  ...
  DBRegionInfo getFirstDaughter() {
    return hri_a;
  }

  DBRegionInfo getSecondDaughter() {
    return hri_b;
  }
