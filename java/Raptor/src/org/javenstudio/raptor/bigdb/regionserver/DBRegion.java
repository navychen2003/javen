...
import org.javenstudio.raptor.bigdb.DroppedSnapshotException;
...
import org.javenstudio.raptor.bigdb.client.RowLock;
import org.javenstudio.raptor.bigdb.client.Scan;
import org.javenstudio.raptor.bigdb.filter.Filter;
import org.javenstudio.raptor.bigdb.filter.IncompatibleFilterException;
...
import org.javenstudio.raptor.bigdb.util.EnvironmentEdgeManager;
 ...
 *
 * <p>We maintain multiple HStores for a single DBRegion.
 *
 ...
 */
public class DBRegion implements HeapSize { // , Writable{
  public static final Logger LOG = Logger.getLogger(DBRegion.class);
   ...
   * {@link DBRegion#newDBRegion(Path, DBLog, FileSystem, Configuration, org.javenstudio.raptor.bigdb.DBRegionInfo, FlushRequester)} method.
   *
   *
   ...
   * failed.  Can be null.
   *
   * @see DBRegion#newDBRegion(Path, DBLog, FileSystem, Configuration, org.javenstudio.raptor.bigdb.DBRegionInfo, FlushRequester)
  ...
  public DBRegion(Path tableDir, DBLog log, FileSystem fs, Configuration conf,
    ...
    this.threadWakeFrequency = conf.getLong(DBConstants.THREAD_WAKE_FREQUENCY,
        10 * 1000);
    ...
    if (flushSize == DBTableDescriptor.DEFAULT_MEMSTORE_FLUSH_SIZE) {
      flushSize = conf.getLong("bigdb.hregion.memstore.flush.size",
                      DBTableDescriptor.DEFAULT_MEMSTORE_FLUSH_SIZE);
    }
    this.memstoreFlushSize = flushSize;
    this.blockingMemStoreSize = this.memstoreFlushSize *
      conf.getLong("bigdb.hregion.memstore.block.multiplier", 2);
  }
  ...
  throws IOException {
    ...
    long maxSeqId = -1;
    for (DBColumnDescriptor c : this.regionInfo.getTableDesc().getFamilies()) {
  ...
  /**
   * Close down this DBRegion.  Flush the cache, shut down each HStore, don't
  ...
  private boolean worthPreFlushing() {
    return this.memstoreSize.get() >
      this.conf.getLong("bigdb.hregion.preclose.flush.size", 1024 * 1024 * 5);
  }
...
//  }
  ...
  throws UnsupportedEncodingException, IOException {
    long seqid = minSeqId;
    NavigableSet<Path> files = DBLog.getSplitEditFilesSorted(this.fs, regiondir);
    ...
    for (Path edits: files) {
      ...
      try {
        ...
        if (skipErrors) {
          Path p = DBLog.moveAsideBadEditsFile(fs, edits);
  ...
  private long replayRecoveredEdits(final Path edits,
    ...
    DBLog.Reader reader = DBLog.getReader(this.fs, edits, conf);
    try {
  ...
  */
  ...
  throws IOException {
    ...
    int interval = this.conf.getInt("bigdb.hstore.report.interval.edits", 2000);
    while ((entry = reader.next()) != null) {
      ...
      for (KeyValue kv: val.getKeyValues()) {
        ...
        if (kv.matchingFamily(DBLog.METAFAMILY) ||
  ...
  protected Store instantiateHStore(Path tableDir, DBColumnDescriptor c)
  throws IOException {
   ...
   */
  public static DBRegion newDBRegion(Path tableDir, DBLog log, FileSystem fs, Configuration conf,
                                   DBRegionInfo regionInfo, FlushRequester flushListener) {
    try {
      @SuppressWarnings("unchecked")
  ...
  public static DBRegion openDBRegion(final DBRegionInfo info, final Path rootDir,
    ...
    DBRegion r = DBRegion.newDBRegion(
        DBTableDescriptor.getTableDir(rootDir, info.getTableDesc().getName()),
        log, FSUtils.getFs(conf), conf, info, null);
  ...
  private static void printUsageAndExit(final String message) {
    ...
    System.out.println("Usage: DBRegion CATLALOG_TABLE_DIR [major_compact]");
  ...
  throws IOException {
    ...
    if (p.getName().startsWith(rootStr)) {
      ...
      region = DBRegion.newDBRegion(p, log, fs, c, DBRegionInfo.FIRST_META_REGIONINFO,
          null);
    } else {
   ...
   * <pre>
   *   ./bin/bigdb org.javenstudio.raptor.bigdb.regionserver.DBRegion
   * </pre>
   * @param args
   * @throws IOException
   */
  public static void main(String[] args) throws IOException {
    ...
    final Configuration c = ConfigurationFactory.get();
    final FileSystem fs = FSUtils.getFs(c);
    final Path logdir = new Path(c.get("bigdb.tmp.dir"),
        "dblog" + tableDir.getName()
        + EnvironmentEdgeManager.currentTimeMillis());
    final Path oldLogDir = new Path(c.get("bigdb.tmp.dir"),
        DBConstants.DBREGION_OLDLOGDIR_NAME);
    final DBLog log = new DBLog(fs, logdir, oldLogDir, c, null);
    try {
