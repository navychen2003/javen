...
class MemStoreFlusher extends Thread implements FlushRequester {
  static final Logger LOG = Logger.getLogger(MemStoreFlusher.class);
  // These two data members go together.  Any entry in the one must have
  // a corresponding entry in the other.
  private final BlockingQueue<FlushQueueEntry> flushQueue =
    new DelayQueue<FlushQueueEntry>();
  private final Map<DBRegion, FlushQueueEntry> regionsInQueue =
    new HashMap<DBRegion, FlushQueueEntry>();
  ...
  private static final String UPPER_KEY =
    "bigdb.regionserver.global.memstore.upperLimit";
  private static final String LOWER_KEY =
    "bigdb.regionserver.global.memstore.lowerLimit";
  ...
  public MemStoreFlusher(final Configuration conf,
      final DBRegionServer server) {
    super();
    this.server = server;
    this.threadWakeFrequency =
      conf.getLong(DBConstants.THREAD_WAKE_FREQUENCY, 10 * 1000);
    ...
    this.blockingStoreFilesNumber =
      conf.getInt("bigdb.hstore.blockingStoreFiles", -1);
    if (this.blockingStoreFilesNumber == -1) {
      this.blockingStoreFilesNumber = 1 +
        conf.getInt("bigdb.hstore.compactionThreshold", 3);
    }
    this.blockingWaitTime = conf.getInt("bigdb.hstore.blockingWaitTime",
      90000);
  ...
  public void request(DBRegion r) {
   ...
   * not flushed.
   */
  private boolean flushRegion(final DBRegion region, final boolean emergencyFlush) {
  ...
  private boolean isTooManyStoreFiles(DBRegion region) {
  ...
  private synchronized void flushSomeRegions() {
    ...
    ArrayList<DBRegion> regionsToCompact = new ArrayList<DBRegion>();
    for (SortedMap<Long, DBRegion> m =
      ...
      DBRegion biggestMemStoreRegion = m.remove(m.firstKey());
    ...
    for (DBRegion region : regionsToCompact) {
      server.compactSplitThread.compactionRequested(region, getName());
    }
  }
  ...
  static class FlushQueueEntry implements Delayed {
    ...
    FlushQueueEntry(final DBRegion r) {
      this.region = r;
