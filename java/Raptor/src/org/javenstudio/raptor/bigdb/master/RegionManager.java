...
public class RegionManager {
  protected static final Logger LOG = Logger.getLogger(RegionManager.class);

  private AtomicReference<DBServerAddress> rootRegionLocation =
    new AtomicReference<DBServerAddress>(null);
  ...
  final DBMaster master;
  private final LoadBalancer loadBalancer;

  /** Set of regions to split. */
  private final SortedMap<byte[], Pair<DBRegionInfo,DBServerAddress>>
    regionsToSplit = Collections.synchronizedSortedMap(
        new TreeMap<byte[],Pair<DBRegionInfo,DBServerAddress>>
  ...
  private final SortedMap<byte[], Pair<DBRegionInfo,DBServerAddress>>
    regionsToFlush = Collections.synchronizedSortedMap(
        new TreeMap<byte[],Pair<DBRegionInfo,DBServerAddress>>
        (Bytes.BYTES_COMPARATOR));
  private final int paxosNumRetries;
  private final int paxosPause;

  RegionManager(DBMaster master) {
    Configuration conf = master.getConfiguration();

    this.master = master;
    this.maxAssignInOneGo = conf.getInt("bigdb.regions.percheckin", 10);
    ...
    paxosNumRetries = conf.getInt(DBConstants.PAXOS_RETRIES,
        DBConstants.DEFAULT_PAXOS_RETRIES);
    paxosPause = conf.getInt(DBConstants.PAXOS_PAUSE,
        DBConstants.DEFAULT_PAXOS_PAUSE);
  ...
  void unsetRootRegion() {
    synchronized (regionsInTransition) {
      ...
      regionsInTransition.remove(
          DBRegionInfo.ROOT_REGIONINFO.getRegionNameAsString());
  ...
  void reassignRootRegion() {
    ...
    if (!master.getShutdownRequested().get()) {
      synchronized (regionsInTransition) {
        RegionState s = new RegionState(DBRegionInfo.ROOT_REGIONINFO,
            RegionState.State.UNASSIGNED);
        regionsInTransition.put(
            DBRegionInfo.ROOT_REGIONINFO.getRegionNameAsString(), s);
   ...
   * @param returnMsgs
   */
  void assignRegions(DBServerInfo info, DBRegionInfo[] mostLoadedRegions,
      ArrayList<DBMsg> returnMsgs) {
    DBServerLoad thisServersLoad = info.getLoad();
   ...
   * @param returnMsgs
   */
  private void assignRegionsToMultipleServers(final DBServerLoad thisServersLoad,
    final Set<RegionState> regionsToAssign, final DBServerInfo info,
    final ArrayList<DBMsg> returnMsgs) {
  ...
  private void assignRegions(final Set<RegionState> regionsToAssign,
      final int nregions, final DBServerInfo info,
      final ArrayList<DBMsg> returnMsgs) {
    int count = nregions;
  ...
  private void assignRegionsToOneServer(final Set<RegionState> regionsToAssign,
      final DBServerInfo info, final ArrayList<DBMsg> returnMsgs) {
    for (RegionState s: regionsToAssign) {
      doRegionAssignment(s, info, returnMsgs);
    }
  }
  ...
  private void doRegionAssignment(final RegionState rs,
      final DBServerInfo sinfo, final ArrayList<DBMsg> returnMsgs) {
    ...
    returnMsgs.add(new DBMsg(DBMsg.Type.MSG_REGION_OPEN, rs.getRegionInfo()));
  }
  ...
  private int regionsToGiveOtherServers(final int numUnassignedRegions,
    final DBServerLoad thisServersLoad) {
    SortedMap<DBServerLoad, Set<String>> lightServers =
      new TreeMap<DBServerLoad, Set<String>>();
    ...
    int nRegions = 0;
    for (Map.Entry<DBServerLoad, Set<String>> e: lightServers.entrySet()) {
      DBServerLoad lightLoad = new DBServerLoad(e.getKey());
      do {
   ...
   * the monitor for RegionManager
   */
  private Set<RegionState> regionsAwaitingAssignment(DBServerAddress addr,
    ...
    synchronized (this.regionsInTransition) {
      rootState = regionsInTransition.get(DBRegionInfo.ROOT_REGIONINFO.getRegionNameAsString());
    }
    ...
    synchronized (this.regionsInTransition) {
      for (RegionState s: regionsInTransition.values()) {
        DBRegionInfo i = s.getRegionInfo();
        if (i == null) {
          continue;
        }
        if (reassigningMetas &&
   ...
   */
  private int computeNextHeaviestLoad(DBServerLoad referenceLoad,
    DBServerLoad heavierLoad) {

    SortedMap<DBServerLoad, Set<String>> heavyServers =
      new TreeMap<DBServerLoad, Set<String>>();
    ...
    int nservers = 0;
    for (Map.Entry<DBServerLoad, Set<String>> e : heavyServers.entrySet()) {
   ...
   */
  void unassignSomeRegions(final DBServerInfo info,
      int numRegionsToClose, final DBRegionInfo[] mostLoadedRegions,
      ArrayList<DBMsg> returnMsgs) {
    ...
    while (regionsClosed < numRegionsToClose &&
      ...
      DBRegionInfo currentRegion = mostLoadedRegions[regionIdx];
      regionIdx++;
      ...
      returnMsgs.add(new DBMsg(DBMsg.Type.MSG_REGION_CLOSE, currentRegion,
        OVERLOADED));
  ...
  /*
   * PathFilter that accepts bigdb tables only.
   */
  static class TableDirFilter implements PathFilter {
    public boolean accept(final Path path) {
      ...
      return (!pathname.equals(DBConstants.DBREGION_LOGDIR_NAME)
              && !pathname.equals(DBConstants.VERSION_FILE_NAME));
    }
  ...
  static class RegionDirFilter implements PathFilter {
    public boolean accept(Path path) {
      return !path.getName().equals(DBConstants.DBREGION_COMPACTIONDIR_NAME);
    }
  }
  ...
  public void stop() {
    ...
    master.getPaxosWrapper().clearRSDirectory();
    master.getPaxosWrapper().close();
  }
   ...
   */
  public MetaRegion getFirstMetaRegionForRegion(DBRegionInfo newRegion) {
    synchronized (onlineMetaRegions) {
    ...
    if (Bytes.equals(tableName, DBConstants.META_TABLE_NAME)) {
      if (rootRegionLocation.get() == null) {
        throw new NotAllMetaRegionsOnlineException(
            Bytes.toString(DBConstants.ROOT_TABLE_NAME));
      }
  ...
  /**
   * Create a new DBRegion, put a row for it into META (or ROOT), and mark the
   * new region unassigned so that it will get assigned to a region server.
   * @param newRegion DBRegionInfo for the region to create
   ...
   * @throws IOException
   */
  public void createRegion(DBRegionInfo newRegion, DBRegionInterface server,
      byte [] metaRegionName)
  throws IOException {
    // 2. Create the DBRegion
    DBRegion region = DBRegion.createDBRegion(newRegion, this.master.getRootDir(),
      master.getConfiguration());

    // 3. Insert into meta
    DBRegionInfo info = region.getRegionInfo();
  ...
  public boolean isRootServer(DBServerAddress server) {
   ...
   */
  public List<byte[]> listMetaRegionsForServer(DBServerAddress server) {
   ...
   */
  public boolean isMetaServer(DBServerAddress server) {
    for ( MetaRegion region : onlineMetaRegions.values() ) {
      if (server.equals(region.getServer())) {
        return true;
      }
   ...
   *
   * @param server Server
   * @return if this server was transitioning a META table then a not null DBRegionInfo pointing to it
   */
  public DBRegionInfo getMetaServerRegionInfo(final String server) {
    synchronized (this.regionsInTransition) {
      for (RegionState s : regionsInTransition.values()) {
        if (s.getRegionInfo().isMetaRegion()
            && !s.isUnassigned()
   ...
   */
  public synchronized boolean offlineMetaServer(DBServerAddress server) {
   ...
   * @param info
   */
  public void removeRegion(DBRegionInfo info) {
    synchronized (this.regionsInTransition) {
   ...
   */
  public void setUnassigned(DBRegionInfo info, boolean force) {
    RegionState s = null;
    synchronized(this.regionsInTransition) {
   ...
   * @param info DBRegionInfo to check for
   ...
   */
  public boolean isUnassigned(DBRegionInfo info) {
    synchronized (regionsInTransition) {
   ...
   * @param setOffline
   */
  public void setClosing(String serverName, final DBRegionInfo regionInfo,
   ...
   */
  public Set<DBRegionInfo> getMarkedToClose(String serverName) {
    Set<DBRegionInfo> result = new HashSet<DBRegionInfo>();
    synchronized (regionsInTransition) {
      for (RegionState s: regionsInTransition.values()) {
   ...
   * @return DBServerAddress describing root region server.
   */
  public DBServerAddress getRootRegionLocation() {
  ...
  public void waitForRootRegionLocation() {
    synchronized (rootRegionLocation) {
      while (!master.getShutdownRequested().get() &&
        ...
        // regionServerReport message from the DBRegionServer that has been
  ...
  private long getPauseTime(int tries) {
    int attempt = tries;
    if (attempt >= DBConstants.RETRY_BACKOFF.length) {
      attempt = DBConstants.RETRY_BACKOFF.length - 1;
    }
    return this.paxosPause * DBConstants.RETRY_BACKOFF[attempt];
  }
  ...
  private void writeRootRegionLocationToPaxos(DBServerAddress address) {
    for (int attempt = 0; attempt < paxosNumRetries; ++attempt) {
      if (master.getPaxosWrapper().writeRootRegionLocation(address)) {
        return;
      }
    ...
    LOG.error("Failed to write root region location to Paxos after " +
              paxosNumRetries + " retries, shutting down");
   ...
   */
  public void setRootRegionLocation(DBServerAddress address) {
    writeRootRegionLocationToPaxos(address);
    synchronized (rootRegionLocation) {
      rootRegionLocation.set(new DBServerAddress(address));
   ...
   * @param op
   */
  public void startAction(byte[] regionName, DBRegionInfo info,
      DBServerAddress server, DBConstants.Modify op) {
    if (LOG.isDebugEnabled()) {
  ...
  private void startAction(final byte[] regionName, final DBRegionInfo info,
      final DBServerAddress server,
      final SortedMap<byte[], Pair<DBRegionInfo,DBServerAddress>> map) {
    map.put(regionName, new Pair<DBRegionInfo,DBServerAddress>(info, server));
  }
   ...
   * @param op
   */
  public void endAction(byte[] regionName, DBConstants.Modify op) {
    if (LOG.isDebugEnabled()) {
   ...
   * @param returnMsgs
   */
  public void applyActions(DBServerInfo serverInfo, ArrayList<DBMsg> returnMsgs) {
    applyActions(serverInfo, returnMsgs, this.regionsToCompact,
        DBMsg.Type.MSG_REGION_COMPACT);
    applyActions(serverInfo, returnMsgs, this.regionsToSplit,
      DBMsg.Type.MSG_REGION_SPLIT);
    applyActions(serverInfo, returnMsgs, this.regionsToFlush,
        DBMsg.Type.MSG_REGION_FLUSH);
    applyActions(serverInfo, returnMsgs, this.regionsToMajorCompact,
        DBMsg.Type.MSG_REGION_MAJOR_COMPACT);
  }

  private void applyActions(final DBServerInfo serverInfo,
      final ArrayList<DBMsg> returnMsgs,
      final SortedMap<byte[], Pair<DBRegionInfo,DBServerAddress>> map,
      final DBMsg.Type msg) {
    DBServerAddress addr = serverInfo.getServerAddress();
    synchronized (map) {
      Iterator<Pair<DBRegionInfo, DBServerAddress>> i = map.values().iterator();
      while (i.hasNext()) {
        Pair<DBRegionInfo,DBServerAddress> pair = i.next();
        if (addr.equals(pair.getSecond())) {
          if (LOG.isDebugEnabled()) {
            LOG.debug("Sending " + msg + " " + pair.getFirst() + " to " + addr);
          }
          returnMsgs.add(new DBMsg(msg, pair.getFirst()));
          i.remove();
        }
      }
    }
  ...
  private class LoadBalancer {
    private float slop;                 // bigdb.regions.slop
    private final int maxRegToClose;    // bigdb.regions.close.max

    LoadBalancer(Configuration conf) {
      this.slop = conf.getFloat("bigdb.regions.slop", (float)0.3);
      ...
      this.maxRegToClose = conf.getInt("bigdb.regions.close.max", -1);
    }
     ...
     */
    void loadBalancing(DBServerInfo info, DBRegionInfo[] mostLoadedRegions,
        ArrayList<DBMsg> returnMsgs) {
      DBServerLoad servLoad = info.getLoad();
    ...
    private int balanceFromOverloaded(final String serverName,
        DBServerLoad srvLoad, double avgLoad) {
     ...
     * @return number of regions to unassign.
     */
    private int balanceToLowloaded(String srvName, DBServerLoad srvLoad,
        double avgLoad) {

      SortedMap<DBServerLoad, Set<String>> loadToServers =
  ...
  static class RegionState implements Comparable<RegionState> {
    private final DBRegionInfo regionInfo;
    ...
    /* Constructor */
    RegionState(DBRegionInfo info, State state) {
      this.regionInfo = info;
      this.state = state;
    }

    synchronized DBRegionInfo getRegionInfo() {
