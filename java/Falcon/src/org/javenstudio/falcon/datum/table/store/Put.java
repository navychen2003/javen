...
public class Put implements HeapSize, Writable, Row, Comparable<Row> {
  ...
  private long mTimestamp = DBConstants.LATEST_TIMESTAMP;
  private long mLockId = -1L;
  private boolean mWriteToWAL = true;

  private Map<byte[], List<KeyValue>> mFamilyMap =
  ...
  public Put(byte[] row, RowLock rowLock) {
      this(row, DBConstants.LATEST_TIMESTAMP, rowLock);
  }
  ...
  public Put(byte[] row, long ts, RowLock rowLock) {
    if(row == null || row.length > DBConstants.MAX_ROW_LENGTH) {
      throw new IllegalArgumentException("Row key is invalid");
    }
    this.mRow = Arrays.copyOf(row, row.length);
    this.mTimestamp = ts;
    if (rowLock != null) {
  ...
  public Put(Put putToCopy) {
    this(putToCopy.getRow(), putToCopy.mTimestamp, putToCopy.getRowLock());
    this.mFamilyMap =
      new TreeMap<byte[], List<KeyValue>>(Bytes.BYTES_COMPARATOR);
    for (Map.Entry<byte[], List<KeyValue>> entry :
      putToCopy.getFamilyMap().entrySet()) {
      this.mFamilyMap.put(entry.getKey(), entry.getValue());
    }
    this.mWriteToWAL = putToCopy.mWriteToWAL;
  }
  ...
  public Put add(byte[] family, byte[] qualifier, byte[] value) {
    return add(family, qualifier, this.mTimestamp, value);
  }
  ...
  public Put add(byte[] family, byte[] qualifier, long ts, byte[] value) {
    ...
    list.add(kv);
    mFamilyMap.put(kv.getFamily(), list);
    return this;
  }
  ...
  public Put add(KeyValue kv) throws IOException{
    ...
    int res = Bytes.compareTo(this.mRow, 0, mRow.length,
    ...
    if (res != 0) {
      throw new IOException("The row in the recently added KeyValue " +
        ...
        Bytes.toStringBinary(this.mRow));
    }
    list.add(kv);
  ...
  private KeyValue createPutKeyValue(byte[] family, byte[] qualifier, long ts, byte[] value) {
    return new KeyValue(this.mRow, family, qualifier, ts, KeyValue.Type.Put, value);
  }
  ...
  public boolean has(byte[] family, byte[] qualifier) {
    return has(family, qualifier, this.mTimestamp, new byte[0], true, true);
  }
  ...
  public boolean has(byte[] family, byte[] qualifier, byte[] value) {
    return has(family, qualifier, this.mTimestamp, value, true, false);
  }
  ...
  public int size() {
    int size = 0;
    for (List<KeyValue> kvList : this.mFamilyMap.values()) {
      size += kvList.size();
    }
    return size;
  }
  ...
  public long heapSize() {
    ...
    //Adding row
    heapsize += ClassSize.align(ClassSize.ARRAY + this.mRow.length);

    //Adding map overhead
    heapsize +=
      ClassSize.align(this.mFamilyMap.size() * ClassSize.MAP_ENTRY);
    for (Map.Entry<byte[], List<KeyValue>> entry : this.mFamilyMap.entrySet()) {
      //Adding key overhead
      heapsize +=
  ...
  public void readFields(final DataInput in)
    ...
    this.mWriteToWAL = in.readBoolean();
    
    int numFamilies = in.readInt();
    if (!this.mFamilyMap.isEmpty()) this.mFamilyMap.clear();
    ...
    for (int i=0; i<numFamilies; i++) {
      ...
      this.mFamilyMap.put(family, keys);
    }
  }
  ...
  public void write(final DataOutput out)
    ...
    out.writeBoolean(this.mWriteToWAL);
