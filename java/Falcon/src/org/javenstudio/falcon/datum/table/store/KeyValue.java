 ...
 * just the Key portion of a KeyValue used mostly in {@link DBFile}.
 *
...
public class KeyValue implements Writable, HeapSize {
  ...
  public static KeyComparator getRowComparator(byte[] tableName) {
    if (Bytes.equals(DBTableDescriptor.ROOT_TABLEDESC.getName(),tableName)) {
      return ROOT_COMPARATOR.getRawComparator();
    }
    if (Bytes.equals(DBTableDescriptor.META_TABLEDESC.getName(), tableName)) {
  ...
  public static final KeyValue LOWESTKEY =
    new KeyValue(DBConstants.EMPTY_BYTE_ARRAY, DBConstants.LATEST_TIMESTAMP);
  ...
  public void setMemstoreTS(long memstoreTS) {
    this.mMemstoreTS = memstoreTS;
  }

  // default value is 0, aka DNC
  private long mMemstoreTS = 0;
  ...
  public KeyValue(final byte[] row, final byte[] family,
    ...
    this(row, family, qualifier, DBConstants.LATEST_TIMESTAMP, Type.Maximum);
  }
  ...
  public KeyValue(final byte[] row, final byte[] family,
    ...
    this(row, family, qualifier, DBConstants.LATEST_TIMESTAMP, Type.Put, value);
  }
  ...
  static byte[] createByteArray(final byte[] row, final int roffset,
    ...
    if (vlength > DBConstants.MAXIMUM_VALUE_LENGTH) { // FindBugs INT_VACUOUS_COMPARISON
      throw new IllegalArgumentException("Valuer > " +
          DBConstants.MAXIMUM_VALUE_LENGTH);
    }
   ...
   * @return The newly created byte array.
   */
  static byte[] createByteArray(final byte[] row, 
  ...
  public boolean equals(Object other) {
    ...
    boolean result = Bytes.BYTES_RAWCOMPARATOR.compare(
      getBuffer(), getKeyOffset(), getKeyLength(),
  ...
  public KeyValue clone() {
    byte[] b = new byte[this.mLength];
    System.arraycopy(this.mBytes, this.mOffset, b, 0, this.mLength);
  ...
  public String toString() {
    ...
    return keyToString(this.mBytes, this.mOffset + ROW_OFFSET, getKeyLength()) +
      "/vlen=" + getValueLength();
  }
...
//      qualifier + "/" + timestamp + "/" + Type.codeToType(type);
  ...
  public int getKeyOffset() {
    return this.mOffset + ROW_OFFSET;
  }
  ...
  public int getKeyLength() {
    if (mKeyLength == 0) {
      mKeyLength = Bytes.toInt(this.mBytes, this.mOffset);
    }
    return mKeyLength;
  }
  ...
  public int getValueLength() {
    return Bytes.toInt(this.mBytes, this.mOffset + Bytes.SIZEOF_INT);
  }
  ...
  public short getRowLength() {
    return Bytes.toShort(this.mBytes, getKeyOffset());
  }
  ...
  public int getFamilyOffset(int rlength) {
    return this.mOffset + ROW_OFFSET + Bytes.SIZEOF_SHORT + rlength + Bytes.SIZEOF_BYTE;
  }
  ...
  public boolean isLatestTimestamp() {
    return  Bytes.compareTo(getBuffer(), getTimestampOffset(), Bytes.SIZEOF_LONG,
      DBConstants.LATEST_TIMESTAMP_BYTES, 0, Bytes.SIZEOF_LONG) == 0;
  }

  /**
   * @param now Time to set into <code>this</code> IFF timestamp ==
   * {@link DBConstants#LATEST_TIMESTAMP} (else, its a noop).
   * @return True is we modified this.
   */
  public boolean updateLatestStamp(final byte[] now) {
    if (this.isLatestTimestamp()) {
      int tsOffset = getTimestampOffset();
      System.arraycopy(now, 0, this.mBytes, tsOffset, Bytes.SIZEOF_LONG);
      return true;
    }
    return false;
  }
  ...
  public byte[] getValue() {
    ...
    System.arraycopy(getBuffer(), offset, result, 0, len);
    return result;
  }
  ...
  public byte[] getRow() {
    if (mRowCache == null) {
      ...
      mRowCache = new byte[l];
      System.arraycopy(getBuffer(), o, mRowCache, 0, l);
    }
    return mRowCache;
  }
   ...
   * @return Timestamp
   */
  private long mTimestampCache = -1;
  
  public long getTimestamp() {
    if (mTimestampCache == -1) {
      mTimestampCache = getTimestamp(getKeyLength());
    }
    return mTimestampCache;
  }
   ...
   * @return Timestamp
   */
  protected long getTimestamp(final int keylength) {
    int tsOffset = getTimestampOffset(keylength);
    return Bytes.toLong(this.mBytes, tsOffset);
  }
   ...
   * @return Type of this KeyValue.
   */
  protected byte getType(final int keylength) {
    return this.mBytes[this.mOffset + keylength - 1 + ROW_OFFSET];
  }
  ...
  public byte[] getFamily() {
    int offset = getFamilyOffset();
    int len = getFamilyLength(offset);
    byte[] result = new byte[len];
    System.arraycopy(this.mBytes, offset, result, 0, len);
    return result;
  }
  ...
  public byte[] getQualifier() {
    int offset = getQualifierOffset();
    int len = getQualifierLength();
    byte[] result = new byte[len];
    System.arraycopy(this.mBytes, offset, result, 0, len);
    return result;
  }
  ...
  public static class SplitKeyValue {
    private byte[][] mSplit;
    SplitKeyValue() {
      this.mSplit = new byte[6][];
    }
    public void setRow(byte[] value) { this.mSplit[0] = value; }
    public void setFamily(byte[] value) { this.mSplit[1] = value; }
    public void setQualifier(byte[] value) { this.mSplit[2] = value; }
    public void setTimestamp(byte[] value) { this.mSplit[3] = value; }
    public void setType(byte[] value) { this.mSplit[4] = value; }
    public void setValue(byte[] value) { this.mSplit[5] = value; }
    public byte[] getRow() { return this.mSplit[0]; }
    public byte[] getFamily() { return this.mSplit[1]; }
    public byte[] getQualifier() { return this.mSplit[2]; }
    public byte[] getTimestamp() { return this.mSplit[3]; }
    public byte[] getType() { return this.mSplit[4]; }
    public byte[] getValue() { return this.mSplit[5]; }
  }

  public SplitKeyValue split() {
    SplitKeyValue split = new SplitKeyValue();
    int splitOffset = this.mOffset;
    int keyLen = Bytes.toInt(mBytes, splitOffset);
    splitOffset += Bytes.SIZEOF_INT;
    int valLen = Bytes.toInt(mBytes, splitOffset);
    splitOffset += Bytes.SIZEOF_INT;
    short rowLen = Bytes.toShort(mBytes, splitOffset);
    splitOffset += Bytes.SIZEOF_SHORT;
    byte[] row = new byte[rowLen];
    System.arraycopy(mBytes, splitOffset, row, 0, rowLen);
    splitOffset += rowLen;
    split.setRow(row);
    byte famLen = mBytes[splitOffset];
    splitOffset += Bytes.SIZEOF_BYTE;
    byte[] family = new byte[famLen];
    System.arraycopy(mBytes, splitOffset, family, 0, famLen);
    splitOffset += famLen;
    ...
    System.arraycopy(mBytes, splitOffset, qualifier, 0, colLen);
    splitOffset += colLen;
    split.setQualifier(qualifier);
    byte[] timestamp = new byte[Bytes.SIZEOF_LONG];
    System.arraycopy(mBytes, splitOffset, timestamp, 0, Bytes.SIZEOF_LONG);
    splitOffset += Bytes.SIZEOF_LONG;
    split.setTimestamp(timestamp);
    byte[] type = new byte[1];
    type[0] = mBytes[splitOffset];
    splitOffset += Bytes.SIZEOF_BYTE;
    split.setType(type);
    byte[] value = new byte[valLen];
    System.arraycopy(mBytes, splitOffset, value, 0, valLen);
  ...
  public boolean matchingFamily(final byte[] family, int offset, int length) {
    if (this.mLength == 0 || this.mBytes.length == 0) 
      return false;
    
    return Bytes.compareTo(family, offset, length,
        this.mBytes, getFamilyOffset(), getFamilyLength()) == 0;
  }
  ...
  public boolean matchingQualifier(final byte[] qualifier, int offset, int length) {
    return Bytes.compareTo(qualifier, offset, length,
        this.mBytes, getQualifierOffset(), getQualifierLength()) == 0;
  }
  ...
  public boolean matchingRow(final byte[] row, int offset, int length) {
    return Bytes.compareTo(row, offset, length,
        this.mBytes, getRowOffset(), getRowLength()) == 0;
  }
  ...
  public boolean matchingColumnNoDelimiter(final byte[] column) {
    ...
    return Bytes.compareTo(column, 0, column.length, this.mBytes, o, l) == 0;
  }
  ...
  public boolean matchingColumn(final byte[] family, final byte[] qualifier) {
    ...
    if (Bytes.compareTo(family, 0, family.length, this.mBytes, o, family.length) != 0) {
      return false;
    }
  ...
  public static class RootComparator extends MetaComparator {
    private final KeyComparator mRawcomparator = new RootKeyComparator();

    public KeyComparator getRawComparator() {
      return this.mRawcomparator;
    }
  ...
  public static class MetaComparator extends KVComparator {
    private final KeyComparator mRawcomparator = new MetaKeyComparator();
  ...
  public static class KVComparator implements java.util.Comparator<KeyValue> {
    private final KeyComparator mRawcomparator = new KeyComparator();

    /**
     * @return RawComparator that can compare the Key portion of a KeyValue.
     * Used in dbfile where indices are the Key portion of a KeyValue.
     */
    public KeyComparator getRawComparator() {
      return this.mRawcomparator;
    }
    ...
    protected int compareTimestamps(final KeyValue left, final int lkeylength,
    ...
    private int compareColumns(final KeyValue left, final short lrowlength,
        final KeyValue right, final short rrowlength) {
     ...
     * Was called matchesWithoutColumn in DBStoreKey.
    ...
    public KVComparator getComparatorIgnoringTimestamps() {
      ...
      try {
        ...
        c.getRawComparator().mIgnoreTimestamp = true;
    ...
    public KVComparator getComparatorIgnoringType() {
      ...
      try {
        c = (KVComparator)this.clone();
        c.getRawComparator().mIgnoreType = true;
      } catch (CloneNotSupportedException e) {
        LOG.error("Not supported", e);
      }
      return c;
  ...
  public static KeyValue createLastOnRow(final byte[] row) {
    return new KeyValue(row, null, null, DBConstants.LATEST_TIMESTAMP, Type.Minimum);
  }
  ...
  public static KeyValue createFirstOnRow(final byte[] row) {
    return createFirstOnRow(row, DBConstants.LATEST_TIMESTAMP);
  }
  ...
  public static KeyValue createFirstOnRow(final byte[] row, final byte[] family,
      final byte[] qualifier) {
    return new KeyValue(row, family, qualifier, DBConstants.LATEST_TIMESTAMP, Type.Maximum);
  }
  ...
  public static KeyValue createFirstOnRow(final byte[] row,
    ...
    return new KeyValue(row, roffset, rlength, family,
        ...
        DBConstants.LATEST_TIMESTAMP, Type.Maximum, null, 0, 0);
  }

  /**
   * @param b
   * @return A KeyValue made of a byte array that holds the key-only part.
   * Needed to convert dbfile index members to KeyValues.
   */
  ...
  public static class RootKeyComparator extends MetaKeyComparator {
    ...
    public int compareRows(byte[] left, int loffset, int llength,
      ...
      //  LOG.debug("ROOT " + Bytes.toString(left, loffset, llength) +
      ...
      int leftFarDelimiter = getDelimiterInReverse(left,
          ...
          llength - metalength, DBRegionInfo.DELIMITER);
  ...
  public static class RowComparator implements Comparator<KeyValue> {
    private final KVComparator mComparator;
    ...
    public int compare(KeyValue left, KeyValue right) {
      return mComparator.compareRows(left, right);
    }
  }
  ...
  public static class MetaKeyComparator extends KeyComparator {
    ...
    public int compareRows(byte[] left, int loffset, int llength,
        byte[] right, int roffset, int rlength) {
	  //if (LOG.isDebugEnabled()) {
      //  LOG.debug("compareRows: META " + Bytes.toString(left, loffset, llength) +
      //    "---" + Bytes.toString(right, roffset, rlength));
	  //}
      ...
      int leftFarDelimiter = getRequiredDelimiterInReverse(left, leftDelimiter,
          llength - (leftDelimiter - loffset), DBRegionInfo.DELIMITER);
  ...
  public static class KeyComparator implements RawComparator<byte[]> {
    private volatile boolean mIgnoreTimestamp = false;
    private volatile boolean mIgnoreType = false;
  ...
  public long heapSize() {
    return ClassSize.align(ClassSize.OBJECT + (2 * ClassSize.REFERENCE) +
        ClassSize.align(ClassSize.ARRAY + mLength) +
        (3 * Bytes.SIZEOF_INT) +
        ClassSize.align(ClassSize.ARRAY + (mRowCache == null ? 0 : mRowCache.length)) +
        (2 * Bytes.SIZEOF_LONG));
  }
