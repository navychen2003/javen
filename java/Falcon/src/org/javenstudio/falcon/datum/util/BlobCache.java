...
public class BlobCache implements Closeable {
    private static final Logger LOG = Logger.getLogger(BlobCache.class);
    ...
    public BlobCache(String path, int maxEntries, int maxBytes, 
    		boolean reset) throws IOException {
        this(path, maxEntries, maxBytes, reset, 0);
    }

    public BlobCache(String path, int maxEntries, int maxBytes, 
    		boolean reset, int version) throws IOException {
        ...
        try {
            ...
            if (mIndexFile.read(buf) != INDEX_HEADER_SIZE) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("cannot read header");
                return false;
            }

            if (readInt(buf, IH_MAGIC) != MAGIC_INDEX_FILE) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("cannot read header magic");
                return false;
            }
            ...
            if (checkSum(buf, 0, IH_CHECKSUM) != sum) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("header checksum does not match");
                return false;
            }
            ...
            if (mDataFile0.read(magic) != 4) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("cannot read data file magic");
                return false;
            }
            
            if (readInt(magic, 0) != MAGIC_DATA_FILE) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("invalid data file magic");
                return false;
            }
            ...
            if (LOG.isDebugEnabled()) { 
            	LOG.debug("loadIndex: maxEntries=" + mMaxEntries + " maxBytes=" + mMaxBytes 
            			+ " activeRegion=" + mActiveRegion + " activeEntries=" + mActiveEntries 
            			+ " activeBytes=" + mActiveBytes);
            }
            
            return true;
        } catch (IOException ex) {
        	if (LOG.isDebugEnabled())
            	LOG.debug("loadIndex failed.", ex);
    ...
    public void insert(long key, byte[] data) throws IOException {
        ...
        //if (LOG.isDebugEnabled())
        //	LOG.debug("insert: key=" + key + " bytesLength=" + data.length);
    }

    public boolean clearEntry(long key) throws IOException {
        if (!lookupInternal(key, mActiveHashStart)) 
    ...
    private void insertInternal(long key, byte[] data, int length)
        ...
        //if (LOG.isDebugEnabled())
        //	LOG.debug("insert: key=" + key + " bytesLength=" + length);
    }
    ...
    private boolean getBlob(RandomAccessFile file, int offset,
        ...
        try {
            ...
            if (file.read(header) != BLOB_HEADER_SIZE) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("cannot read blob header");
                return false;
            }
            ...
            if (blobKey != req.key) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("blob key does not match: " + blobKey);
                return false;
            }
            ...
            if (blobOffset != offset) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("blob offset does not match: " + blobOffset);
                return false;
            }
            ...
            if (length < 0 || length > mMaxBytes - offset - BLOB_HEADER_SIZE) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("invalid blob length: " + length);
                return false;
            }
            ...
            if (file.read(blob, 0, length) != length) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("cannot read blob data");
                return false;
            }
            
            if (checkSum(blob, 0, length) != sum) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("blob checksum does not match: " + sum);
                return false;
            }
    ...
    private boolean lookupInternal(long key, int hashStart) {
        ...
        while (true) {
            ...
            if (candidateOffset == 0) {
                ...
                if (slot == slotBegin) {
                	if (LOG.isDebugEnabled())
                    	LOG.debug("corrupted index: clear the slot.");
    ...
    public void syncAll() {
        ...
        try {
        	...
        	if (LOG.isDebugEnabled())
            	LOG.debug("sync data file 0 failed", t);
        }
        try {
            mDataFile1.getFD().sync();
        } catch (Throwable t) {
        	if (LOG.isDebugEnabled())
            	LOG.debug("sync data file 1 failed", t);
        }
    }
	...
	int getActiveCount() {
        ...
        if (count == mActiveEntries) {
        	...
        	if (LOG.isDebugEnabled())
            	LOG.debug("wrong active count: " + mActiveEntries + " vs " + count);
