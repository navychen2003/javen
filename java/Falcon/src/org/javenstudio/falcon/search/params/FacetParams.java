...
public interface FacetParams {
  ...
  /** 
   * Value for FACET_METHOD param to indicate that Lightning 
   * should enumerate over terms
  ...
  /** 
   * Value for FACET_METHOD param to indicate that Lightning 
   * should enumerate over documents
   * and count up terms by consulting an uninverted 
   * representation of the field values
   * (such as the FieldCache used for sorting).
   */
  public static final String FACET_METHOD_fc = "fc";

  /** 
   * Value for FACET_METHOD param, like FACET_METHOD_fc 
   * but counts per-segment.
   */
  public static final String FACET_METHOD_fcs = "fcs";

  /**
   * Any formated queries the user would like to use for
   ...
   * </p>
   * <p>
   * The default value if none are specified is: [lower,upper,edge] <i>
   * (NOTE: This is different then FACET_RANGE_INCLUDE)</i>
   * </p>
   * <p>
   * Can be overriden on a per field basis.
   * </p>
