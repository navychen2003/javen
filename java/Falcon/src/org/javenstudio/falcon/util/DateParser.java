...
public class DateParser  {
  ...
  private static Map<String,Integer> makeUnitsMap() {
    ...
    // from a param via RequestInfo as well.
   ...
   * otherwise specified in the RequestInfo
   * 
   * @see #DEFAULT_MATH_TZ
   ...
   * If null, then this method defaults to the value dicated by the RequestInfo if it 
   ...
   * @see RequestInfo#getClientTimeZone
   */
  public DateParser(TimeZone tz, Locale l) {
    loc = (null != l) ? l : DEFAULT_MATH_LOCALE;
    if (null == tz) {
      //RequestInfo reqInfo = RequestInfo.getRequestInfo();
   ...
   * first defines 'now' as the value dictated by the RequestInfo if it 
   * exists -- otherwise it uses a new Date instance at the moment getNow() 
   * is first called.
   * @see #setNow
   * @see RequestInfo#getNOW
   */
  public Date getNow() {
    if (now == null) {
      //RequestInfo reqInfo = RequestInfo.getRequestInfo();
