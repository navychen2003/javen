...
import org.javenstudio.raptor.fs.shell.CommandFormat;
import org.javenstudio.raptor.fs.shell.Count;
import org.javenstudio.raptor.io.DataInputBuffer;
import org.javenstudio.raptor.io.DataOutputBuffer;
...
import org.javenstudio.raptor.util.Tool;
...
public class FsShell extends Configured implements Tool {
  ...
  protected void init() throws IOException {
    getConf().setQuietMode(QUIET);
    if (this.fs == null) {
      initNamesystem(getConf()); 
   ...
   * @see org.javenstudio.raptor.fs.FileSystem.globStatus 
   */
   ...
   * @throws IOException  
   * @see org.javenstudio.raptor.fs.FileSystem#globStatus(Path)
   */
  ...
  private int rename(String argv[], Configuration conf) throws IOException {
    ...
    for (; i < argv.length - 1; i++) {
      try {
        ...
        //
        // This is a error returned by raptor server. Print
   ...
   * @deprecated Consider using {@link org.javenstudio.raptor.util.StringUtils#byteDesc} instead.
   */
  @Deprecated
  ...
  /**
   * @deprecated Consider using {@link org.javenstudio.raptor.util.StringUtils#limitDecimalTo2} instead.
   */
  @Deprecated
  ...
  private void printHelp(String cmd) {
    String summary = "java fs is the command to execute fs commands. " +
      "The full syntax is: \n\n" +
      "java fs [-fs <local | file system URI>] [-conf <configuration file>]\n\t" +
    ...
    String fs = "-fs [local | <file system URI>]: \tSpecify the file system to use.\n" + 
      ...
      "\t\t\tcore-default.xml inside the raptor jar file \n" +
      "\t\t\tcore-site.xml in $HAWK_CONF_DIR \n" +
    ...
    String lsr = "-lsr <path>: \tRecursively list the contents that match the specified\n" +
      "\t\tfile pattern.  Behaves very similarly to raptor fs -ls,\n" + 
  ...
  private static void printUsage(String cmd) {
    ...
    if ("-fs".equals(cmd)) {
      ...
      System.err.println("Usage: java FsShell [command] <options...> <parameters...>");
      System.err.println("the commands and options are");
      System.err.println(" [-ls <path>]                 List the contents that match the specified file pattern.");
      System.err.println(" [-lsr <path>]                Recursively list the contents that match the specified");
      System.err.println(" [-du <path>]                 Show the amount of space, in bytes");
      System.err.println(" [-dus <path>]                Show the amount of space, in bytes");
      System.err.println(" [" + Count.USAGE + "]          Count the number of directories, files and bytes");
      System.err.println(" [-mv <src> <dst>]            Move files that match the specified file pattern to a destination");
      System.err.println(" [-cp <src> <dst>]            Copy files that match the file pattern <src> to a destination");
      System.err.println(" [-rm [-skipTrash] <path>]    Delete all files that match the specified file pattern");
      System.err.println(" [-rmr [-skipTrash] <path>]   Remove all directories which match the specified file pattern");
      System.err.println(" [-expunge]                   ");
      System.err.println(" [-put <localsrc> ... <dst>]  Copy files from the local file system into fs");
      System.err.println(" [-copyFromLocal <localsrc> ... <dst>]  Identical to the -put command");
      System.err.println(" [-moveFromLocal <localsrc> ... <dst>]  Same as -put, except that the source is deleted after it's copied");
      System.err.println(" [" + GET_SHORT_USAGE + "]    Copy files that match the file pattern to the local name");
      System.err.println(" [-getmerge <src> <localdst> [addnl]]  Get all the files in the directories that match the source file pattern");
      System.err.println(" [-cat <src>]                 Fetch all files that match the file pattern and display");
      System.err.println(" [-text <src>]                Takes a source file and outputs the file in text format");
      System.err.println(" [" + COPYTOLOCAL_SHORT_USAGE + "]  Identical to the -get command");
      System.err.println(" [-moveToLocal [-crc] <src> <localdst>]   Not implemented yet");
      System.err.println(" [-mkdir <path>]              Create a directory in specified location");
      System.err.println(" [" + SETREP_SHORT_USAGE + "]  Set the replication level of a file");
      System.err.println(" [-touchz <path>]             Write a timestamp in yyyy-MM-dd HH:mm:ss format in a file");
      System.err.println(" [-test -[ezd] <path>]        If file { exists, has zero length, is a directory then return 0, else return 1");
      System.err.println(" [-stat [format] <path>]      Print statistics about the file/directory in the specified format");
      System.err.println(" [" + TAIL_USAGE + "]          Show the last 1KB of the file");
      System.err.println(" [" + FsShellPermissions.CHMOD_USAGE + "]   Changes permissions of a file");      
      System.err.println(" [" + FsShellPermissions.CHOWN_USAGE + "]   Changes owner and group of a file");
      System.err.println(" [" + FsShellPermissions.CHGRP_USAGE + "]  This is equivalent to -chown ... :GROUP ...");
      System.err.println(" [-help [cmd]]                Displays help for given command or all commands if none");
      System.err.println();
  ...
  protected static void doMain(FsShell shell, String argv[]) throws Exception {
    int res;
    try {
