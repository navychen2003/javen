...
public class Path implements Comparable<Object> {
  ...
  public Path(String pathString) {
    ...
    if ((colon != -1) && //slash >= 0 &&            // RFC 2396 not support opaque part here
  ...
  public static String getWindowsDrive(String path, boolean slashed) {
    if (!WINDOWS) return "/";
    int start = slashed ? 1 : 0;
    if (path.length() >= start+2 &&
      (slashed ? path.charAt(0) == '/' : true) &&
      path.charAt(start+1) == ':' &&
      ((path.charAt(start) >= 'A' && path.charAt(start) <= 'Z') ||
       (path.charAt(start) >= 'a' && path.charAt(start) <= 'z')))
    	return (""+path.charAt(start)+":/").toUpperCase();
    return "/";
  }
  ...
  public int compareTo(Object o) {
	if (o == null || !(o instanceof Path)) return -1;
    Path that = (Path)o;
