...
import org.javenstudio.jfm.main.JFMClassLoader;
...
public abstract class JFMFileSystem {
  ...
  public static final String LOCAL_FILESYSTEM = "org.javenstudio.jfm.filesystems.local.JFMLocalFilesystem";
  public static final String SFTP_FILESYSTEM  = "org.javenstudio.jfm.filesystems.sftp.JFMSftpFilesystem";
  public static final String DFS_FILESYSTEM   = "org.javenstudio.jfm.filesystems.dfs.JFMDfsFilesystem";
  public static final String BDB_FILESYSTEM   = "org.javenstudio.jfm.filesystems.bdb.JFMDBFilesystem";
  ...
  private static List<String> registeredNames = new ArrayList<String>(); 

  static{
    //we always have the local filesystem available
    registeredFilesystems.put("local", LOCAL_FILESYSTEM);
    registeredFilesystems.put("sftp",  SFTP_FILESYSTEM);
    registeredNames.add("local"); 
    registeredNames.add("sftp"); 

    try {
      if (Class.forName("org.javenstudio.raptor.conf.Configuration") != null &&
          Class.forName("org.javenstudio.raptor.fs.FileSystem") != null) {
        registeredFilesystems.put("dfs", DFS_FILESYSTEM);
        registeredNames.add("dfs"); 
      }
    } catch (Exception e) {
    }
    ...
    try {
      if (Class.forName("org.javenstudio.raptor.conf.Configuration") != null &&
        ...
        //registeredFilesystems.put("bdb", BDB_FILESYSTEM);
        //registeredNames.add("bdb"); 
      }
    } catch (Exception e) {
    }
  }
  ...
  public static String getRegisteredFilesystem(String name) {
    return name != null ? registeredFilesystems.get(name) : null; 
  }

  public static String getFsLongName(String name) {
    String className = getRegisteredFilesystem(name); 
    if (className == null) return null; 

    try {
      Class<?> clazz = Class.forName(className); 
      Method nameMethod = clazz.getDeclaredMethod("getLongName", (Class<?>[])null);
      return (String) nameMethod.invoke(clazz); 
    } catch (Exception e) {
      e.printStackTrace(); 
      return null; 
    }
  ...
  public static String getFsIconName(String name) {
    String className = getRegisteredFilesystem(name);
    if (className == null) return null;

    try {
      Class<?> clazz = Class.forName(className);
      Method nameMethod = clazz.getDeclaredMethod("getIconName", (Class<?>[])null);
      return (String) nameMethod.invoke(clazz); 
    } catch (Exception e) {
      e.printStackTrace(); 
      return null;
  ...
  public static boolean setConnectUri(String name, URI uri) {
    String className = getRegisteredFilesystem(name);
    if (className == null) return false;

    try {
      Class<?> clazz = Class.forName(className);
      Method nameMethod = clazz.getDeclaredMethod("setConnectUri", new Class<?>[]{URI.class});
      return ((Boolean)nameMethod.invoke(clazz, uri)).booleanValue();
    } catch (Exception e) {
      e.printStackTrace();
      return false;
    }
  ...
  public static int tryDisconnectFs(String name) {
    String className = getRegisteredFilesystem(name);
    if (className == null) return -1;

    try {
      Class<?> clazz = Class.forName(className);
      Method nameMethod = clazz.getDeclaredMethod("tryDisconnect", (Class<?>[])null);
      return ((Integer)nameMethod.invoke(clazz)).intValue();
    } catch (Exception e) {
      e.printStackTrace();
      return -1;
    }
  ...
  public static String getFsConnectName(String name) {
    String className = getRegisteredFilesystem(name);
    if (className == null) return null;

    try {
      Class<?> clazz = Class.forName(className);
      Method nameMethod = clazz.getDeclaredMethod("getConnectName", (Class<?>[])null);
      return (String) nameMethod.invoke(clazz); 
    } catch (Exception e) {
      e.printStackTrace();
      return null;
  ...
  private static void registerFilesystem(File f) 
    ...
    if (!registeredFilesystems.containsKey(name)) {
      registeredFilesystems.put(name,theClass);  
      registeredNames.add(name); 
    } else {
      System.out.println("  WARN: "+name+"="+theClass+" already existed."); 
      return; 
    }

    List<String> dependencies = handler.getDependencies();
    for (int i=0; i<dependencies.size(); i++) {
      JFMClassLoader.addAditionalJar(dependencies.get(i));
    }
  }
  ...
  public static JFMFileSystem createFileSystem(String name) {
    ...
    if(fs == null) {
      name = null;
      try {
        Class fsClass = Class.forName(LOCAL_FILESYSTEM);
        fs = (JFMFileSystem)fsClass.newInstance();
      } catch (Exception e) {
        e.printStackTrace();
        //ignore the exception
    ...
    /**
     * The default root directory in the case of filesystems with more than 
     * one root directories (like windows)
     ...
     */
    public abstract JFMFile[] listFiles(JFMFile rootFile) throws IOException;
     ...
     *
     */
    public abstract JFMFile[] listFiles(JFMFile rootFile, FilenameFilter filter) throws IOException;
     ...
     */
    public abstract boolean mkdir(JFMFile parent, String name) throws IOException;
     ...
     *
     */
    public abstract boolean createNewFile(JFMFile parent, String name) throws IOException;
     ...
     * <code>{@link org.javenstudio.jfm.filesystems.JFMFile#lastModified}</code> method will return the (possibly
     ...
     */
    public abstract boolean setLastModified(JFMFile file, long time) throws IOException;
     ...
     *          method denies write access to the named file
     */
    public abstract boolean setReadOnly(JFMFile file) throws IOException;
     ...
     * method should return the drive letter of the drive that this file belongs to. On Unix filesystems 
    ...
    public static JFMFile newFile(URI uri) {
      try {
        String name = uri.getScheme(); 
        String path = uri.getPath(); 
        JFMFileSystem fs = createFileSystem(getRegisteredFilesystem(name)); 
