...
import org.javenstudio.mail.Utility;
...
import org.javenstudio.mail.store.ImapResponseParser.ImapList;
import org.javenstudio.mail.store.ImapResponseParser.ImapResponse;
import org.javenstudio.mail.transport.CountingOutputStream;
...
public class ImapStore extends Store {
	private static Logger LOG = Logger.getLogger(ImapStore.class); 
    ...
    public static Store newInstance(String uri, PersistentDataCallbacks callbacks)
    	...
    	try { 
    		...
    		LOG.error("ImapStore.newInstance(\""+uri+"\") error", e); 
    		throw e; 
    	}
    }
     ...
     */
    protected ImapStore(String uriString) throws MessagingException {
    	if (LOG.isDebugEnabled()) 
    		LOG.debug("ImapStore: new ImapStore with "+uriString); 
     ...
     */
    public String getImapId(String userName, String host, String capability) {
        // The first section is global to all IMAP connections, and generates the fixed
        // values in any IMAP ID message
        synchronized (ImapStore.class) {
            if (sImapId == null) 
                sImapId = Preference.getPreference().makeCommonImapId();
        }
        ...
        String vendorId =
        		Preference.getPreference().getImapIdValues(userName, host, capability);
        ...
        try {
            String devUID = Preference.getPreference().getDeviceUID();
            ...
            LOG.warn("ImapStore: couldn't obtain SHA-1 hash for device UID");
        }
    ...
    @Override
    public void closeFolders(boolean expunge) throws MessagingException { 
    	synchronized (mFolderCache) {
    		for (ImapFolder folder : mFolderCache.values()) { 
    			if (folder.isOpen()) 
        ...
        try {
            ...
            for (ImapResponse response : responses) {
                if (response.get(0).equals("LIST")) {
                    ...
                    if (folder.equalsIgnoreCase(MailboxData.NAME_INBOX)) 
                        continue;
            ...
            folders.add(getFolder(MailboxData.NAME_INBOX));
    ...
    protected class ImapFolder extends Folder {
        ...
        public void open(OpenMode mode, PersistentDataCallbacks callbacks)
            ...
            try {
                ...
                for (ImapResponse response : responses) {
                    if (response.getTag() == null && response.get(1).equals("EXISTS")) {
        ...
        public void copyMessages(Message[] messages, Folder folder,
            ...
            try {
                ...
                for (ImapResponse response : responseList) {
                    ...
                    ImapList copyResponse = response.getList(1);
                    if ("COPYUID".equals(copyResponse.getString(0))) {
                        String origIdSet = copyResponse.getString(2);
                        String newIdSet = copyResponse.getString(3);
                        
                        String[] origIdArray = ImapUtility.getSequenceValues(origIdSet);
                        String[] newIdArray = ImapUtility.getSequenceValues(newIdSet);
                        
                        // There has to be a 1:1 mapping between old and new IDs
                        if (origIdArray == null || newIdArray == null || origIdArray.length != newIdArray.length) {
                        ...
                        for (int i = 0; i < origIdArray.length; i++) {
                            ...
                            if (message != null) 
                                callbacks.onMessageUidChange(message, newIdArray[i]);
                        }
                    }
                }
            } catch (IOException ioe) {
            ...
            try {
                ...
                List<ImapResponse> responses = mConnection.executeSimpleCommand(
                        String.format("STATUS \"%s\" (UNSEEN)",
                                encodeFolderName(mName)));
                for (ImapResponse response : responses) {
                    if (response.getTag() == null && response.get(0).equals("STATUS")) {
        ...
        public Message getMessage(String uid) throws MessagingException {
            ...
            try {
                ...
                for (ImapResponse response : responses) {
                    if (response.getTag() == null && response.get(0).equals("SEARCH")) {
            ...
            if (LOG.isDebugEnabled()) { 
            	LOG.debug("ImageStore: message uid: "+uid+" not found on server"); 
            }
            return null; //new ImapMessage(uid, this);
        }
        ...
        public void fetch(Message[] messages, FetchProfile fp, MessageRetrievalListener listener)
            ...
            try {
                ...
                LOG.warn("ImapStore: Exception detected: " + e.getMessage());
        ...
        public void fetchInternal(Message[] messages, FetchProfile fp,
            ...
            if (fp.contains(FetchProfile.Item.BODY_SANE)) {
                fetchFields.add(String.format("BODY.PEEK[]<0.%d>", Preference.getPreference().getBodySaneSize()));
            }
            ...
            for (Object o : fp) {
                ...
                } else if (o instanceof FetchProfile.Attachment) { 
                	FetchProfile.Attachment att = (FetchProfile.Attachment)o; 
                	String partId = att.getLocation(); 
                	if (partId != null) { 
                        ...
                        if (fp.contains(FetchProfile.Item.STRUCTURE)) {
                            ...
                            if (bs != null) {
                                try {
                                    ...
                                    if (LOG.isDebugEnabled()) {
                                        LOG.debug("ImapStore: Error handling message", e);
                                    }
                        ...
                        for (Object o : fp) {
                            if (o instanceof Part) {
                            	...
                            	InputStream bodyStream =
                                    ...
                                    String contentTransferEncoding = att.getContentTransferEncoding(); 
                                    MimeUtility.decodeBody(
                                            bodyStream,
                                            contentTransferEncoding, 
                                            att.getOutputBody()); 
                            }
         ...
         * @param responses
         */
        protected void handleUntaggedResponses(List<ImapResponse> responses) {
            for (ImapResponse response : responses) {
        ...
        protected void parseBodyStructure(ImapList bs, Part part, String id)
                throws MessagingException {
...
//                    [MESSAGE, RFC822, [NAME, Fwd: [#HTR-517941]:  update plans at 1am Friday - Memory allocation - displayware.eml], NIL, NIL, 7BIT, 5974, NIL, [INLINE, [FILENAME*0, Fwd: [#HTR-517941]:  update plans at 1am Friday - Memory all, FILENAME*1, ocation - displayware.eml]], NIL]
        ...
        public void appendMessages(Message[] messages) throws MessagingException {
            ...
            try {
                for (Message message : messages) {
                    ...
                    do {
                        response = mConnection.readResponse();
                        if (response.isCommandContinuationRequested()) {
                    ...
                    for (ImapResponse response1 : responses) {
                        if (response1.getTag() == null && response1.get(0).equals("SEARCH")
                                && response1.size() > 1) {
        ...
        protected MessagingException ioExceptionHandler(ImapConnection connection, IOException ioe)
                throws MessagingException {
            connection.close();
            close(false);
     ...
     */
    protected class ImapConnection {
            ...
            try {
                ...
                if (mTransport.canTryTlsSecurity()) { 
                    if (capabilities.contains("STARTTLS")) {
                        ...
                        if (LOG.isDebugEnabled()) 
                            LOG.debug("ImapStore: TLS not supported but required");
                ...
                String userAgent = getImapId(mUsername, mRootTransport.getHost(), capabilities);
                if (userAgent != null) {
                    mIdPhrase = "ID (" + userAgent + ")";
                ...
                if (mIdPhrase != null) {
                    try {
                        ...
                        if (LOG.isDebugEnabled()) 
                            LOG.debug("ImapStore: " + ie.toString(), ie);
                ...
                try {
                   if (mHost.endsWith("yahoo.com")) {
                        if (LOG.isDebugEnabled())
                            LOG.debug("ImapStore: Found Yahoo! account. Sending proprietary commands.");
                ...
                if (LOG.isDebugEnabled()) 
                    LOG.debug("ImapStore: open connection error", e);
                ...
                if (LOG.isDebugEnabled()) 
                    LOG.debug("ImapStore: open connection error", ioe);
                
                throw ioe;
                
            } catch (MessagingException me) { 
            	if (LOG.isDebugEnabled()) 
                    LOG.debug("ImapStore: open connection error", me);
            	
            	throw me; 
            	
            } catch (NullPointerException ne) { 
            	if (LOG.isDebugEnabled()) 
                    LOG.debug("ImapStore: open connection error", ne);
            	
            	throw new MessagingException(ne.getMessage(), ne); 
            }
        }
...
//            }
        ...
        public List<ImapResponse> executeSimpleCommand(String command, boolean sensitive)
            ...
            do {
                ...
                if (response.getTag() != null && !response.getTag().equals(tag)
    ...
    protected class ImapMessage extends MimeMessage {
    ...
    protected class ImapBodyPart extends MimeBodyPart {
...
//        }
    ...
    protected class ImapException extends MessagingException {
