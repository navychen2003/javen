...
public class SimpleSocketFactory extends javax.net.SocketFactory 
		implements org.apache.http.conn.scheme.SocketFactory, ISocketFactory {
    ...
    private final HostNameResolver mNameResolver;
    ...
    @Override 
    public SimpleSocketFactory getFactoryInstance() { 
    	return this;
    }
    ...
    public Socket connectSocket(Socket sock, String host, int port, 
            InetAddress localAddress, int localPort, int timeout) throws IOException {
        if (host == null) 
        ...
        if (mNameResolver != null) {
            remoteAddress = new InetSocketAddress(mNameResolver.resolve(host), port); 
        } else {
