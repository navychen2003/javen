...
import org.javenstudio.common.indexdb.IndexFormatTooOldException;
...
public final class CodecUtil {
	 ...
	 */
	public final static int CODEC_MAGIC = Constants.CODEC_MAGIC; //0x3fd76c17;
	 ...
	 */
	public static void writeHeader(IDataOutput out, String codec, int version)
			throws IOException {
		BytesRef bytes = new BytesRef(codec);
		if (bytes.getLength() != codec.length() || bytes.getLength() >= 128) {
			throw new IllegalArgumentException("codec must be simple ASCII, " + 
					"less than 128 characters in length [got " + codec + "]");
		}
	 ...
	 * @see #writeHeader(DataOutput, String, int)
	 */
	public static int checkHeader(IDataInput in, String codec, int minVersion, int maxVersion)
			throws IOException {
		// Safety to guard against reading a bogus string:
		final int actualHeader = in.readInt();
		if (actualHeader != CODEC_MAGIC) {
			throw new CorruptIndexException("codec header mismatch: actual header=" + actualHeader + 
					" vs expected header=" + CODEC_MAGIC + " (resource: " + in + ")");
		}
	 ...
	 */
	public static int checkHeaderNoMagic(IDataInput in, String codec, int minVersion, int maxVersion) 
			throws IOException {
		final String actualCodec = in.readString();
		if (!actualCodec.equals(codec)) {
			throw new CorruptIndexException("codec mismatch: actual codec=" + actualCodec + 
					" vs expected codec=" + codec + " (resource: " + in + ")");
		}
