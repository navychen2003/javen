...
import org.javenstudio.hornet.codec.block.BlockTreeTermsReader;
import org.javenstudio.hornet.codec.block.BlockTreeTermsWriter;
import org.javenstudio.hornet.codec.stored.StoredTermsFormat;
 ...
 * BlockTreeTermsDict around StoredPostingsBaseFormat; ie
 * we should not duplicate the code from that class here:
 */
public class StoredPostingsFormat extends PostingsFormat {

	private final StoredTermPostingsFormat mTermPostingsFormat;
	private final StoredTermsFormat mTermsFormat;
	
	public StoredPostingsFormat(IIndexFormat format) { 
		mTermPostingsFormat = new StoredTermPostingsFormat(format);
		mTermsFormat = new StoredTermsFormat(format);
	}
	
	public final StoredTermPostingsFormat getTermPostingsFormat() { 
		return mTermPostingsFormat; 
	}
	
	@Override
	public final String getName() { 
		return mTermPostingsFormat.getFormatName(); 
	}
	
	@Override
	public FieldsConsumer getFieldsConsumer(IDirectory dir, ISegmentWriteState state) 
			throws IOException {
		TermPostingsWriter docs = mTermPostingsFormat.createWriter(dir, state);
	    ...
	    try {
	    	FieldsConsumer ret = new BlockTreeTermsWriter(
	    			mTermsFormat, (SegmentWriteState)state, docs);
	    	success = true;
	    	return ret;
	    } finally {
	    	if (!success) 
	...
	public FieldsProducer getFieldsProducer(IDirectory dir, ISegmentReadState state) 
			throws IOException {
	    TermPostingsReader postings = mTermPostingsFormat.createReader(dir, state);

	    boolean success = false;
	    try {
	    	FieldsProducer ret = new BlockTreeTermsReader(
	    			mTermsFormat, (SegmentReadState)state, postings);
	    	success = true;
	    	return ret;
	    } finally {
	    	if (!success) 
