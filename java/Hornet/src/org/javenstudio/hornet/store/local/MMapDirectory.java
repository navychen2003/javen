...
import org.javenstudio.hornet.store.ByteBufferIndexInput;
...
public class MMapDirectory extends FSDirectory {
	 ...
	 */
	public static final int DEFAULT_MAX_BUFF = JvmUtil.JRE_IS_64BIT ? (1 << 30) : (1 << 28);
	
	private boolean mUseUnmapHack = UNMAP_SUPPORTED;
	...
	public MMapDirectory(File path, LockFactory lockFactory, 
		...
		mChunkSizePower = 31 - Integer.numberOfLeadingZeros(maxChunkSize);
		assert mChunkSizePower >= 0 && mChunkSizePower <= 30;
	}
	...
	public final int getMaxChunkSize() {
		return 1 << mChunkSizePower;
	}

	/** Creates an IndexInput for the file with the given name. */
	@Override
	public IndexInput openIndexInput(IIndexContext context, String name) throws IOException {
		ensureOpen();
		File f = new File(getDirectory(), name);
		RandomAccessFile raf = new RandomAccessFile(f, "r");
		try {
			return new MMapIndexInput(context, raf);
		} finally {
			raf.close();
		}
	}
	...
	@Override
	public IndexInputSlicer createSlicer(IIndexContext context, String name) throws IOException {
		final MMapIndexInput full = (MMapIndexInput) openInput(context, name);
		return new IndexInputSlicer() {
			@Override
			public IndexInput openSlice(long offset, long length) throws IOException {
				ensureOpen();
				return full.slice(offset, length);
			}
	...
	private final class MMapIndexInput extends ByteBufferIndexInput {
		...
		MMapIndexInput(IIndexContext context, RandomAccessFile raf) throws IOException {
			super(context, map(raf, 0, raf.length()), raf.length(), mChunkSizePower);
		}
	...
	private ByteBuffer[] map(RandomAccessFile raf, long offset, long length) throws IOException {
		if ((length >>> mChunkSizePower) >= Integer.MAX_VALUE)
			throw new IllegalArgumentException("RandomAccessFile too big for chunk size: " + raf.toString());
    
		final long chunkSize = 1L << mChunkSizePower;
    
		// we always allocate one more buffer, the last one may be a 0 byte one
		final int nrBuffers = (int) (length >>> mChunkSizePower) + 1;
