...
public class OpenBitSetIterator extends DocIdSetIterator {
	...
	protected final static int[] sBitlist = {
  ...
  ******/
	...
	protected final long[] mArr;
	protected final int mWords;
	private int mPos = -1; // i
	private long mWord;
	private int mWordShift;
	private int mIndexArray;
	private int mCurDocId = -1;

	public OpenBitSetIterator(OpenBitSet obs) {
		this(obs.getBitsArray(), obs.getNumWords());
	}
	...
	// 64 bit shifts
	private void shift() {
		if ((int)mWord ==0) {mWordShift +=32; mWord = mWord >>>32; }
		if ((mWord & 0x0000FFFF) == 0) { mWordShift +=16; mWord >>>=16; }
		if ((mWord & 0x000000FF) == 0) { mWordShift +=8; mWord >>>=8; }
		mIndexArray = sBitlist[(int)mWord & 0xff];
	}
  ...
  ******/
	...
	public int nextDoc() {
		if (mIndexArray == 0) {
			...
			while (mWord == 0) {
				if (++mPos >= mWords) 
					return mCurDocId = NO_MORE_DOCS;
				
				mWord = mArr[mPos];
				mWordShift = -1; // loop invariant code motion should move this
			}
		...
		int bitIndex = (mIndexArray & 0x0f) + mWordShift;
		mIndexArray >>>= 4;
		
		// should i<<6 be cached as a separate variable?
		// it would only save one cycle in the best circumstances.
		return mCurDocId = (mPos<<6) + bitIndex;
	}
	...
	public int advance(int target) {
		...
		mPos = target >> 6;
		if (mPos >= mWords) {
			mWord = 0; // setup so next() will also return -1
			return mCurDocId = NO_MORE_DOCS;
		}
		
		mWordShift = target & 0x3f;
		mWord = mArr[mPos] >>> mWordShift;
		
		if (mWord != 0) {
			mWordShift --; // compensate for 1 based arrIndex
