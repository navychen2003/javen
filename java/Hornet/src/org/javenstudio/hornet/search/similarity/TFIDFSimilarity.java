...
import org.javenstudio.common.indexdb.util.SmallFloat;
...
public abstract class TFIDFSimilarity extends Similarity {
	 ...
	 * {@link org.javenstudio.common.indexdb.index.segment.apache.lucene.index.IndexReader#numDocs() IndexReader#numDocs()} because also 
     ...
     * @throws IOException
     */
	public IExplanation idfExplain(ICollectionStatistics collectionStats, ITermStatistics termStats) {
		final long df = termStats.getDocFreq();
		final long max = collectionStats.getMaxDoc();
	...
	public IExplanation idfExplain(ICollectionStatistics collectionStats, ITermStatistics termStats[]) {
		...
		for (final ITermStatistics stat : termStats ) {
			final long df = stat.getDocFreq();
	...
	@Override
	public final SimilarityWeight computeWeight(float queryBoost, 
			ICollectionStatistics collectionStats, ITermStatistics... termStats) {
		final IExplanation idf = termStats.length == 1 ?
				idfExplain(collectionStats, termStats[0]) : idfExplain(collectionStats, termStats);
		return new IDFStats(collectionStats.getField(), idf, queryBoost);
	}

	@Override
	public final IExactSimilarityScorer getExactSimilarityScorer(ISimilarityWeight stats, 
			IAtomicReaderRef context) throws IOException {
		IDFStats idfstats = (IDFStats) stats;
		return new ExactTFIDFDocScorer(idfstats); //, context.getReader().normValues(idfstats.field));
	}

	@Override
	public final ISloppySimilarityScorer getSloppySimilarityScorer(ISimilarityWeight stats, 
			IAtomicReaderRef context) throws IOException {
		IDFStats idfstats = (IDFStats) stats;
		return new SloppyTFIDFDocScorer(idfstats); //, context.getReader().normValues(idfstats.field));
	}
  
	// TODO: we can specialize these for omitNorms up front, but 
	// we should test that it doesn't confuse stupid hotspot.

	protected final class ExactTFIDFDocScorer extends ExactSimilarityScorer {
		private static final int SCORE_CACHE_SIZE = 32;
		
		private final IDFStats mStats;
		private final float mWeightValue;
		private final byte[] mNorms;
		private float[] mScoreCache = new float[SCORE_CACHE_SIZE];
    
		ExactTFIDFDocScorer(IDFStats stats) throws IOException {
			mStats = stats;
			mWeightValue = stats.mValue;
			mNorms = null; //norms == null ? null : (byte[])norms.getSource().getArray(); 
			for (int i = 0; i < SCORE_CACHE_SIZE; i++) {
				mScoreCache[i] = tf(i) * mWeightValue;
			}
		}
		...
		public float score(int doc, int freq) {
			final float raw =                	// compute tf(f)*weight
					...
					? mScoreCache[freq]        	// cache hit
					: tf(freq)*mWeightValue;   	// cache miss

			return (mNorms == null) ? raw : raw * decodeNormValue(mNorms[doc]); // normalize for field
		}

		@Override
		public IExplanation explain(int doc, IExplanation freq) {
			return explainScore(doc, freq, mStats, mNorms);
		}
	}
  
	protected final class SloppyTFIDFDocScorer extends SloppySimilarityScorer {
		private final IDFStats mStats;
		private final float mWeightValue;
		private final byte[] mNorms;
    
		SloppyTFIDFDocScorer(IDFStats stats) throws IOException {
			mStats = stats;
			mWeightValue = stats.mValue;
			mNorms = null; //norms == null ? null : (byte[])norms.getSource().getArray();
		}
		...
		public float score(int doc, float freq) {
			final float raw = tf(freq) * mWeightValue; // compute tf(f)*weight
	 ...
	 */
	protected static class IDFStats extends SimilarityWeight {
		private final String mField;
		/** The idf and its explanation */
		private final IExplanation mIdf;
		private float mQueryNorm;
		private float mQueryWeight;
		private final float mQueryBoost;
		private float mValue;
    
		public IDFStats(String field, IExplanation idf, float queryBoost) {
			// TODO: Validate?
			mField = field;
			mIdf = idf;
			mQueryBoost = queryBoost;
			mQueryWeight = idf.getValue() * queryBoost; // compute query weight
		}

		public final String getField() { return mField; }
		public final float getValue() { return mValue; }
		
		@Override
		public float getValueForNormalization() {
			// TODO: (sorta LUCENE-1907) make non-static class and expose 
			// this squaring via a nice method to subclasses?
			return mQueryWeight * mQueryWeight;			// sum of squared weights
		}

		@Override
		public void normalize(float queryNorm, float topLevelBoost) {
			mQueryNorm = queryNorm * topLevelBoost;
			mQueryWeight *= mQueryNorm;             	// normalize query weight
			mValue = mQueryWeight * mIdf.getValue();	// idf for document
		}
	}
  
	private IExplanation explainScore(int doc, IExplanation freq, IDFStats stats, byte[] norms) {
		...
		Explanation boostExpl = new Explanation(stats.mQueryBoost, "boost");
		if (stats.mQueryBoost != 1.0f)
			queryExpl.addDetail(boostExpl);
		queryExpl.addDetail(stats.mIdf);

		Explanation queryNormExpl = new Explanation(stats.mQueryNorm, "queryNorm");
		queryExpl.addDetail(queryNormExpl);

		queryExpl.setValue(boostExpl.getValue() * stats.mIdf.getValue() * queryNormExpl.getValue());
		...
		fieldExpl.addDetail(stats.mIdf);
		...
		fieldExpl.setValue(tfExplanation.getValue() * stats.mIdf.getValue() * fieldNormExpl.getValue());
