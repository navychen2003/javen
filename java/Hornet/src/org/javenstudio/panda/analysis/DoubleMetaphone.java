...
/**
 * Encodes a string into a double metaphone value. This Implementation is based on 
 * the algorithm by <CITE>LawrencePhilips</CITE>.
 * <p>
 * This class is conditionally thread-safe. The instance field {@link #maxCodeLen} is mutable
 * {@link #setMaxCodeLen(int)} but is not volatile, and accesses are not synchronized. 
 * If an instance of the class is shared between threads, the caller needs to ensure 
 * that suitable synchronization is used to ensure safe publication
 * of the value between threads, and must not invoke {@link #setMaxCodeLen(int)} 
 * after initial setup.
 *
...
public class DoubleMetaphone {
    ...
    public class DoubleMetaphoneResult {
        ...
        private final StringBuilder mPrimary = new StringBuilder(getMaxCodeLen());
        private final StringBuilder mAlternate = new StringBuilder(getMaxCodeLen());
        ...
        public void appendPrimary(char value) {
            if (mPrimary.length() < mMaxLength) {
                mPrimary.append(value);
            }
        }

        public void appendAlternate(char value) {
            if (mAlternate.length() < mMaxLength) {
                mAlternate.append(value);
            }
        }
        ...
        public void appendPrimary(String value) {
            int addChars = mMaxLength - mPrimary.length();
            if (value.length() <= addChars) {
                mPrimary.append(value);
            } else {
                mPrimary.append(value.substring(0, addChars));
            }
        }

        public void appendAlternate(String value) {
            int addChars = mMaxLength - mAlternate.length();
            if (value.length() <= addChars) {
                mAlternate.append(value);
            } else {
                mAlternate.append(value.substring(0, addChars));
            }
        }

        public String getPrimary() {
            return mPrimary.toString();
        }

        public String getAlternate() {
            return mAlternate.toString();
        }

        public boolean isComplete() {
            return mPrimary.length() >= mMaxLength &&
                   mAlternate.length() >= mMaxLength;
        }
    }
}
