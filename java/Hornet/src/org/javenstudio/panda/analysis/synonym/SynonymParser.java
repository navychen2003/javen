 ...
 *  </ol>
 */
public class SynonymParser extends SynonymMap.Builder {
	
	private final Analyzer mAnalyzer;
	private final boolean mExpand;
  
	public SynonymParser(boolean dedup, boolean expand, Analyzer analyzer) {
		super(dedup);
		...
		while ((line = in.readLine()) != null) {
			...
			if (sides.length > 1) { // explicit mapping
				...
				for (int i = 0; i < inputs.length; i++) {
					inputs[i] = analyze(mAnalyzer, unescape(inputStrings[i]).trim(), new CharsRef());
				}
				...
				for (int i = 0; i < outputs.length; i++) {
					outputs[i] = analyze(mAnalyzer, unescape(outputStrings[i]).trim(), new CharsRef());
				}
