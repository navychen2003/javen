...
public class SectionDetailsItem extends SectionDetailsTab.DetailsItem {

	public static class NameValue implements INameValue, View.OnClickListener { 
	...
	public SectionDetailsItem(int name) {
		this(ResourceHelper.getResources().getString(name));
	}
	
	public SectionDetailsItem(String name) {
		mName = name;
	}
	...
	public View getView(Context context, View convertView, boolean dropDown) { 
		...
		if (view == null || view.getId() != R.layout.sectioninfo_details) 
			view = inflater.inflate(R.layout.sectioninfo_details, null);
		
		View contentView = view.findViewById(R.id.sectioninfo_details_content);
		if (contentView != null) {
			int backgroundRes = getCardBackgroundRes();
			if (backgroundRes != 0) contentView.setBackgroundResource(backgroundRes);
		}
		
		TextView titleView = (TextView)view.findViewById(R.id.sectioninfo_details_title);
		if (titleView != null) { 
		...
		ViewGroup itemsRoot = (ViewGroup)view.findViewById(R.id.sectioninfo_details_items);
		if (itemsRoot != null) {
	...
	protected void bindNameValues(LayoutInflater inflater, ViewGroup itemsRoot) {
		...
		synchronized (mItems) { 
			for (final NameValue item : mItems) { 
				...
				View itemView = inflater.inflate(R.layout.sectioninfo_details_item, itemsRoot, false);
				
				TextView itemName = (TextView)itemView.findViewById(R.id.sectioninfo_details_item_name);
				TextView itemValue = (TextView)itemView.findViewById(R.id.sectioninfo_details_item_value);
				View buttonView = (View)itemView.findViewById(R.id.sectioninfo_details_item_button);
	...
	protected int getCardBackgroundRes() {
		return AppResources.getInstance().getDrawableRes(
				AppResources.drawable.sectioninfo_card_background);
	}
