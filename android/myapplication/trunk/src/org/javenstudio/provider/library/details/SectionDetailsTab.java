...
public class SectionDetailsTab extends SectionActionTab {
	private static final Logger LOG = Logger.getLogger(SectionDetailsTab.class);

	public static abstract class DetailsItem { 
		private boolean mShown = false;
		public abstract View getView(Context context, View convertView, boolean dropDown);
	}
	
	private final List<DetailsItem> mItems = new ArrayList<DetailsItem>();
	private ListAdapter mAdapter = null;
	
	public SectionDetailsTab(SectionInfoItem item, String name) { 
		this(item, name, 0);
	}
	
	public SectionDetailsTab(SectionInfoItem item, String name, int iconRes) { 
	...
	public void addDetailsItems(DetailsItem[] items) { 
		if (items == null) return;
		for (DetailsItem item : items) {
			addDetailsItem(item);
		}
	}
	...
	public int getDetailsItemCount() { 
		synchronized (mItems) {
			return mItems.size();
		}
	}
	...
	public void reloadOnThread(ProviderCallback callback, 
		...
		if (type == ReloadType.FORCE) postUpdateViews();
	}
	...
	public static class DetailsAdapter extends ArrayAdapter<DetailsItem> {
		...
		protected View getView(int position, View convertView, boolean dropDown) {
			...
			View view = item.getView(getContext(), convertView, dropDown);
			if (view != null && item.mShown == false) {
				Animation ani = AnimationUtils.loadAnimation(getContext(), R.anim.slide_out_up);
