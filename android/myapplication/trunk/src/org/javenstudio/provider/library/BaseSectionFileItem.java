...
import org.javenstudio.provider.library.section.SectionFileItem;

public abstract class BaseSectionFileItem extends SectionFileItem
		implements FileOperation.OnShowListener {
	private static final Logger LOG = Logger.getLogger(BaseSectionFileItem.class);

	public BaseSectionFileItem(LibraryProvider p, ISectionData data) {
		super(p, data);
	}
			...
			FileOperation.showOperationDialog(activity.getActivity(), ops, 
				...
				new FileOperation.OnSelectListener() {
					...
					public void onOperationSelected(Operation op) {
						BaseSectionFileItem.this.onOperationSelected(activity, op);
					}
				}, this);
			return true;
		}
	...
	protected void onOperationSelected(IActivity activity, 
		...
		} else if (op == FileOperation.Operation.DOWNLOAD) {
			onItemDownload(activity);
		}
	}
