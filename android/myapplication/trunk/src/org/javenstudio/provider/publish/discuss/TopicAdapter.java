...
public class TopicAdapter extends ColumnAdapter implements IActivityListener {
	private static final Logger LOG = Logger.getLogger(TopicAdapter.class);
	
	static TopicAdapter createAdapter(final IActivity activity, 
			final TopicDataSets dataSets, final DataBinder.BinderCallback callback, 
			int resource, int colsize) { 
		if (activity == null || dataSets == null || callback == null) 
			return null;
		
		final TopicAdapter adapter = new TopicAdapter(activity.getActivity(), 
		...
		adapter.setViewBinder(new AdvancedAdapter.ViewBinder() {
				...
				public void onViewBinded(AdvancedAdapter.DataSet dataSet, View view, int position) {
					final TopicDataSets dataSets = adapter.getDataSets();
					final int colsize = adapter.getColumnSize();
					
					if (LOG.isDebugEnabled())
						LOG.debug("bindRowView: container=" + view + " row=" + position + " colsize=" + colsize);
					
					TopicItem[] items = new TopicItem[colsize];
					
					for (int i=0; i < colsize; i++) { 
						TopicItem item = dataSets.getTopicItemAt(i + position);
	...
	private TopicAdapter(Context context, TopicDataSets data,
            int resource, int colsize) {
	...
	private TopicAdapter(AdapterImpl impl, int colsize) {
		super(impl);
	...
	public TopicDataSets getDataSets() { 
		return (TopicDataSets)getImpl().getDataSets();
	}
	
	public TopicDataSet getDataSet(int position) { 
		return (TopicDataSet)getImpl().getDataSet(position);
	}
	...
	private static class AdapterImpl extends AbstractAdapter<TopicItem> { 
		public AdapterImpl(Context context, TopicDataSets data, int resource) {
		...
		final void onActivityStop() { 
			for (int i=0; i < getCount(); i++) {
				TopicDataSet dataSet = (TopicDataSet)getDataSet(i);
				TopicItem item = dataSet != null ? dataSet.getTopicItem() : null;
				if (item != null) 
