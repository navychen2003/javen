...
public class TopicItem extends DataBinderItem implements ImageListener {
	private static final Logger LOG = Logger.getLogger(TopicItem.class);
	
	private final TopicProvider mProvider;
	private final ITopicData mData;
	
	private HttpImage mAvatar = null;
	private int mFetchRequest = 0;
	
	public TopicItem(TopicProvider p, ITopicData data) { 
		mProvider = p;
		mData = data;
	...
	public TopicProvider getProvider() { return mProvider; }
	public ITopicData getTopicData() { return mData; }
	
	public CharSequence getSummary() { 
		return InformationHelper.formatContentSpanned(getTopicData().getMessage()); 
	}
	
	public String getCreateDate() { 
		return Utilities.formatDate(getTopicData().getCreateDate()); 
	}
	
	public String getUpdateDate() { 
		return Utilities.formatDate(getTopicData().getUpdateDate()); 
	}
	
	public String getTitle() { 
		return getTopicData().getTopicSubject(); 
	}
	
	public String getUserName() { 
		return getTopicData().getUserName(); 
	}
	
	public View.OnClickListener getUserClickListener() { 
		return getTopicData().getUserClickListener(); 
	}
	
	public Drawable getProviderIcon() { 
		return getTopicData().getProviderIcon(); 
	}
	...
	public synchronized Drawable getUserIcon(int width, int height) { 
		if (mAvatar == null) {
			mAvatar = HttpResource.getInstance().getImage(
					getTopicData().getAvatarLocation());
	...
	protected void onUpdateViewsOnVisible(boolean restartSlide) { 
		TopicBinder binder = getTopicBinder();
		if (binder != null) binder.onUpdateViews(this);
	}
	
	private TopicBinder getTopicBinder() { 
		final TopicBinder binder = (TopicBinder)getProvider().getBinder();
