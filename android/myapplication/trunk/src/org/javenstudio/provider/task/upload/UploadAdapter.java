...
public class UploadAdapter extends ColumnAdapter implements IActivityListener {
	private static final Logger LOG = Logger.getLogger(UploadAdapter.class);
	
	static UploadAdapter createAdapter(final IActivity activity, 
			final UploadDataSets dataSets, final DataBinder.BinderCallback callback, 
			int resource, int colsize) { 
		if (activity == null || dataSets == null || callback == null) 
			return null;
		
		final UploadAdapter adapter = new UploadAdapter(activity.getActivity(), 
		...
		adapter.setViewBinder(new AdvancedAdapter.ViewBinder() {
				...
				public void onViewBinded(AdvancedAdapter.DataSet dataSet, View view, int position) {
					final UploadDataSets dataSets = adapter.getDataSets();
					final int colsize = adapter.getColumnSize();
					
					if (LOG.isDebugEnabled())
						LOG.debug("bindRowView: container=" + view + " row=" + position + " colsize=" + colsize);
					
					UploadItem[] items = new UploadItem[colsize];
					
					for (int i=0; i < colsize; i++) { 
						UploadItem item = dataSets.getUploadItemAt(i + position);
	...
	private UploadAdapter(Context context, UploadDataSets data,
            int resource, int colsize) {
	...
	private UploadAdapter(AdapterImpl impl, int colsize) {
		super(impl);
	...
	public UploadDataSets getDataSets() { 
		return (UploadDataSets)getImpl().getDataSets();
	}
	
	public UploadDataSet getDataSet(int position) { 
		return (UploadDataSet)getImpl().getDataSet(position);
	}
	...
	private static class AdapterImpl extends AbstractAdapter<UploadItem> { 
		public AdapterImpl(Context context, UploadDataSets data, int resource) {
		...
		final void onActivityStop() { 
			for (int i=0; i < getCount(); i++) {
				UploadDataSet dataSet = (UploadDataSet)getDataSet(i);
				UploadItem item = dataSet != null ? dataSet.getUploadItem() : null;
				if (item != null) 
