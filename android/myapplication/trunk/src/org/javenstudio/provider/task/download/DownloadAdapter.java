...
public class DownloadAdapter extends ColumnAdapter implements IActivityListener {
	private static final Logger LOG = Logger.getLogger(DownloadAdapter.class);
	
	static DownloadAdapter createAdapter(final IActivity activity, 
			final DownloadDataSets dataSets, final DataBinder.BinderCallback callback, 
			int resource, int colsize) { 
		if (activity == null || dataSets == null || callback == null) 
			return null;
		
		final DownloadAdapter adapter = new DownloadAdapter(activity.getActivity(), 
		...
		adapter.setViewBinder(new AdvancedAdapter.ViewBinder() {
				...
				public void onViewBinded(AdvancedAdapter.DataSet dataSet, View view, int position) {
					final DownloadDataSets dataSets = adapter.getDataSets();
					final int colsize = adapter.getColumnSize();
					
					if (LOG.isDebugEnabled())
						LOG.debug("bindRowView: container=" + view + " row=" + position + " colsize=" + colsize);
					
					DownloadItem[] items = new DownloadItem[colsize];
					
					for (int i=0; i < colsize; i++) { 
						DownloadItem item = dataSets.getDownloadItemAt(i + position);
	...
	private DownloadAdapter(Context context, DownloadDataSets data,
            int resource, int colsize) {
	...
	private DownloadAdapter(AdapterImpl impl, int colsize) {
		super(impl);
	...
	public DownloadDataSets getDataSets() { 
		return (DownloadDataSets)getImpl().getDataSets();
	}
	
	public DownloadDataSet getDataSet(int position) { 
		return (DownloadDataSet)getImpl().getDataSet(position);
	}
	...
	private static class AdapterImpl extends AbstractAdapter<DownloadItem> { 
		public AdapterImpl(Context context, DownloadDataSets data, int resource) {
		...
		final void onActivityStop() { 
			for (int i=0; i < getCount(); i++) {
				DownloadDataSet dataSet = (DownloadDataSet)getDataSet(i);
				DownloadItem item = dataSet != null ? dataSet.getDownloadItem() : null;
				if (item != null) 
