...
public class ContactAdapter extends ColumnAdapter implements IActivityListener {
	private static final Logger LOG = Logger.getLogger(ContactAdapter.class);
	
	static ContactAdapter createAdapter(final IActivity activity, 
			final ContactDataSets dataSets, final DataBinder.BinderCallback callback, 
			int resource, int colsize) { 
		if (activity == null || dataSets == null || callback == null) 
			return null;
		
		final ContactAdapter adapter = new ContactAdapter(activity.getActivity(), 
		...
		adapter.setViewBinder(new AdvancedAdapter.ViewBinder() {
				...
				public void onViewBinded(AdvancedAdapter.DataSet dataSet, View view, int position) {
					final ContactDataSets dataSets = adapter.getDataSets();
					final int colsize = adapter.getColumnSize();
					
					if (LOG.isDebugEnabled())
						LOG.debug("bindRowView: container=" + view + " row=" + position + " colsize=" + colsize);
					
					ContactItem[] items = new ContactItem[colsize];
					
					for (int i=0; i < colsize; i++) { 
						ContactItem item = dataSets.getContactItemAt(i + position);
	...
	private ContactAdapter(Context context, ContactDataSets data,
            int resource, int colsize) {
	...
	private ContactAdapter(AdapterImpl impl, int colsize) {
		super(impl);
	...
	public ContactDataSets getDataSets() { 
		return (ContactDataSets)getImpl().getDataSets();
	}
	
	public ContactDataSet getDataSet(int position) { 
		return (ContactDataSet)getImpl().getDataSet(position);
	}
	...
	private static class AdapterImpl extends AbstractAdapter<ContactItem> { 
		public AdapterImpl(Context context, ContactDataSets data, int resource) {
		...
		final void onActivityStop() { 
			for (int i=0; i < getCount(); i++) {
				ContactDataSet dataSet = (ContactDataSet)getDataSet(i);
				ContactItem item = dataSet != null ? dataSet.getContactItem() : null;
				if (item != null) 
