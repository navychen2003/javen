...
public class GroupAdapter extends ColumnAdapter implements IActivityListener {
	private static final Logger LOG = Logger.getLogger(GroupAdapter.class);
	
	static GroupAdapter createAdapter(final IActivity activity, 
			final GroupDataSets dataSets, final DataBinder.BinderCallback callback, 
			int resource, int colsize) { 
		if (activity == null || dataSets == null || callback == null) 
			return null;
		
		final GroupAdapter adapter = new GroupAdapter(activity.getActivity(), 
		...
		adapter.setViewBinder(new AdvancedAdapter.ViewBinder() {
				...
				public void onViewBinded(AdvancedAdapter.DataSet dataSet, View view, int position) {
					final GroupDataSets dataSets = adapter.getDataSets();
					final int colsize = adapter.getColumnSize();
					
					if (LOG.isDebugEnabled())
						LOG.debug("bindRowView: container=" + view + " row=" + position + " colsize=" + colsize);
					
					GroupItem[] items = new GroupItem[colsize];
					
					for (int i=0; i < colsize; i++) { 
						GroupItem item = dataSets.getGroupItemAt(i + position);
	...
	private GroupAdapter(Context context, GroupDataSets data,
            int resource, int colsize) {
	...
	private GroupAdapter(AdapterImpl impl, int colsize) {
		super(impl);
	...
	public GroupDataSets getDataSets() { 
		return (GroupDataSets)getImpl().getDataSets();
	}
	
	public GroupDataSet getDataSet(int position) { 
		return (GroupDataSet)getImpl().getDataSet(position);
	}
	...
	private static class AdapterImpl extends AbstractAdapter<GroupItem> { 
		public AdapterImpl(Context context, GroupDataSets data, int resource) {
		...
		final void onActivityStop() { 
			for (int i=0; i < getCount(); i++) {
				GroupDataSet dataSet = (GroupDataSet)getDataSet(i);
				GroupItem item = dataSet != null ? dataSet.getGroupItem() : null;
				if (item != null) 
