...
public class SpaceAdapter extends ColumnAdapter implements IActivityListener {
	private static final Logger LOG = Logger.getLogger(SpaceAdapter.class);
	
	static SpaceAdapter createAdapter(final IActivity activity, 
			final SpaceDataSets dataSets, final DataBinder.BinderCallback callback, 
			int resource, int colsize) { 
		if (activity == null || dataSets == null || callback == null) 
			return null;
		
		final SpaceAdapter adapter = new SpaceAdapter(activity.getActivity(), 
		...
		adapter.setViewBinder(new AdvancedAdapter.ViewBinder() {
				...
				public void onViewBinded(AdvancedAdapter.DataSet dataSet, View view, int position) {
					final SpaceDataSets dataSets = adapter.getDataSets();
					final int colsize = adapter.getColumnSize();
					
					if (LOG.isDebugEnabled())
						LOG.debug("bindRowView: container=" + view + " row=" + position + " colsize=" + colsize);
					
					SpaceItem[] items = new SpaceItem[colsize];
					
					for (int i=0; i < colsize; i++) { 
						SpaceItem item = dataSets.getSpaceItemAt(i + position);
	...
	private SpaceAdapter(Context context, SpaceDataSets data,
            int resource, int colsize) {
	...
	private SpaceAdapter(AdapterImpl impl, int colsize) {
		super(impl);
	...
	public SpaceDataSets getDataSets() { 
		return (SpaceDataSets)getImpl().getDataSets();
	}
	
	public SpaceDataSet getDataSet(int position) { 
		return (SpaceDataSet)getImpl().getDataSet(position);
	}
	...
	private static class AdapterImpl extends AbstractAdapter<SpaceItem> { 
		public AdapterImpl(Context context, SpaceDataSets data, int resource) {
		...
		final void onActivityStop() { 
			for (int i=0; i < getCount(); i++) {
				SpaceDataSet dataSet = (SpaceDataSet)getDataSet(i);
				SpaceItem item = dataSet != null ? dataSet.getSpaceItem() : null;
				if (item != null) 
