...
public class AccountListAdapter extends ColumnAdapter implements IActivityListener {
	private static final Logger LOG = Logger.getLogger(AccountListAdapter.class);
	
	static AccountListAdapter createAdapter(final IActivity activity, 
			final AccountListDataSets dataSets, final DataBinder.BinderCallback callback, 
			int resource, int colsize) { 
		if (activity == null || dataSets == null || callback == null) 
			return null;
		
		final AccountListAdapter adapter = new AccountListAdapter(activity.getActivity(), 
		...
		adapter.setViewBinder(new AdvancedAdapter.ViewBinder() {
				...
				public void onViewBinded(AdvancedAdapter.DataSet dataSet, View view, int position) {
					final AccountListDataSets dataSets = adapter.getDataSets();
					final int colsize = adapter.getColumnSize();
					
					if (LOG.isDebugEnabled())
						LOG.debug("bindRowView: container=" + view + " row=" + position + " colsize=" + colsize);
					
					AccountListItem[] items = new AccountListItem[colsize];
					
					for (int i=0; i < colsize; i++) { 
						AccountListItem item = dataSets.getAccountListItemAt(i + position);
	...
	public AccountListAdapter(Context context, AccountListDataSets data,
            int resource, int colsize) {
	...
	private AccountListAdapter(AdapterImpl impl, int colsize) {
		super(impl);
	...
	public AccountListDataSets getDataSets() { 
		return (AccountListDataSets)getImpl().getDataSets();
	}
	
	public AccountListDataSet getDataSet(int position) { 
		return (AccountListDataSet)getImpl().getDataSet(position);
	}
	...
	private static class AdapterImpl extends AbstractAdapter<AccountListItem> { 
		public AdapterImpl(Context context, AccountListDataSets data, int resource) {
		...
		final void onActivityStop() { 
			for (int i=0; i < getCount(); i++) {
				AccountListDataSet dataSet = (AccountListDataSet)getDataSet(i);
				AccountListItem item = dataSet != null ? dataSet.getAccountListItem() : null;
				if (item != null) 
