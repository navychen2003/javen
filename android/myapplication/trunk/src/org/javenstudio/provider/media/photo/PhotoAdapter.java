...
public class PhotoAdapter extends ColumnAdapter implements IActivityListener {
	private static final Logger LOG = Logger.getLogger(PhotoAdapter.class);
	
	static PhotoAdapter createAdapter(final IActivity activity, 
			final PhotoDataSets dataSets, final DataBinder.BinderCallback callback, 
			int resource, int colsize) { 
		if (activity == null || dataSets == null || callback == null) 
			return null;
		
		final PhotoAdapter adapter = new PhotoAdapter(activity.getActivity(), 
		...
		adapter.setViewBinder(new AdvancedAdapter.ViewBinder() {
				...
				public void onViewBinded(AdvancedAdapter.DataSet dataSet, View view, int position) {
					final PhotoDataSets dataSets = adapter.getDataSets();
					final int colsize = adapter.getColumnSize();
					
					if (LOG.isDebugEnabled())
						LOG.debug("bindRowView: container=" + view + " row=" + position + " colsize=" + colsize);
					
					PhotoItem[] items = new PhotoItem[colsize];
					
					for (int i=0; i < colsize; i++) { 
						PhotoItem item = dataSets.getPhotoItemAt(i + position);
	...
	private PhotoAdapter(Context context, PhotoDataSets data,
            int resource, int colsize) {
	...
	private PhotoAdapter(AdapterImpl impl, int colsize) {
		super(impl);
	...
	public PhotoDataSets getDataSets() { 
		return (PhotoDataSets)getImpl().getDataSets();
	}
	
	public PhotoDataSet getDataSet(int position) { 
		return (PhotoDataSet)getImpl().getDataSet(position);
	}
	...
	private static class AdapterImpl extends AbstractAdapter<PhotoItem> { 
		public AdapterImpl(Context context, PhotoDataSets data, int resource) {
		...
		final void onActivityStop() { 
			for (int i=0; i < getCount(); i++) {
				PhotoDataSet dataSet = (PhotoDataSet)getDataSet(i);
				PhotoItem item = dataSet != null ? dataSet.getPhotoItem() : null;
				if (item != null) 
