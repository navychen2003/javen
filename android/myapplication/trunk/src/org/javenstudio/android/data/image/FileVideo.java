...
import org.javenstudio.android.data.media.local.LocalVideo;
...
public class FileVideo extends Image {

	private final DataApp mApp;
	private final ImageBitmap mBitmap;
	private final FileVideoInfo mInfo;
	private final DataPath mPath;
	private final String mLocation;
	
	public FileVideo(DataApp app, DataPath path, String filepath) { 
		mApp = app;
		mLocation = filepath;
		mPath = path;
		mInfo = new FileVideoInfo(filepath);
	...
	public Job<BitmapRef> requestImage(BitmapHolder holder, int type) {
		return new LocalVideo.LocalVideoRequest(mApp, 
