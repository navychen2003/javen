...
import org.javenstudio.cocoka.opengl.EdgeView;
import org.javenstudio.cocoka.opengl.FlingScroller;
import org.javenstudio.cocoka.opengl.GLActivity;
import org.javenstudio.cocoka.opengl.GLCanvas;
import org.javenstudio.cocoka.opengl.GLRoot;
import org.javenstudio.cocoka.opengl.GLView;
import org.javenstudio.cocoka.opengl.GestureRecognizer;
import org.javenstudio.cocoka.opengl.PositionController;
...
import org.javenstudio.cocoka.opengl.Texture;
import org.javenstudio.cocoka.opengl.TileImageView;
...
import org.javenstudio.cocoka.util.RangeArray;
import org.javenstudio.cocoka.util.Utils;
import org.javenstudio.common.util.Logger;

public class PhotoView extends GLView {
    private static final Logger LOG = Logger.getLogger(PhotoView.class);
    
    private final int mPlaceholderColor;

    public static final int INVALID_SIZE = -1;
    public static final long INVALID_DATA_VERSION =
    		IMediaObject.INVALID_DATA_VERSION;
    ...
    public interface Model extends TileImageView.Model {
        ...
        public static final int LOADING_SCREENNAIL_FAIL = 2;
        public static final int LOADING_FULLIMAGE_FAIL = 3;
    ...
    public static final int SCREEN_NAIL_MAX = PositionController.SCREEN_NAIL_MAX; //3;
    ...
    public PhotoView(GLActivity activity, BitmapHolder holder, 
    		StringTexture noThumbnailText) {
    	mContext = activity.getActivityContext();
    	
        mTileView = new TileImageView(activity, holder);
        addComponent(mTileView);
        
        mPlaceholderColor = mContext.getResources().getColor(
                R.color.photo_placeholder);
        
        mEdgeView = new EdgeView(mContext, holder, 
        		BaseResources.getInstance().getDrawableRes(BaseResources.drawable.overscroll_edge), 
        		BaseResources.getInstance().getDrawableRes(BaseResources.drawable.overscroll_glow));
        addComponent(mEdgeView);
        
        mUndoBar = new UndoBarView(mContext, holder);
        ...
        mNoThumbnailText = noThumbnailText;
        //	StringTexture.newInstance(holder,
        ...
        mVideoPlayIcon = new ResourceTexture(mContext, holder, 
        		BaseResources.getInstance().getDrawableRes(
        				BaseResources.drawable.icon_control_play));
        ...
        public void handleMessage(Message message) {
            switch (message.what) {
                ...
                case MSG_DELETE_ANIMATION_DONE: {
                    ...
                    mListener.onDeleteImage((IMediaItem) message.obj, message.arg1);
                    ...
                    if (numberOfPictures == 2) {
                        if (mModel.isCamera(mNextBound) || mModel.isCamera(mPrevBound)) 
                            numberOfPictures--;
                    }
    ...
    private void updateCameraRect() {
        ...
        if (LOG.isDebugEnabled()) {
        	LOG.debug("compensation = " + mCompensation
    ...
    class FullPicture implements Picture {
        ...
        private void updateSize() {
            if (mIsCamera && !mIsStaticCamera) {
                mRotation = getCameraRotation();
            } else {
                mRotation = mModel.getImageRotation(0);
            }

            int w = mTileView.getImageWidth();
            int h = mTileView.getImageHeight();
        ...
        private void drawTileView(GLCanvas canvas, Rect r) {
            ...
            if (mLoadingState == Model.LOADING_SCREENNAIL_FAIL || 
            	mLoadingState == Model.LOADING_FULLIMAGE_FAIL) {
                drawLoadingFailMessage(canvas, mLoadingState);
            }

            // Draw a debug indicator showing which picture has focus (index == 0).
    ...
    private void drawLoadingFailMessage(GLCanvas canvas, int state) {
    	if (state == Model.LOADING_SCREENNAIL_FAIL) {
	        StringTexture m = mNoThumbnailText;
	        if (m != null) m.draw(canvas, -m.getWidth() / 2, -m.getHeight() / 2);
    	}
    }
    ...
    private class MyGestureListener implements GestureRecognizer.Listener {
        ...
        public boolean onSingleTapUp(float x, float y) {
            ...
            if (mFilmMode && !mDownInScrolling) {
                ...
                if ((supported & IMediaItem.SUPPORT_ACTION) == 0) {
                    setFilmMode(false);
                    mIgnoreUpEvent = true;
                    return true;
                }
        ...
        private boolean flingImages(float velocityX, float velocityY) {
            ...
            int maxVelocity = MediaUtils.dpToPixel(MAX_DISMISS_VELOCITY);
            int escapeVelocity = MediaUtils.dpToPixel(SWIPE_ESCAPE_VELOCITY);
        ...
        private void deleteAfterAnimation(int duration) {
           	IMediaItem item = mModel.getMediaItem(mTouchBoxIndex);
            if (item == null) return;
    ...
    public PhotoFallbackEffect buildFallbackEffect(GLView root, GLCanvas canvas) {
        ...
        for (int i = -SCREEN_NAIL_MAX; i <= SCREEN_NAIL_MAX; ++i) {
            IMediaItem item = mModel.getMediaItem(i);
            ...
            effect.addEntry(item, rect, texture);
        }
        return effect;
    }
