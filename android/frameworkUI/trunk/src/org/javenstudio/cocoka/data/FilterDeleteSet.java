...
//
public class FilterDeleteSet extends AbstractMediaSet implements ContentListener {
    ...
    private static class Request {
        ...
        IMediaItem item;
        int indexHint;
        public Request(int type, IMediaItem item, int indexHint) {
            this.type = type;
            this.item = item;
    ...
    private static class Deletion {
        IMediaItem item;
        int index;
        public Deletion(IMediaItem item, int index) {
            this.item = item;
            this.index = index;
        }
    }
    ...
    private final IMediaSet mBaseSet;

    // Pending Requests
    private List<Request> mRequests = new ArrayList<Request>();

    // Deletions currently in effect, ordered by index
    private List<Deletion> mCurrent = new ArrayList<Deletion>();
    
    private long mDataVersion = 0;

    public FilterDeleteSet(IMediaSet baseSet) {
        //super(path, -1 /*INVALID_DATA_VERSION*/);
        mBaseSet = baseSet;
    ...
    public int getItemCount() {
        return mBaseSet.getItemCount() - mCurrent.size();
    }
    
    @Override
    public int getIndexHint() {
    	return mBaseSet.getIndexHint();
    }
    
    @Override
    public void setIndexHint(int index) {
    	mBaseSet.setIndexHint(index);
    }
    ...
    public List<IMediaItem> getItemList(int start, int count) {
        if (count <= 0) return new ArrayList<IMediaItem>();
        ...
        List<IMediaItem> base = mBaseSet.getItemList(start + i, count + (j - i));
    ...
    public long reloadData() {
        boolean newData = mBaseSet.reloadData() > mDataVersion;
            ...
            for (int i = 0; i < mRequests.size(); i++) {
                ...
                switch (r.type) {
                    case REQUEST_ADD: {
                        ...
                        int j;
                        for (j = 0; j < n; j++) {
                            if (mCurrent.get(j).item == r.item) break;
                        }
                        if (j == n) {
                            mCurrent.add(new Deletion(r.item, r.indexHint));
                        }
                        break;
                    }
                    case REQUEST_REMOVE: {
    ...
    private void sendRequest(int type, IMediaItem item, int indexHint) {
        Request r = new Request(type, item, indexHint);
        synchronized (mRequests) {
            mRequests.add(r);
        }
    ...
    public void addDeletion(IMediaItem item, int indexHint) {
        sendRequest(REQUEST_ADD, item, indexHint);
    }

    public void removeDeletion(IMediaItem item) {
        sendRequest(REQUEST_REMOVE, item, 0 /* unused */);
    }
