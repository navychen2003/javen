...
public abstract class UploadedTexture extends BasicTexture {
    ...
    private static HashMap<BorderKey, BitmapRef> sBorderLines =
            new HashMap<BorderKey, BitmapRef>();
    private static BorderKey sBorderKey = new BorderKey();

    protected final BitmapHolder mHolder;
    ...
    protected BitmapRef mBitmap;
    private int mBorder;

    protected UploadedTexture(BitmapHolder holder) {
        this(holder, false);
    }

    protected UploadedTexture(BitmapHolder holder, boolean hasBorder) {
    ...
    private BitmapRef getBorderLine(
        ...
        BitmapRef bitmap = sBorderLines.get(key);
        if (bitmap == null || bitmap.isRecycled()) {
        	BitmapHolder holder = mHolder; //ResourceHelper.getBitmapHolder();
            bitmap = vertical
                    ? BitmapRef.createBitmap(holder, 1, length, config)
                    : BitmapRef.createBitmap(holder, length, 1, config);
    ...
    protected abstract BitmapRef onGetBitmap();
    protected abstract void onFreeBitmap(BitmapRef bitmap);
    ...
    public void updateContent(GLCanvas canvas) {
        if (!isLoaded()) {
            ...
            BitmapRef bitmap = getBitmap();
            int format = GLUtils.getInternalFormat(bitmap.get());
            int type = GLUtils.getType(bitmap.get());
            canvas.getGLInstance().glBindTexture(GL11.GL_TEXTURE_2D, mId);
            GLUtils.texSubImage2D(GL11.GL_TEXTURE_2D, 0, mBorder, mBorder,
                    bitmap.get(), format, type);
            freeBitmap();
            mContentValid = true;
        }
    }
    ...
    static final float[] sCropRect = new float[4];
        ...
        if (bitmap != null) {
            try {
                ...
                if (bWidth == texWidth && bHeight == texHeight) {
                    GLUtils.texImage2D(GL11.GL_TEXTURE_2D, 0, bitmap.get(), 0);
                    ...
                    GLUtils.texSubImage2D(GL11.GL_TEXTURE_2D, 0,
                            mBorder, mBorder, bitmap.get(), format, type);

                    if (mBorder > 0) {
                        // Left border
                        BitmapRef line = getBorderLine(true, config, texHeight);
                        GLUtils.texSubImage2D(GL11.GL_TEXTURE_2D, 0,
                                0, 0, line.get(), format, type);
                    ...
                    if (mBorder + bWidth < texWidth) {
                        BitmapRef line = getBorderLine(true, config, texHeight);
                        GLUtils.texSubImage2D(GL11.GL_TEXTURE_2D, 0,
                                mBorder + bWidth, 0, line.get(), format, type);
                    }

                    // Bottom border
                    if (mBorder + bHeight < texHeight) {
                        BitmapRef line = getBorderLine(false, config, texWidth);
                        GLUtils.texSubImage2D(GL11.GL_TEXTURE_2D, 0,
                                0, mBorder + bHeight, line.get(), format, type);
                    }
                }
            } finally {
                freeBitmap();
