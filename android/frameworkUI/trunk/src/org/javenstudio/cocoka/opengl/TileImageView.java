...
import org.javenstudio.util.LongSparseArray;

public class TileImageView extends GLView {
	private static final Logger LOG = Logger.getLogger(TileImageView.class);
    ...
    public TileImageView(GLActivityContext context, BitmapHolder holder) {
        //mThreadPool = context.getThreadPool();
    	mHolder = holder;
        mTileDecoder = JobSubmit.submit(new TileDecoder());
        if (TILE_SIZE == 0) {
            if (MediaUtils.isHighResolution(context.getActivityContext())) {
                TILE_SIZE = 510 ;
            } else {
                TILE_SIZE = 254;
            }
    ...
    private synchronized Tile obtainTile(int x, int y, int level) {
        ...
        return new Tile(mHolder, x, y, level);
    }
    ...
    private class Tile extends UploadedTexture {
        ...
        public Tile mNext;
        public BitmapRef mDecodedTile;
        public volatile int mTileState = STATE_ACTIVATED;

        public Tile(BitmapHolder holder, int x, int y, int level) {
        	super(holder);
            mX = x;
            mY = y;
            mTileLevel = level;
        ...
        boolean decode() {
            ...
            try {
                mDecodedTile = BitmapUtil.ensureGLCompatibleBitmap(mHolder, mModel.getTile(
                        mTileLevel, mX, mY, TILE_SIZE, TILE_BORDER, sTilePool));
            } catch (Throwable t) {
            	if (LOG.isDebugEnabled())
                	LOG.debug("fail to decode tile", t);
            }
    ...
    private class TileDecoder implements Job<Void> {
        private JobCancelListener mNotifier = new JobCancelListener() {
	            @Override
