...
import org.javenstudio.cocoka.util.RangeIntArray;
...
public class PositionController {
    ...
    private static final int BOX_MAX = SCREEN_NAIL_MAX;
    private static final int[] CENTER_OUT_INDEX = new int[2 * BOX_MAX + 1];

    private static final int IMAGE_GAP = 0;//GalleryUtils.dpToPixel(16);
    private static final int HORIZONTAL_SLACK = 0;//GalleryUtils.dpToPixel(12);
    ...
    public PositionController(Context context, 
    		FlingScroller scroller, Listener listener) {
        mListener = listener;
        mPageScroller = scroller != null ? scroller : new FlingScroller();
        try {
    ...
    public void forceImageSize(int index, GLView.Size s) {
    ...
    public void setImageSize(int index, GLView.Size s, Rect cFrame) {
        if (s.width == 0 || s.height == 0) return;
    ...
    private void dumpState() {
        ...
        for (int i = -BOX_MAX; i < BOX_MAX; i++) {
        	if (LOG.isDebugEnabled())
            	LOG.debug("Gap " + i + ": " + mGaps.get(i).mCurrentGap);
        }
        ...
        for (int i = -BOX_MAX; i <= BOX_MAX; i++) {
            for (int j = i + 1; j <= BOX_MAX; j++) {
                if (Rect.intersects(mRects.get(i), mRects.get(j))) {
                	if (LOG.isDebugEnabled())
                    	LOG.debug("rect " + i + " and rect " + j + "intersects!");
                }
            }
        }
    }
    ...
    private void initBox(int index, GLView.Size size) {
    ...
    public void moveBox(int fromIndex[], boolean hasPrev, boolean hasNext,
            boolean constrained, GLView.Size[] sizes) {
