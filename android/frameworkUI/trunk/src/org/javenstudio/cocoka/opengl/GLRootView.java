...
import org.javenstudio.cocoka.util.MotionEventHelper;
import org.javenstudio.cocoka.util.Profile;
...
public class GLRootView extends GLSurfaceView
    ...
    private static final Logger LOG = Logger.getLogger(GLRootView.class);
    ...
    private final EGLConfigChooserImpl mEglConfigChooser =
            new EGLConfigChooserImpl();
    ...
    public void requestRender() {
        if (DEBUG_INVALIDATE && LOG.isDebugEnabled()) {
            StackTraceElement e = Thread.currentThread().getStackTrace()[4];
            String caller = e.getFileName() + ":" + e.getLineNumber() + " ";
            LOG.debug("invalidate: " + caller);
        }
    ...
    private void layoutContentPane() {
        ...
        if (LOG.isDebugEnabled()) {
        	LOG.debug("layout content pane " + w + "x" + h
    ...
    public void onSurfaceCreated(GL10 gl1, EGLConfig config) {
        ...
        if (mGL != null) {
        	...
        	if (LOG.isDebugEnabled())
            	LOG.debug("GLObject has changed from " + mGL + " to " + gl);
        }
    ...
    private void outputFps() {
        ...
        if (mFrameCountingStart == 0) {
        	...
        	if (LOG.isDebugEnabled()) {
            	LOG.debug("fps: " + (double) mFrameCount * 
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        if (DEBUG_PROFILE_SLOW_ONLY) {
            ...
            if (durationInMs > 34) {  // 34ms -> we skipped at least 2 frames
            	if (LOG.isDebugEnabled()) {
                	LOG.debug("----- SLOW (" + durationDrawInMs + "/" +
                        durationInMs + ") -----");
            	}
    ...
    private void onDrawFrameLocked(GL10 gl) {
        ...
        if ((mOrientationSource != null && mDisplayRotation != 
        		mOrientationSource.getDisplayRotation()) || 
    ...
    public void onPause() {
        ...
        if (DEBUG_PROFILE) {
        	if (LOG.isDebugEnabled())
            	LOG.debug("Stop profiling");
        	
            Profile.disableAll();
            Profile.dumpToFile("/sdcard/glrootview.prof");
            Profile.reset();
        }
    }
