...
public class MediaUtils {
	private static final Logger LOG = Logger.getLogger(MediaUtils.class);
    ...
    public static final String TOP_SET_PATH = ApiHelper.HAS_MTP
            ? "/combo/{/mtp,/local/all,/picasa/all}"
            : "/combo/{/local/all,/picasa/all}";

    public static final String TOP_IMAGE_SET_PATH = ApiHelper.HAS_MTP
            ? "/combo/{/mtp,/local/image,/picasa/image}"
            : "/combo/{/local/image,/picasa/image}";

    public static final String TOP_VIDEO_SET_PATH =
            "/combo/{/local/video,/picasa/video}";

    public static final String TOP_LOCAL_SET_PATH = "/local/all";
    public static final String TOP_LOCAL_IMAGE_SET_PATH = "/local/image";
    public static final String TOP_LOCAL_VIDEO_SET_PATH = "/local/video";
	
    public static final Comparator<IMediaItem> sDateTakenComparator =
            new DateTakenComparator();

    private static class DateTakenComparator implements Comparator<IMediaItem> {
	        @Override
	        public int compare(IMediaItem item1, IMediaItem item2) {
	            return -Utils.compare(item1.getDateInMs(), item2.getDateInMs());
	        }
	    }
    
	public static String getTopSetPath(int typeBits) {
        switch (typeBits) {
	...
	public static String[] getTopSetPaths() { 
    	return new String[] { 
    			TOP_IMAGE_SET_PATH, 
    			TOP_VIDEO_SET_PATH, 
    			TOP_SET_PATH, 
    			TOP_LOCAL_IMAGE_SET_PATH, 
    			TOP_LOCAL_VIDEO_SET_PATH, 
    			TOP_LOCAL_SET_PATH
    		};
    }
    ...
    public static int determineTypeBits(Context context, Intent intent) {
        ...
        if (MediaUtils.MIME_TYPE_ALL.equals(type)) {
            typeBits = MediaUtils.INCLUDE_ALL;
        } else if (MediaUtils.MIME_TYPE_IMAGE.equals(type) ||
        		MediaUtils.DIR_TYPE_IMAGE.equals(type)) {
            typeBits = MediaUtils.INCLUDE_IMAGE;
        } else if (MediaUtils.MIME_TYPE_VIDEO.equals(type) ||
        		MediaUtils.DIR_TYPE_VIDEO.equals(type)) {
            typeBits = MediaUtils.INCLUDE_VIDEO;
        } else {
            typeBits = MediaUtils.INCLUDE_ALL;
        }
        ...
        if (ApiHelper.HAS_INTENT_EXTRA_LOCAL_ONLY) {
            if (intent.getBooleanExtra(Intent.EXTRA_LOCAL_ONLY, false)) {
                typeBits |= MediaUtils.INCLUDE_LOCAL_ONLY;
            }
        }
    ...
    private static void initializeThumbnailSizes(DisplayMetrics metrics, Resources r) {
        ...
        BitmapHelper.setThumbnailSizes(maxPixels / 2, maxPixels / 5);
        ...
        if (!sWarned) {
            if (Thread.currentThread() == sCurrentThread) {
                ...
                if (LOG.isWarnEnabled())
                	LOG.warn("Should not do this in render thread");
            }
        }
    }
    ...
    public static boolean isValidLocation(double latitude, double longitude) {
        ...
        return (latitude != INVALID_LATLNG || longitude != INVALID_LATLNG);
    }
    ...
    public static void showOnMap(Context context, double latitude, double longitude) {
        try {
        	...
        	if (LOG.isErrorEnabled())
        		LOG.error("GMM activity not found!", e);
    ...
    //    if ((typeBits & MediaSource.INCLUDE_VIDEO) != 0) {
    //        return (typeBits & MediaSource.INCLUDE_IMAGE) == 0
    ...
    public static boolean hasSpaceForSize(long size) {
        ...
        try {
        	...
        	if (LOG.isWarnEnabled())
            	LOG.warn("Fail to access external storage", e);
        }
