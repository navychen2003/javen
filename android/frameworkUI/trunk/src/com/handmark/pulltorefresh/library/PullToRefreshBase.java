...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	private static final Logger LOG = Logger.getLogger(PullToRefreshBase.class);
	...
	public void addView(View child, int index, ViewGroup.LayoutParams params) {
		if (DEBUG && LOG.isDebugEnabled()) {
			LOG.debug("addView: " + child.getClass().getSimpleName());
		}
    ...
    public final boolean dispatchTouchEvent(MotionEvent ev) {
		OnRefreshListener<T> listener = mOnRefreshListener;
	...
	public final void onRefreshComplete() {
		if (DEBUG && LOG.isDebugEnabled()) {
			LOG.debug("onRefreshComplete: isRefreshing=" + isRefreshing());
		}
		if (isRefreshing()) {
	...
	public void setTextColor(int color) {
		getLoadingLayoutProxy().setTextColor(color);
	}
	...
	public void setSubTextColor(int color) {
		getLoadingLayoutProxy().setSubTextColor(color);
	}
	...
	public final void setMode(Mode mode) {
		if (mode != mMode) {
			if (DEBUG && LOG.isDebugEnabled()) {
				LOG.debug("setMode: setting mode to: " + mode);
			}
			mMode = mode;
			updateUIForMode();
		}
	...
	public void setPullToRefreshEnabled(boolean enable) {
		if (DEBUG && LOG.isDebugEnabled()) {
			LOG.debug("setPullToRefreshEnabled: enable=" + enable);
		}
	...
	public final void setPullToRefreshOverScrollEnabled(boolean enabled) {
		if (DEBUG && LOG.isDebugEnabled()) {
			LOG.debug("setPullToRefreshOverScrollEnabled: enabled=" + enabled);
		}
	...
	@Override
	public final void setRefreshing(boolean... params) {
		if (DEBUG && LOG.isDebugEnabled()) {
			LOG.debug("setRefreshing: doScroll=" + params + " isRefreshing=" + isRefreshing());
		}
		if (!isRefreshing()) {
			setState(State.MANUAL_REFRESHING, params);
		}
	}
	...
	final void setState(State state, final boolean... params) {
		mState = state;
		if (DEBUG && LOG.isDebugEnabled()) {
			LOG.debug("setState: state=" + mState.name());
		}
	...
	protected LoadingLayout createLoadingLayout(Context context, Mode mode, TypedArray attrs) {
		LoadingLayout layout = mLoadingAnimationStyle.createLoadingLayout(context, mode,
				...
				getDefaultLoadingDrawableRes(mLoadingAnimationStyle == AnimationStyle.FLIP),
				this);
		layout.setVisibility(View.INVISIBLE);
		return layout;
	}

	protected int getDefaultLoadingDrawableRes(boolean flip) { 
		return RotateLoadingLayout.getDefaultLoadingDrawableRes(flip);
	}
	
	protected LoadingLayout newRotateLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs, int defaultDrawableRes) {
		return new RotateLoadingLayout(context, mode, scrollDirection, attrs, defaultDrawableRes);
	}
	
	protected LoadingLayout newFlipLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs, int defaultDrawableRes) {
		return new FlipLoadingLayout(context, mode, scrollDirection, attrs, defaultDrawableRes);
	}
	...
	protected void onRefreshing(final boolean... params) {
		...
		final boolean doScroll = (params != null && params.length > 0) ? 
				params[0] : false;
	...
	protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
		if (DEBUG && LOG.isDebugEnabled()) {
			LOG.debug(String.format("onSizeChanged: W: %d, H: %d", w, h));
		}
	...
	protected final void refreshLoadingViewsSize() {
		...
		if (DEBUG && LOG.isDebugEnabled()) {
			LOG.debug(String.format("refreshLoadingViewsSize: setting padding. L: %d, T: %d, R: %d, B: %d", pLeft, pTop, pRight, pBottom));
		}
	...
	protected final void setHeaderScroll(int value) {
		if (DEBUG && LOG.isDebugEnabled()) {
			LOG.debug("setHeaderScroll: value=" + value);
		}
	...
	private void callRefreshListener(boolean... params) {
		if (null != mOnRefreshListener) {
			mOnRefreshListener.onRefresh(this, params);
		} else if (null != mOnRefreshListener2) {
			if (mCurrentMode == Mode.PULL_FROM_START) {
				mOnRefreshListener2.onPullDownToRefresh(this, params);
			} else if (mCurrentMode == Mode.PULL_FROM_END) {
				mOnRefreshListener2.onPullUpToRefresh(this, params);
			}
		}
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (a.hasValue(R.styleable.PullToRefresh_ptrRefreshableViewBackground)) {
			...
			PTRUtils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
	...
	public static enum AnimationStyle {
		...
		LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs, int defaultDrawableRes, PullToRefreshBase<?> ptr) {
			switch (this) {
				case ROTATE:
				default:
					return ptr.newRotateLoadingLayout(context, mode, scrollDirection, attrs, defaultDrawableRes);
				case FLIP:
					return ptr.newFlipLoadingLayout(context, mode, scrollDirection, attrs, defaultDrawableRes);
			}
		}
	}
	...
	public static interface OnRefreshListener<V extends View> {
		 ...
		 * end
		 */
		public void onRefresh(PullToRefreshBase<V> refreshView, boolean... params);
		
		public void onDispatchTouchEvent(PullToRefreshBase<V> refreshView, MotionEvent event);
	...
	public static interface OnRefreshListener2<V extends View> {
		 ...
		 */
		public void onPullDownToRefresh(final PullToRefreshBase<V> refreshView, final boolean... params);

		/**
		 * onPullUpToRefresh will be called only when the user has Pulled from
		 * the end, and released.
		 */
		public void onPullUpToRefresh(final PullToRefreshBase<V> refreshView, final boolean... params);
