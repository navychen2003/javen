...
import com.slidingmenu.lib.CustomViewContent.OnPageChangeListener;
...
public class SlidingMenu extends RelativeLayout {
	...
	public SlidingMenu(Context context, AttributeSet attrs, int defStyle) {
		...
		mViewContent.setOnPageChangeListener(new OnPageChangeListener() {
		...
		int viewAbove = ta.getResourceId(R.styleable.SlidingMenu_viewContent, -1);
		if (viewAbove != -1) {
			setContent(viewAbove);
		} else {
			setContent(new FrameLayout(context));
		}
		int viewBehind = ta.getResourceId(R.styleable.SlidingMenu_viewMenu, -1);
		if (viewBehind != -1) {
			setMenu(viewBehind); 
		} else {
		...
		int touchModeAbove = ta.getInt(R.styleable.SlidingMenu_touchModeContent, TOUCHMODE_MARGIN);
		setTouchModeContent(touchModeAbove);
		int touchModeBehind = ta.getInt(R.styleable.SlidingMenu_touchModeMenu, TOUCHMODE_MARGIN);
		setTouchModeMenu(touchModeBehind);

		int offsetBehind = (int) ta.getDimension(R.styleable.SlidingMenu_menuOffset, -1);
		int widthBehind = (int) ta.getDimension(R.styleable.SlidingMenu_menuWidth, -1);
		if (offsetBehind != -1 && widthBehind != -1)
			throw new IllegalStateException("Cannot set both behindOffset and behindWidth for a SlidingMenu");
		else if (offsetBehind != -1)
			setMenuOffset(offsetBehind);
		else if (widthBehind != -1)
			setMenuWidth(widthBehind);
		else
			setMenuOffset(0);
	...
	public void setSecondaryMenu(View v) {
		mViewMenu.setSecondaryContent(v);
