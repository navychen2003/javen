...
public class CustomViewContent extends ViewGroup {
	private static final String TAG = "CustomViewContent";
	...
	public int getBehindWidth() {
		if (mViewMenu == null) {
			...
			return mViewMenu.getBehindWidth();
		}
	}
	...
	private boolean thisSlideAllowed(float dx) {
		...
		if (isMenuOpen()) {
			allowed = mViewMenu.menuOpenSlideAllowed(dx);
		} else {
			allowed = mViewMenu.menuClosedSlideAllowed(dx);
		}
		if (DEBUG)
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (thisTouchAllowed(ev)) {
				...
				if (isMenuOpen() && mViewMenu.menuTouchInQuickReturn(mContent, mCurItem, ev.getX() + mScrollX)) {
					mQuickReturn = true;
				}
			} else {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			if (mIsBeingDragged) {
				...
				mActivePointerId = INVALID_POINTER;
				endDrag();
			} else if (mQuickReturn && mViewMenu.menuTouchInQuickReturn(mContent, mCurItem, ev.getX() + mScrollX)) {
				// close the menu
				setCurrentItem(1);
				endDrag();
			}
	...
	public void scrollTo(int x, int y) {
		if (DEBUG) Log.v(TAG, "scrollTo: x=" + x + " y=" + y);
		super.scrollTo(x, y);
		mScrollX = x;
		if (mEnabled) mViewMenu.scrollBehindTo(mContent, x, y);	
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		mViewMenu.drawShadow(mContent, canvas);
		mViewMenu.drawFade(mContent, canvas, getPercentOpen());
		mViewMenu.drawSelector(mContent, canvas, getPercentOpen());
	}
