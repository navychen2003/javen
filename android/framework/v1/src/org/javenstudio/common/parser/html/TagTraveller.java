...
public class TagTraveller implements TagTree.Callbacks {
	...
	TagTree newBeginTree(String rootTag, String[] attrNames, String[] attrValues, Tag.AttributesMatcher matcher) { 
		synchronized (this) { 
			if (!mInited) { 
	...
	TagTree newEndTree(String rootTag, String[] attrNames, String[] attrValues, Tag.AttributesMatcher matcher) { 
		synchronized (this) { 
			if (!mInited) { 
	...
	TagTree newContentTree(String rootTag, String[] attrNames, String[] attrValues, Tag.AttributesMatcher matcher) { 
		synchronized (this) { 
			if (!mInited) { 
	...
	public synchronized void init() throws ParseException { 
		if (mInited) return; 
	...
	public synchronized void startTag(String name, Tag.IAttributes attributes) { 
		if (!mBeginFound) { 
			TagTree tree = mBeginTree; 
			if (tree != null)
	...
	public synchronized void endTag(String name) { 
		if (!mBeginFound) { 
			TagTree tree = mBeginTree; 
			if (tree != null)
				tree.onEndTag(name); 
