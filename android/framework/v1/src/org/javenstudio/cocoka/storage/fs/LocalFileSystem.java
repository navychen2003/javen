...
public class LocalFileSystem implements IFileSystem {
	...
	protected boolean mkdirFile(File dir) throws IOException { 
		...
		LOG.info("LocalFileSystem: mkdir: " + dir.getAbsolutePath() + " " + result);
	...
	public OutputStream create(IFile file) throws IOException {
		File f = getFileImpl(file); 
		
		IFile parent = file.getParentFile(); 
		if (parent != null && !exists(parent) && !mkdirs(parent)) 
			throw new IOException("directory: "+parent.getAbsolutePath()+" cannot created"); 
