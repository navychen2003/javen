...
public class SettingGroup extends Setting implements GenericInflater.Parent<Setting> {
    ...
    public void setInitializer(GroupInitializer initializer) { 
    	mInitializer = initializer; 
    	mInitialized = false; 
    }
    
    public GroupInitializer getInitializer() { 
    	return mInitializer; 
    }
    	...
    	if (!mInitialized) { 
    		...
    		if (initializer != null) { 
    			if (Constants.isDebug()) 
    				Log.d(Constants.getTag(), "SettingGroup: initialize "+getClass().getName()+" with key: "+getKey());
    ...
    public boolean addSetting(Setting setting) {
        ...
        if (containsSetting(setting)) {
            Log.w(Constants.getTag(), "setting: "+setting.getKey()+" already existed");
            return true;
        }
    ...
    protected boolean removeSettingInternal(Setting setting) {
        ...
        synchronized(this) {
            ...
            if (key != null) 
            	getSettingMap().remove(key);
            
            return getSettingList().remove(setting);
        }
    }
