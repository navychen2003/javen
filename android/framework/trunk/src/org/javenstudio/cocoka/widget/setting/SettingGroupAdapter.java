...
public class SettingGroupAdapter extends BaseAdapter 
		implements Setting.OnSettingChangeInternalListener {
	private static final Logger LOG = Logger.getLogger(SettingGroupAdapter.class);
    ...
    public SettingGroupAdapter(SettingGroup settingGroup) {
    	if (LOG.isDebugEnabled()) 
    		LOG.debug("SettingGroupAdapter: create, group=" + settingGroup);
    ...
    public final void syncMySettings() {
    	if (LOG.isDebugEnabled()) LOG.debug("syncMySettings: isSyncing=" + mIsSyncing);
        synchronized(this) {
    ...
    private void flattenSettingGroup(List<Setting> settings, SettingGroup group) {
    	//if (LOG.isDebugEnabled()) LOG.debug("flattenSettingGroup: group=" + group);
        ...
        for (int i = 0; i < groupSize; i++) {
            ...
            //if (LOG.isDebugEnabled())
            //	LOG.debug("flattenSettingGroup: setting=" + setting);
            
            if (layoutRes != 0) { 
            	if (!setting.isHidden()) {
            		if (LOG.isDebugEnabled())
                    	LOG.debug("flattenSettingGroup: add setting: " + setting);
    ...
    public void onSettingHierarchyChange(Setting setting) {
    	if (LOG.isDebugEnabled()) 
    		LOG.debug("onSettingHierarchyChange: setting=" + setting);
