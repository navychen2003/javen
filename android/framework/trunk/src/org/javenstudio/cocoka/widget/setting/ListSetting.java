...
public class ListSetting extends SettingScreen {
    ...
    public ListSetting(SettingManager manager, AttributeSet attrs) {
        super(manager, attrs); 
        super.addSetting(mCategory = createCategory(), false);
    }
    ...
    @Override
    public final boolean addSetting(Setting setting, boolean replace) { 
    	return false; //addItem(setting);
    }
    
    protected boolean addItem(ListItemSetting setting) { 
    	return getCategory().addSetting(setting, false); 
    }
    
    protected ListItemSetting createItem(int index, CharSequence entry, 
    		CharSequence entryValue, CharSequence entrySummary) { 
    	ListItemSetting item = new ListItemSetting(this); 
    	item.setKey(getKey() + ".entry_" + index); 
    	item.setEntry(entry); 
    	item.setEntryValue(entryValue); 
    	item.setSummary(entrySummary);
    	
    	return item;
    }
    
    public final int setEntries(int entriesRes, int entryValuesRes) { 
    	return setEntries(entriesRes, entryValuesRes, 0);
    }
    
    public final int setEntries(int entriesRes, int entryValuesRes, int entrySummaryRes) { 
    	...
    	CharSequence[] entrySummaries = entrySummaryRes != 0 ? 
    			getResourceContext().getStringArray(entrySummaryRes) : null; 
    	return setEntries(entries, entryValues, entrySummaries); 
    }
    
    public final int setEntries(CharSequence[] entries, CharSequence[] entryValues) { 
    	return setEntries(entries, entryValues, null); 
    }
    
    public final int setEntries(CharSequence[] entries, CharSequence[] entryValues, CharSequence[] entrySummaries) { 
    	int count = 0;
    	if (entries != null && entryValues != null && entries.length == entryValues.length) { 
    		if (entrySummaries != null && entrySummaries.length != entries.length)
    			return count;
    		...
    		for (int i=0; i < entries.length && i < entryValues.length; i++) { 
    			...
    			CharSequence entrySummary = entrySummaries != null ? entrySummaries[i] : null; 
    			
    			if (addItem(createItem(i, entry, entryValue, entrySummary))) 
    				count ++;
    		}
    	}
    	return count;
    ...
    public final void setCheckedValueResource(int valueRes) { 
    ...
    public final void setCheckedValuesResource(int valuesRes) { 
