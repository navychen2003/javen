...
import org.javenstudio.cocoka.android.ResourceContextHelper;
...
public abstract class Implements {
	...
	public static final void startApp(Context fromActivity, String className, Intent intent) { 
	...
	private void doInit() { 
		...
		mResourceContext = ResourceContextHelper.createResourceContext(mHelper, 
		...
		SocketFactoryCreator.Helper.setSocketFactoryCreator(mHelper);
		...
		mModuleManager.onInitialized(mContext); 
		ResourceContextHelper.onInitialized(mResourceContext); 
	...
	private void initFileSystems(final Context context) {
		...
		FetchHelper.initFetchManager(StorageManager.getInstance(), mScheduler);
	}
	...
	public BitmapHolder getGlobalBitmapHolder() { 
		return mHelper;
	}
	
	public int getResourceWithId(int id) { return 0; }
	
	protected void setAppMessage(String className, String message) { 
		ImplementHelper.setAppMessage(getModuleApps(), className, message);
	}
	
	protected void startAppActivity(Context fromActivity, String className, Intent intent) { 
		ImplementHelper.startActivity(fromActivity, className, intent); 
	}
	...
	protected void onInitHttpParams(HttpParams params) { 
		SimpleHttpClient.initHttpParamsDefault(params);
	}
