...
public class BlobCache implements Closeable {
    ...
    public void syncAll() {
        ...
        try {
        	...
        	if (LOG.isWarnEnabled())
            	LOG.warn("sync data file 0 failed", t);
        }
        try {
            mDataFile1.getFD().sync();
        } catch (Throwable t) {
        	if (LOG.isWarnEnabled())
            	LOG.warn("sync data file 1 failed", t);
        }
    }
