...
public abstract class DBOpenHelper {
    private static final Logger LOG = Logger.getLogger(DBOpenHelper.class);
    ...
    public synchronized Database getReadableDatabase() {
        ...
        try {
        	...
        	if (LOG.isErrorEnabled())
            	LOG.error("Couldn't open " + mName + " for writing (will try read-only):", e);
        }
        ...
        try {
            ...
            if (db.getVersion() != mNewVersion) {
            	if (LOG.isErrorEnabled()) {
                	LOG.error("Can't upgrade read-only database from version " 
            ...
            if (LOG.isWarnEnabled())
            	LOG.warn("Opened " + mName + " in read-only mode");
