...
var compute_plugin_data = function( response, changeset )
{
  ...
  for( var i = 0; i < response['info-mbeans'].length; i++ )
  {
    if( !( i % 2 ) )
    {
      plugin_key = response['info-mbeans'][i];
    }
    else
    {
      plugin_data[plugin_key] = response['info-mbeans'][i];
    }
  }
...
(
  ...
  function( event, params )
  {
    ...
    (
      {
        url : core_basepath + '/admin/mbeans?stats=true&wt=json&token=' + app_token(),
        dataType : 'json',
