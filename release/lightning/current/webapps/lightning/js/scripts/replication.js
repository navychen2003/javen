        ...
        if( is_slave )
        {
          ...
          else
          {
            ...
            if( 'false' === data.slave.isPollingDisabled && data.slave.pollInterval )
            {
              ...
              {
                var nextExecutionAt = new LightningDate( data.slave.nextExecutionAt );
                var currentDate = new LightningDate( data.slave.currentDate );
          ...
          (
            ...
            function( event )
            {
              ...
              else if( command )
              {
                $.get
                (
                  core_basepath + '/replication?command=' + command + '&wt=json&token=' + app_token(),
                  function()
                  {
                    replication_fetch_status();
                  }
