...
(
  ...
  function( event, params )
  {
    ...
    (
      {
        url : core_basepath + '/admin/mbeans?cat=QUERYHANDLER&wt=json&token=' + app_token(),
        dataType : 'json',
        ...
        {
          var handlers = response['info-mbeans'][1];
          var import_handlers = [];
          for( var key in handlers )
          {
            if( handlers[key]['class'] !== key &&
              handlers[key]['class'] === 'org.javenstudio.lightning.handler.DataImportHandler' )
            {
              import_handlers.push( key );
            }
          }
          params.callback( import_handlers );
        },
  ...
  function( context )
  {
    ...
    (
      'dataimport_queryhandler_load',
      {
        ...
        callback :  function( import_handlers )
        {
          if( 0 === import_handlers.length )
          {
            $( '#content' )
          ...
          dataimport_handlers = import_handlers;
...
(
  ...
  function( context )
  {
    ...
    if (path_parts && path_parts.length > 2) { 
      var path_part = path_parts[2];
      if (path_part && path_part.length > 0)
        handler_name = path_part;
    }
    
    if (handler_name == null && dataimport_handlers) { 
      if (dataimport_handlers.length > 0)
        handler_name = dataimport_handlers[0];
    }
    
    if (handler_name == null || handler_name.length == 0)
      handler_name = '/dataimport';
    
    var handler_url = core_basepath + handler_name;
    ...
    (
      ...
      function( template )
      {
        ...
        abort_import_element
          ...
          (
            ...
            function( event )
            {
              ...
              (
                {
                  url : handler_url + '?command=abort&wt=json&token=' + app_token(),
                  dataType : 'json',
                  type: 'POST',
                  context: $( this ),
                  beforeSend : function( xhr, settings )
        ...
        function dataimport_fetch_status( clear_timeout )
        {
          ...
          (
            {
              url : handler_url + '?command=status&indent=true&wt=json&token=' + app_token(),
              dataType : 'json',
              beforeSend : function( xhr, settings )
              {
                $( 'span', status_button )
