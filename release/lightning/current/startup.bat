...
set START_OPTS=-jar %LIGHTNINGAPP_HOME%\start.jar %LIGHTNINGAPP_HOME%\etc\jetty.xml

set JAVA_OPTS=
set JAVA_OPTS=%JAVA_OPTS% -Dlightning.website.uri=http://localhost:%LIGHTNING_HTTPPORT%
set JAVA_OPTS=%JAVA_OPTS% -Dlightning.stopcommand.file=%LIGHTNINGAPP_HOME%/stop.bat
...
REM @start http://localhost:%LIGHTNING_HTTPPORT%
REM msg %username% "Please visit \"http://localhost:%LIGHTNING_HTTPPORT%\", or run \"stop.bat\" to shut down lightning service."
