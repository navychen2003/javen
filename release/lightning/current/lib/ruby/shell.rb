...
module Shell
  ...
  def self.load_command(name, group)
    ...
    begin
      ...
      raise "Can't load bigdb shell command: #{name}. Error: #{e}\n#{e.backtrace.join("\n")}"
    end
  end
  ...
  class Shell
    ...
    def initialize(bigdb, formatter)
      self.bigdb = bigdb
      self.formatter = formatter
    end

    def bigdb_admin
      @bigdb_admin ||= bigdb.admin(formatter)
    end

    def bigdb_table(name)
      bigdb.table(name, self)
    end

    def bigdb_replication_admin
      @bigdb_replication_admin ||= bigdb.replication_admin(formatter)
    end

    def bigdb_security_admin
      @bigdb_security_admin ||= bigdb.security_admin(formatter)
    end
    ...
    def print_banner
      puts "BigDB Shell; enter 'help<RETURN>' for list of supported commands."
      puts 'Type "exit<RETURN>" to leave the BigDB Shell'
      print 'Version '
      command('version')
      puts
    end
    ...
    def help_header
      return "BigDB Shell, version #{org.javenstudio.lightning.Constants.getVersion()}, " +
    ...
    def help_footer
      puts <<-HERE
Quote all names in BigDB Shell such as table and column names.  Commas delimit
...
double-quote'd hexadecimal representation. For example:
  ...
  bigdb> get 't1', "key\\x03\\x3f\\xcd"
  bigdb> get 't1', "key\\003\\023\\011"
  bigdb> put 't1', "test\\xef\\xff", 'f1:', "\\x01\\x33\\x40"

The BigDB shell is the (J)Ruby IRB with the above BigDB-specific commands added.
For more on the BigDB Shell, see http://bigdb.javenstudio.org/docs/current/book.html
      HERE
    end
  end
end
...
Shell.load_command_group(
  'general',
  :full_name => 'GENERAL BIGDB SHELL COMMANDS',
  :commands => %w[
    status
    version
    table_help
...
Shell.load_command_group(
  ...
  :comment => "In order to use these tools, bigdb.replication must be true.",
  :commands => %w[
    add_peer
    remove_peer
    list_peers
