...
java_import org.javenstudio.raptor.bigdb.util.Pair
#java_import org.javenstudio.raptor.bigdb.util.RegionSplitter
java_import org.javenstudio.raptor.bigdb.util.Bytes
...
module Bigdb
  class Admin
    ...
    #----------------------------------------------------------------------------------------------
    # Returns a list of tables in bigdb
    def list(regex = ".*")
    ...
    def paxosdump
      @admin.getConnection().getPaxosWrapper().dump()
    end
    ...
    def create(table_name, *args)
      ...
      htd = org.javenstudio.raptor.bigdb.DBTableDescriptor.new(table_name) #org.javenstudio.raptor.bigdb.TableName.valueOf(table_name))
      splits = nil
      ...
      args.each do |arg|
        ...
        if arg.has_key?(SPLITS) 
          ...
          arg.delete(SPLITS).each do |split|
            splits[idx] = org.javenstudio.raptor.bigdb.util.Bytes.toBytesBinary(split)
            idx = idx + 1
          end
          ...
          split_algo = 0; #RegionSplitter.newSplitAlgoInstance(@conf, arg.delete(SPLITALGO))
        ...
        htd.setDurability(org.javenstudio.raptor.bigdb.client.Durability.valueOf(arg.delete(DURABILITY))) if arg[DURABILITY]
    ...
    #----------------------------------------------------------------------------------------------
    # Closes a region.
    # If server name is nil, we presume region_name is full region name (DBRegionInfo.getRegionName).
    # If server name is not nil, we presume it is the region's encoded name (DBRegionInfo.getEncodedName)
    ...
    def truncate(table_name, conf = @conf)
      h_table = org.javenstudio.raptor.bigdb.client.DBTable.new(conf, table_name)
    ...
    def truncate_preserve(table_name, conf = @conf)
      ...
      splits = org.javenstudio.raptor.bigdb.util.Bytes.toByteArrays(splits)
      table_description = h_table.getTableDescriptor()
      yield 'Disabling table...' if block_given?
      disable(table_name)
    ...
    def alter_status(table_name)
      ...
      status = Pair.new()
      begin
        #status = @admin.getAlterStatus(org.javenstudio.raptor.bigdb.TableName.valueOf(table_name))
          ...
          arg.each do |key, value|
            ...
            if (k =~ /coprocessor/i)
              ...
              htd.getValues().each do |k1, v1|
                attrName = org.javenstudio.raptor.bigdb.util.Bytes.toString(k1.get())
    ...
    def status(format)
      ...
      if format == "detailed"
        puts("version %s" % [ status.getBigDBVersion() ])
        ...
        puts("%d live servers" % [ status.getLiveServerCount() ])
        ...
        puts("%d dead servers" % [ status.getDeadServerCount() ])
        ...
        puts "#{status.getLiveServerCount} servers, #{status.getDeadServerCount} dead, #{'%.4f' % status.getAverageLoad} average load"
      end
    end
    ...
    #----------------------------------------------------------------------------------------------
    # Return a new DBColumnDescriptor made of passed args
    def hcd(arg, htd)
      # String arg, single parameter constructor
      return org.javenstudio.raptor.bigdb.DBColumnDescriptor.new(arg) if arg.kind_of?(String)
      ...
      family ||= org.javenstudio.raptor.bigdb.DBColumnDescriptor.new(name.to_java_bytes)

      family.setBlockCacheEnabled(JBoolean.valueOf(arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::BLOCKCACHE))) if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::BLOCKCACHE)
      family.setScope(JInteger.valueOf(arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::REPLICATION_SCOPE))) if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::REPLICATION_SCOPE)
      family.setInMemory(JBoolean.valueOf(arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::IN_MEMORY))) if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::IN_MEMORY)
      family.setTimeToLive(JInteger.valueOf(arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::TTL))) if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::TTL)
      family.setDataBlockEncoding(org.javenstudio.raptor.bigdb.io.encoding.DataBlockEncoding.valueOf(arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::DATA_BLOCK_ENCODING))) if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::DATA_BLOCK_ENCODING)
      family.setEncodeOnDisk(JBoolean.valueOf(arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::ENCODE_ON_DISK))) if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::ENCODE_ON_DISK)
      family.setBlocksize(JInteger.valueOf(arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::BLOCKSIZE))) if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::BLOCKSIZE)
      family.setMaxVersions(JInteger.valueOf(arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::VERSIONS))) if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::VERSIONS)
      family.setMinVersions(JInteger.valueOf(arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::MIN_VERSIONS))) if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::MIN_VERSIONS)
      family.setKeepDeletedCells(JBoolean.valueOf(arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::KEEP_DELETED_CELLS))) if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::KEEP_DELETED_CELLS)
      family.setValue(COMPRESSION_COMPACT, arg.delete(COMPRESSION_COMPACT)) if arg.include?(COMPRESSION_COMPACT)
      if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::BLOOMFILTER)
        bloomtype = arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::BLOOMFILTER).upcase
        unless org.javenstudio.raptor.bigdb.regionserver.BloomType.constants.include?(bloomtype)
          raise(ArgumentError, "BloomFilter type #{bloomtype} is not supported. Use one of " + org.javenstudio.raptor.bigdb.regionserver.StoreFile::BloomType.constants.join(" ")) 
        else 
          family.setBloomFilterType(org.javenstudio.raptor.bigdb.regionserver.BloomType.valueOf(bloomtype))
        end
      end
      if arg.include?(org.javenstudio.raptor.bigdb.DBColumnDescriptor::COMPRESSION)
        compression = arg.delete(org.javenstudio.raptor.bigdb.DBColumnDescriptor::COMPRESSION).upcase
        unless org.javenstudio.raptor.bigdb.io.compress.Compression::Algorithm.constants.include?(compression)
          raise(ArgumentError, "Compression #{compression} is not supported. Use one of " + org.javenstudio.raptor.bigdb.io.compress.Compression::Algorithm.constants.join(" "))
        else
          family.setCompressionType(org.javenstudio.raptor.bigdb.io.compress.Compression::Algorithm.valueOf(compression))
        end
      end
    ...
    def online(region_name, on_off)
      # Open meta table
      meta = org.javenstudio.raptor.bigdb.client.DBTable.new(
          org.javenstudio.raptor.bigdb.TableName::META_TABLE_NAME)

      # Read region info
      # FIXME: fail gracefully if can't find the region
      region_bytes = region_name.to_java_bytes
      g = org.javenstudio.raptor.bigdb.client.Get.new(region_bytes)
      g.addColumn(org.javenstudio.raptor.bigdb.DBConstants::CATALOG_FAMILY, org.javenstudio.raptor.bigdb.DBConstants::REGIONINFO_QUALIFIER)
      hri_bytes = meta.get(g).value

      # Change region status
      hri = org.javenstudio.raptor.bigdb.util.Writables.getWritable(hri_bytes, org.javenstudio.raptor.bigdb.DBRegionInfo.new)
      hri.setOffline(on_off)

      # Write it back
      put = org.javenstudio.raptor.bigdb.client.Put.new(region_bytes)
      put.add(org.javenstudio.raptor.bigdb.DBConstants::CATALOG_FAMILY, org.javenstudio.raptor.bigdb.DBConstants::REGIONINFO_QUALIFIER, org.javenstudio.raptor.bigdb.util.Writables.getBytes(hri))
      meta.put(put)
    end
    ...
    #----------------------------------------------------------------------------------------------
    # Returns a list of namespaces in bigdb
    def list_namespace
    ...
    def create_namespace(namespace_name, *args)
      ...
      # Start defining the table
      nsb = org.javenstudio.raptor.bigdb.NamespaceDescriptor::create(namespace_name)
      args.each do |arg|
    ...
    def alter_namespace(namespace_name, *args)
      ...
      nsb = org.javenstudio.raptor.bigdb.NamespaceDescriptor::create(nsd)
