...
# Wrapper for org.javenstudio.raptor.bigdb.client.DBTable
...
EOF
    ...
    def initialize(configuration, table_name, shell)
      if @@thread_pool then
        @table = org.javenstudio.raptor.bigdb.client.DBTable.new(configuration, table_name.to_java_bytes, @@thread_pool)
      else
        @table = org.javenstudio.raptor.bigdb.client.DBTable.new(configuration, table_name)
    ...
    def _put_internal(row, column, value, timestamp = nil)
      p = org.javenstudio.raptor.bigdb.client.Put.new(row.to_s.to_java_bytes)
    ...
    #----------------------------------------------------------------------------------------------
    # Delete a cell
    def _delete_internal(row, column, timestamp = org.javenstudio.raptor.bigdb.DBConstants::LATEST_TIMESTAMP)
      _deleteall_internal(row, column, timestamp)
    end

    #----------------------------------------------------------------------------------------------
    # Delete a row
    def _deleteall_internal(row, column = nil, timestamp = org.javenstudio.raptor.bigdb.DBConstants::LATEST_TIMESTAMP)
      raise ArgumentError, "Row Not Found" if _get_internal(row).nil?
      d = org.javenstudio.raptor.bigdb.client.Delete.new(row.to_s.to_java_bytes, timestamp)
      if column
        family, qualifier = parse_column_name(column)
    ...
    def _count_internal(interval = 1000, caching_rows = 10)
      ...
      scan = org.javenstudio.raptor.bigdb.client.Scan.new
      scan.cache_blocks = false
      scan.caching = caching_rows
      scan.setFilter(org.javenstudio.raptor.bigdb.filter.FirstKeyOnlyFilter.new)
      ...
      while iter.hasNext
        ...
        yield(count, 
              org.javenstudio.raptor.bigdb.util.Bytes::toStringBinary(row.getRow))
      end
    ...
    def _get_internal(row, *args)
      get = org.javenstudio.raptor.bigdb.client.Get.new(row.to_s.to_java_bytes)
      maxlength = -1
      @converters.clear()
      ...
      unless filter.class == String
        ...
        get.setFilter(org.javenstudio.raptor.bigdb.filter.ParseFilter.new.parseFilterString(filter))
      end

      # Call bigdb for the results
      ...
      result.list.each do |kv|
        ...
        qualifier = org.javenstudio.raptor.bigdb.util.Bytes::toStringBinary(kv.getQualifier)
    ...
    #----------------------------------------------------------------------------------------------
    # Fetches and decodes a counter value from bigdb
    def _get_counter_internal(row, column)
      ...
      # Fetch cell value
      cell = result.list[0]
      org.javenstudio.raptor.bigdb.util.Bytes::toLong(cell.getValue)
    end
    ...
    def _scan_internal(args = {})
      ...
      if args.any?
        ...
        scan = if stoprow
          org.javenstudio.raptor.bigdb.client.Scan.new(startrow.to_java_bytes, stoprow.to_java_bytes)
        else
          org.javenstudio.raptor.bigdb.client.Scan.new(startrow.to_java_bytes)
        end
        ...
        unless filter.class == String
          ...
          scan.setFilter(org.javenstudio.raptor.bigdb.filter.ParseFilter.new.parseFilterString(filter))
        end
      ...
      while iter.hasNext
        ...
        row = iter.next
        key = org.javenstudio.raptor.bigdb.util.Bytes::toStringBinary(row.getRow)
    ...
    def is_meta_table?
      tn = @table.table_name
      org.javenstudio.raptor.bigdb.util.Bytes.equals(tn,
          org.javenstudio.raptor.bigdb.TableName::META_TABLE_NAME.getName)
    end

    # Returns family and (when has it) qualifier for a column name
    def parse_column_name(column)
      split = org.javenstudio.raptor.bigdb.KeyValue.parseColumn(column.to_java_bytes)
      set_converter(split) if split.length > 1
      return split[0], (split.length > 1) ? split[1] : nil
    end

    # Make a String of the passed kv
    # Intercept cells whose format we know such as the info:regioninfo in bigdb:meta
    def to_string(column, kv, maxlength = -1)
      if is_meta_table?
        if column == 'info:regioninfo' or column == 'info:splitA' or column == 'info:splitB'
          hri = org.javenstudio.raptor.bigdb.DBRegionInfo.parseFromOrNull(kv.getValue)
          return "timestamp=%d, value=%s" % [kv.getTimestamp, hri.toString]
        end
        if column == 'info:serverstartcode'
          if kv.getValue.length > 0
            str_val = org.javenstudio.raptor.bigdb.util.Bytes.toLong(kv.getValue)
          else
            str_val = org.javenstudio.raptor.bigdb.util.Bytes.toStringBinary(kv.getValue)
          end
      ...
      if kv.isDelete
        val = "timestamp=#{kv.getTimestamp}, type=#{org.javenstudio.raptor.bigdb.KeyValue::Type::codeToType(kv.getType)}"
      else
    ...
    def convert(column, kv)
      #use org.javenstudio.raptor.bigdb.util.Bytes as the default class
      klazz_name = 'org.javenstudio.raptor.bigdb.util.Bytes'
      #use org.javenstudio.raptor.bigdb.util.Bytes::toStringBinary as the default convertor
    ...
    def set_converter(column)
      ...
      parts = org.javenstudio.raptor.bigdb.KeyValue.parseColumn(column[1])
      if parts.length > 1
